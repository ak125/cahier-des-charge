    ...
    for (const file of projectIndex.files) {
      ...
      if (file.imports) {
        for (const importPath of file.imports) {
          ...
          const targetFile = projectIndex.files.find(
            (f) => f.relativePath === relativeResolvedPath
          );
...
function generateMermaidDiagram(graph: StructureGraph): string {
  ...
  const filteredEdges = graph.edges.filter(
    (e) => importantNodeIds.has(e.source) && importantNodeIds.has(e.target)
  );
...
(async function main() {
  try {
    ...
    console.log(
      '- Dans GitHub en intégrant le contenu du fichier .mmd dans un bloc de code Mermaid'
    );
