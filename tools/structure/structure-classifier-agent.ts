...
const DEFAULT_CONFIG = {
    ...
    outputMarkdownPath: path.resolve(process.cwd(), 'structure', 'structure_classification_report.md'),
    ignorePatterns: [
        '**/node_modules/**',
        '**/.git/**',
        '**/dist/**',
...
function deepContentAnalysis(filePath: string): Partial<Classification> {
    try {
        ...
        const isDeprecated = /deprecated|legacy|@obsolete|TODO:.*remove|to be removed|will be deprecated|\.bak$/.test(content + filePath);

        const isAgent = /extends\s+BaseAgent|implements\s+AgentInterface|@Agent\(|class.*Agent/.test(content);

        const isGenerator = /generate[A-Z]|Generator|createNew[A-Z]/.test(content);

        const isOrchestrator = /Orchestrator|Pipeline|Workflow|orchestrate|@Orchestration/.test(content);
        ...
        const isSharedUtil = /export\s+(const|function|class|interface|type)/.test(content) &&
            (filePath.includes('/utils/') || filePath.includes('/shared/') ||
                filePath.includes('/common/') || filePath.includes('/helpers/'));

        // Patterns existants
        const isMigration = /migration|migrate|migrat(e|ion)|\.sql$/.test(content + filePath) ||
            /database.*changes?|schema.*change/.test(content);

        const isDocker = /docker-compose|dockerfile|containeriz(e|ation)|kubernetes|k8s/.test(content + filePath.toLowerCase());

        const isTypeDefinition = /interface\s+\w+|type\s+\w+\s*=|declare\s+module|\.d\.ts$/.test(content + filePath);

        const isTool = /tool|utility|helper|cli|command|script|automation/.test(content.toLowerCase() + filePath.toLowerCase());

        const isReport = /report|statistics|analytics|metrics|log\./.test(content.toLowerCase() + filePath.toLowerCase());

        const isConfig = /config|configuration|settings|setup|\.json$|\.ya?ml$|\.conf$|\.ini$/.test(content + filePath);

        const isSEO = /seo|search\s*engine|meta\s*tag|sitemap|robots\.txt|canonical|index(ing|ed|er)/.test(content.toLowerCase());

        const isQuality = /qualit(y|ative)|lint|eslint|prettier|codequality|sonar|metric|score/.test(content.toLowerCase());

        const isAnalysis = /analy(sis|tic|ze)|evaluate|assessment|diagnostic|check|audit/.test(content.toLowerCase());

        // Nouveaux patterns de détection plus précis
        const isDashboard = /dashboard|tableau\s*de\s*bord|statistics\s*view|graph\s*view|chart\s*(component|view)|analytics\s*display/.test(
        ...
        const isIntegration = /integrat(e|ion)|connector|adapter|bridge|client\s*api|external\s*(service|api)|webhook/.test(
            content.toLowerCase() + filePath.toLowerCase());

        const isBackup = /backup|save|archive|copy|dump|snapshot/.test(
            content.toLowerCase() + filePath.toLowerCase());

        const isCI = /ci\/|continuous\s*integration|github\s*actions|gitlab\s*ci|jenkins|travis|circle\s*ci|workflow/.test(
            content.toLowerCase() + filePath.toLowerCase());

        const isSecurity = /security|authentication|authorization|auth|permission|role|access\s*control|encrypt|decrypt|hash|jwt|token/.test(
            content.toLowerCase() + filePath.toLowerCase());

        const isMonitoring = /monitor|telemetry|observability|logging|trace|metric|alert|health\s*check|status|heartbeat/.test(
            content.toLowerCase() + filePath.toLowerCase());

        const isErrorHandling = /error\s*(handler|handling)|exception|try\s*catch|catch\s*block|fallback|recovery|resilience/.test(
            content.toLowerCase());

        // Patterns de comportement temporel
        const isStable = /stable|production\s*ready|release|v\d+\.\d+\.\d+/.test(content.toLowerCase()) &&
            !isDeprecated && !content.includes('TODO:') && !content.includes('FIXME:');

        const isDeveloping = /wip|work\s*in\s*progress|draft|prototype|poc|proof\s*of\s*concept|experimental|beta/.test(
            content.toLowerCase()) || content.includes('TODO:') || content.includes('FIXME:');
        ...
        if (filePath.includes('-report') || filePath.includes('report-') ||
 ...
 */
function combineClassifications(patternClassification: Classification, deepAnalysis: Partial<Classification>): Classification {
    ...
    return {
        layer: (deepAnalysis.layer && deepConfidence > patternConfidence) ?
            deepAnalysis.layer : patternClassification.layer,

        domain: (deepAnalysis.domain && deepConfidence > patternConfidence) ?
            deepAnalysis.domain : patternClassification.domain,

        status: (deepAnalysis.status && deepConfidence > 0.7) ? // On privilégie le statut détecté profondément
            deepAnalysis.status : patternClassification.status,
...
function performNeighborhoodAnalysis(classifications: Record<string, Classification>): {
    ...
    filePaths.forEach(filePath => {
        ...
        if (classification) {
            dimensions.forEach(dimension => {
                if (classification[dimension] && classification[dimension] !== 'unknown' && classification.confidence >= 0.6) {
    ...
    filePaths.forEach(filePath => {
            ...
            if (dirStats.size > 0) {
                ...
                if (entries.length > 0) {
                    ...
                    if (firstEntry) {
                        ...
                        if (dominantRatio >= .5 && totalFiles >= 3) {
                            ...
                            classification.confidence = Math.min(.7, dominantRatio);
        ...
        if (classification.layer === 'unknown' && extension) {
            switch (extension) {
                ...
                case '.pdf':
                    ...
                    classification.layer = 'shared';
                    classification.domain = classification.domain === 'unknown' ? 'documentation' : classification.domain;
        ...
        if (classification.domain === 'unknown' || classification.layer === 'unknown') {
            ...
            const relatedFiles = filePaths.filter(p => {
                ...
                return p !== filePath && (otherName.includes(baseName) || baseName.includes(otherName.replace(path.extname(p), '')));
            });
...
${contextualAnalysis ? `- **Fichiers améliorés par analyse contextuelle :** ${contextualAnalysis.filesImproved}` : ''}
...
function aggregateResults(
    ...
    Object.entries(classifications).forEach(([file, classification]) => {
        ...
        statistics.byDomain[classification.domain] = (statistics.byDomain[classification.domain] || 0) + 1;
        statistics.byStatus[classification.status] = (statistics.byStatus[classification.status] || 0) + 1;
        statistics.bySource[classification.source] = (statistics.bySource[classification.source] || 0) + 1;
...
function validateStructureMap(structureMap: unknown): asserts structureMap is StructureMap {
    ...
    if (!defaultClassif.layer || !taxo.layer.values.includes(defaultClassif.layer)) {
        throw new Error(`La valeur par défaut pour "layer" (${defaultClassif.layer}) n'est pas valide. Valeurs possibles : ${taxo.layer.values.join(', ')}`);
    }

    if (!defaultClassif.domain || !taxo.domain.values.includes(defaultClassif.domain)) {
        throw new Error(`La valeur par défaut pour "domain" (${defaultClassif.domain}) n'est pas valide. Valeurs possibles : ${taxo.domain.values.join(', ')}`);
    }

    if (!defaultClassif.status || !taxo.status.values.includes(defaultClassif.status)) {
        throw new Error(`La valeur par défaut pour "status" (${defaultClassif.status}) n'est pas valide. Valeurs possibles : ${taxo.status.values.join(', ')}`);
    }
    ...
    for (let i = 0; i < sm.folderClassifications.length; i++) {
        ...
        if (typeof rule.pattern !== 'string') {
            throw new Error(`La règle de classification de dossier à l'index ${i} doit avoir un champ "pattern" de type string`);
        }
        if (!rule.classification || typeof rule.classification !== 'object') {
            throw new Error(`La règle de classification de dossier à l'index ${i} doit avoir un champ "classification" valide`);
        }
    ...
    for (let i = 0; i < sm.fileClassifications.length; i++) {
        ...
        if (typeof rule.pattern !== 'string') {
            throw new Error(`La règle de classification de fichier à l'index ${i} doit avoir un champ "pattern" de type string`);
        }
        if (!rule.classification || typeof rule.classification !== 'object') {
            throw new Error(`La règle de classification de fichier à l'index ${i} doit avoir un champ "classification" valide`);
        }
...
if (isMainThread) {
    async function main() {
        ...
        try {
            ...
            if (!fs.existsSync(config.structureMapPath)) {
                throw new Error(`Le fichier structure-map.json est introuvable au chemin : ${config.structureMapPath}`);
            }
            ...
            try {
                ...
                throw new Error(`Erreur lors de la lecture du fichier ${config.structureMapPath}: ${(error as Error).message}`);
            }
            ...
            try {
                ...
                throw new Error(`Le fichier ${config.structureMapPath} n'est pas un JSON valide: ${(error as Error).message}`);
            }
            ...
            try {
                ...
                throw new Error(`Validation du fichier structure-map.json échouée: ${(error as Error).message}`);
            }

            const structureMap: StructureMap = structureMapJson as StructureMap;
            console.log(`📚 Utilisation de structure-map.json v${structureMap.version} (mise à jour : ${structureMap.updated})`);
            ...
            console.log(`✅ Classification initiale terminée pour ${Object.keys(classifications).length} fichiers`);

            // Effectuer l'analyse contextuelle par voisinage
            console.log("🔍 Application de l'analyse contextuelle par voisinage...");
            const { improvedClassifications, improvements } = performNeighborhoodAnalysis(classifications);
            ...
            console.log(`⏱️ Temps d'analyse total : ${((Date.now() - startTime) / 1000).toFixed(2)} secondes`);
    ...
    files.forEach((filePath: string) => {
        try {
            ...
            const shouldDoDeepAnalysis = patternClassification.source === 'default' ||
                patternClassification.confidence < 0.7;
