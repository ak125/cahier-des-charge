...
const DEFAULT_CONFIG = {
  ...
  outputSummaryPath: path.resolve(
    process.cwd(),
    'structure',
    'structure_classification_summary.json'
  ),
  ...
  preservePaths: ['packages', 'apps', 'scripts', 'tools', 'agents', 'src'],
};
...
function generateTargetPath(file: string, classification: FileClassification, config: any): string {
  ...
  const _fileExt = path.extname(file).toLowerCase();
  ...
  for (const preservePath of config.preservePaths) {
    if (file.startsWith(`${preservePath}/`)) {
      ...
      const _relativePath = file.substring(preservePath.length + 1);
...
function generateSuggestionMarkdown(plan: MigrationPlan): string {
  ...
  const byTargetFolder: Record<
    string,
    Array<{ source: string; destination: string; confidence: number }>
  > = {};
...
- **Fichiers à déplacer:** ${statistics.totalMoves} (${(
    (statistics.totalMoves / statistics.totalFiles) *
    100
  ).toFixed(2)}%)
...
`;
    ...
    for (const file of files.sort((a, b) => a.source.localeCompare(b.source))) {
      markdown += `| \`${file.source}\` | \`${file.destination}\` | ${(
        file.confidence * 100
      ).toFixed(0)}% |\n`;
    }
...
async function main() {
  ...
  try {
    ...
    console.log(
      `📊 Plan généré: ${migrationPlan.statistics.totalMoves} fichiers à déplacer sur ${migrationPlan.statistics.totalFiles} fichiers analysés`
    );
    ...
    if (!config.dryRun) {
      ...
      console.log(
        `⚠️ L'exécution directe n'est pas encore implémentée. Utilisez le script shell généré.`
      );
    }
