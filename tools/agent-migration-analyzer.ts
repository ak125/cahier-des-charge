...
function findAgentFiles(agentTypePattern: string): string[] {
  try {
    const cmd = `find ${searchDirectories.join(
      ' '
    )} -type f -name "*${agentTypePattern}*.ts" | grep -v "node_modules" | grep -v ".test.ts" | grep -v ".d.ts" | sort`;
...
function generateMigrationRecommendation(agentTypeInfo: {
  name: string;
  type: string;
  ...
  const bestVersion = agentFiles.sort((a, b) => {
...
async function main() {
  ...
  for (const recommendation of recommendations) {
    console.log(
      `${recommendation.agentType}: ${recommendation.bestVersion.path} -> ${recommendation.targetPath}`
    );
  }
