...
const determineAgentType = (agentName: string, filePath: string): string => {
  ...
  if (
    ...
    lowerName.includes('analyseur')
  ) {
    return 'analyzer';
  }
  if (
    lowerName.includes('generator') ||
    lowerPath.includes('generator') ||
    lowerName.includes('générateur')
  ) {
    return 'generator';
  }
  if (
    lowerName.includes('validator') ||
    lowerPath.includes('validator') ||
    lowerName.includes('validateur')
  ) {
    return 'validator';
  }
  if (
    ...
    lowerName.includes('parseur')
  ) {
    return 'parser';
  }
  if (
    lowerName.includes('orchestrator') ||
    lowerPath.includes('orchestrator') ||
    lowerName.includes('orchestrateur')
  ) {
    return 'orchestrator';
  }
  if (
    lowerName.includes('monitor') ||
    lowerPath.includes('monitor') ||
    lowerName.includes('moniteur')
  ) {
    return 'monitor';
  }
  if (lowerName.includes('bridge') || lowerPath.includes('bridge') || lowerName.includes('pont')) {
    return 'bridge';
  }
  if (
    lowerName.includes('adapter') ||
    lowerPath.includes('adapter') ||
    lowerName.includes('adaptateur')
  ) {
    return 'adapter';
  }
...
const determineAgentDomain = (agentName: string, filePath: string): string => {
  ...
  if (lowerName.includes('seo') || lowerPath.includes('seo')) {
    return 'seo';
  }
  if (
    lowerName.includes('qa') ||
    lowerPath.includes('qa') ||
    lowerName.includes('quality') ||
    lowerPath.includes('quality')
  ) {
    return 'quality';
  }
  if (lowerPath.includes('dashboard') || lowerName.includes('dashboard')) {
    return 'dashboard';
  }
...
const archiveFile = (filePath: string, dryRun = false): void => {
  ...
  while (fs.existsSync(uniqueArchivePath)) {
    uniqueArchivePath = path.join(
      LEGACY_DIR,
      `${path.basename(fileName, path.extname(fileName))}_${counter}${path.extname(fileName)}`
    );
    counter++;
  }
...
const updateImports = async (oldPath: string, newPath: string, dryRun = false): Promise<void> => {
  ...
  try {
    ...
    const _oldRelativePath = path.relative(BASE_DIR, oldImportPath);
    const _newRelativePath = path.relative(BASE_DIR, newImportPath);
    ...
    for (const file of files) {
      ...
      while ((match = importRegex.exec(content)) !== null) {
        ...
        if (fullImportPath.includes(path.basename(oldImportPath))) {
          const newImport = fullImportPath.replace(
            path.basename(oldImportPath),
            path.basename(newImportPath)
          );
...
const consolidateAgents = async (
  ...
  let eligibleAgents = agentAudit.filter(
    (agent) =>
      agent.files.length > 1 &&
      (agent.recommendedAction === 'CONSOLIDATION_URGENTE' || agent.files.length >= threshold)
  );

  // Limiter aux N agents les plus redondants si spécifié
  if (topN && topN > 0) {
    eligibleAgents = eligibleAgents.sort((a, b) => b.files.length - a.files.length).slice(0, topN);
  }
  ...
  for (const agent of eligibleAgents) {
    ...
    if (!fs.existsSync(sourceOfTruth)) {
      console.log(
        `Le fichier principal ${sourceOfTruth} n'existe pas. Recherche d'une alternative...`
      );
    ...
    console.log(
      `Classification: couche=${classification.layer}, domaine=${classification.domain}, statut=${classification.status}`
    );
...
const parseArgs = (): {
  ...
  return {
    ...
    threshold: parseFloat(
      args.find((arg) => arg.startsWith('--threshold='))?.split('=')[1] || '3.0'
    ),
