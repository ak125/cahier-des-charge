...
async function main() {
  try {
    ...
    console.log(`🔌 Connexion à MySQL: ${mysqlConfig.host}:${mysqlConfig.port}/${mysqlConfig.database}`);
    ...
    if (options.tables) {
      ...
      console.log(`📋 Tables spécifiées dans le fichier de configuration: ${tablesToExport.join(', ')}`);
    } else {
    ...
    for (const tableName of tablesToExport) {
      ...
      while (true) {
        try {
          ...
          console.log(`🔄 Exportation en cours: ${Math.min(offset, totalRows)}/${totalRows} lignes`);
          
        } catch (err) {
          console.error(`Erreur lors de l'exportation du lot à partir de l'offset ${offset}: ${err}`);
          errorCount++;
      ...
      await new Promise<void>((resolve, reject) => {
        writeStream.on('finish', () => {
          console.log(`✅ Export terminé pour la table ${tableName}: ${exportedCount} lignes exportées, ${errorCount} erreurs`);
          stats.push({
            tableName,
            rowCount: totalRows,
            exportedCount,
    ...
    for (const stat of stats) {
      console.log(`   ${stat.tableName}: ${stat.exportedCount}/${stat.rowCount} lignes exportées, ${stat.errors} erreurs`);
    }
