...
BACKUP_COUNT=$(find /workspaces/migration-ai-pipeline/structure -path "*/backup-*" -type d | wc -l)
...
find /workspaces/migration-ai-pipeline/structure -path "*/backup-*" -type d | sort

echo "Suppression des dossiers de sauvegarde en cours..."

# Supprimer tous les dossiers de sauvegarde
find /workspaces/migration-ai-pipeline/structure -path "*/backup-*" -type d -exec rm -rf {} \; 2>/dev/null || true

# Vérifier qu'il ne reste plus de dossiers de sauvegarde
REMAINING=$(find /workspaces/migration-ai-pipeline/structure -path "*/backup-*" -type d | wc -l)
