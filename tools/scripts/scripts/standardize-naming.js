...
// Fichiers et dossiers spécifiques à exclure
const EXEMPTIONS = [
    'README.md',
    'LICENSE',
    'Dockerfile',
...
function shouldConvert(name) {
    // Vérifier si le nom est dans la liste des exemptions
    if (EXEMPTIONS.includes(name)) {
        return false;
    }

    // Vérifier si c'est un fichier spécial (commençant par un point)
    if (name.startsWith('.')) {
        return false;
    }
...
async function main() {
    ...
    try {
        ...
        if (!DRY_RUN) {
            ...
            console.log(`\nEn mode simulation, ${renamePaths.size} fichiers/répertoires seraient renommés.`);
            if (VERBOSE) {
                for (const [oldPath, newPath] of renamePaths.entries()) {
