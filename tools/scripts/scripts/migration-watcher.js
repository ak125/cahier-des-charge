...
async function handleNewFile(filePath) {
  try {
    ...
    console.error(
      chalk.red(`❌ Erreur lors du traitement du fichier ${filePath}: ${error.message}`)
    );
  }
}
...
async function checkIfMigratedFile(filePath) {
  try {
    ...
    console.error(
      chalk.red(`❌ Erreur lors de la vérification du fichier ${filePath}: ${error.message}`)
    );
    return false;
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    generateProcess.on('close', (code) => {
      if (code === 0) {
        ...
        console.error(
          chalk.red(`❌ Erreur lors de la génération du fichier d'audit pour: ${filePath}`)
        );
