    ...
    try {
      ...
      try {
        const response = await axios.get(
          `${N8N_API_URL}?filters=${encodeURIComponent(
            JSON.stringify({ name: workflowData.name })
          )}`
        );
        existingWorkflow = response.data.data.find((wf) => wf.name === workflowData.name);
      } catch (error) {
        console.log(
          chalk.yellow(`⚠️ Impossible de vérifier l'existence du workflow: ${error.message}`)
        );
      }
      ...
      if (existingWorkflow) {
        ...
        try {
          ...
          console.error(
            chalk.red(
              `❌ Erreur lors de la mise à jour du workflow ${workflowData.name}: ${error.message}`
            )
          );
        }
      } else {
        ...
        try {
          ...
          console.error(
            chalk.red(
              `❌ Erreur lors de l'importation du workflow ${workflowData.name}: ${error.message}`
            )
          );
        }
      }
