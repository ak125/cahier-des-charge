...
function groupAgentsByBaseName(agents: AgentFile[]): AgentGroup[] {
  ...
  return Array.from(groupMap.entries()).map(([baseName, files]) => {
    ...
    const versions = Array.from(
      new Set(files.map((f) => f.version).filter((v) => v !== null) as string[])
    );
    ...
    for (const file of files) {
      try {
        const count = parseInt(
          execSync(`git grep -l "${file.name}" --exclude="${path.basename(file.path)}" | wc -l`, {
            encoding: 'utf8',
          }).trim(),
          10
        );
...
function calculateVersioningScore(groups: AgentGroup[]): VersioningScore {
  ...
  for (const group of groups) {
    ...
    if (
      ...
      !group.versions.every((v) => v.startsWith('1.') || v.startsWith('2.'))
    ) {
  ...
  const consistentVersioning =
    totalAgents > 0 ? 100 - (inconsistentVersions.length / totalAgents) * 100 : 100;

  const backupCleanliness =
    totalAgents > 0 ? 100 - (unnecessaryBackups.length / totalAgents) * 100 : 100;

  const duplicateReduction = totalAgents > 0 ? 100 - (duplicates.length / totalAgents) * 100 : 100;

  const explicitVersionDeclaration =
    totalAgents > 0 ? 100 - (missingVersions.length / totalAgents) * 100 : 100;
...
async function main() {
  ...
  const agentPatterns = [`${AGENTS_DIR}/**/*.{ts,js}`, `${PACKAGES_DIR}/mcp-agents/**/*.{ts,js}`];
  ...
  await fs.writeJson(
    path.join(REPORT_DIR, 'agent_versions.json'),
    {
  ...
  await fs.writeJson(path.join(REPORT_DIR, 'score_versioning.json'), versioningScore, {
    spaces: 2,
  });
