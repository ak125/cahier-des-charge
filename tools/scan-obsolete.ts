...
async function isImported(filePath: string): Promise<boolean> {
  try {
    ...
    const { stdout } = await execPromise(
      `grep -r "import.*${searchPattern}" --include="*.{ts,tsx,js,jsx}" .`
    );
...
async function classifyFile(filePath: string): Promise<FileStatus> {
  ...
  if (obsoleteSuffixes.some((suffix) => filename.includes(suffix))) {
    return {
      ...
      status: '⚠️ Obsolète',
      reason: `Nom contient un suffixe de version/doublon (${obsoleteSuffixes.find((suffix) =>
  ...
  if (generatedPaths.some((part) => pathParts.includes(part))) {
    if (
      auditExtensions.includes(extension) ||
      auditExtensions.some((ext) => filename.includes(ext))
    ) {
      return {
        filePath: relativePath,
...
function generateReport(files: FileStatus[]): string {
  ...
  report += '```bash\n';
  obsolete
    .filter((f) => f.reason.includes('suffixe') || f.reason.includes('doublon'))
    .forEach((file) => {
  ...
  archive
    .filter((f) => !f.filePath.includes('backup') && !f.filePath.includes('archive'))
    .forEach((file) => {
      const targetDir = file.filePath.includes('sql')
        ? 'archives/sql-legacy/'
        : file.filePath.includes('php')
          ? 'archives/php-legacy/'
  ...
  report +=
    '   - Centraliser les générateurs dans un dossier unique (idéalement `packagesDoDotmcp-agents/generators/`)\n';
...
async function main() {
  ...
  console.log(
    `Classification : ${allFiles.filter((f) => f.status === '✅ Actif').length} actifs, ${
      allFiles.filter((f) => f.status === '🌀 Généré (non validé)').length
    } générés, ${allFiles.filter((f) => f.status === '⚠️ Obsolète').length} obsolètes, ${
      allFiles.filter((f) => f.status === '📦 Archive').length
    } archives`
  );
}
