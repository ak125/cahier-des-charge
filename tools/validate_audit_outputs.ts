...
async function validateAuditOutputs(
  ...
  console.log(`Vérification terminée: ${auditFiles.length} fichiers d'audit attendus, ${missingFiles.length} manquants`);
  ...
  for (const dup of duplicateAudits) {
    issues.push({
      ...
      sourceFile: dup,
      affectedFiles: auditFiles
        .filter(f => f.sourceFile === dup)
        .map(f => f.filePath)
    });
  }
...
function getExpectedAuditFiles(phpFileName: string, basePath: string): AuditFile[] {
  ...
  expectedFiles.push({
    filePath: path.join(basePath, 'docs', 'cahier-des-charges', 'audits', `${phpFileName}.audit.md`),
    type: 'audit.md',
    exists: false
  });
  ...
  expectedFiles.push({
    filePath: path.join(basePath, 'docs', 'cahier-des-charges', 'audits', `${phpFileName}.backlog.json`),
    type: 'backlog.json',
    exists: false
  });
  ...
  expectedFiles.push({
    filePath: path.join(basePath, 'docs', 'cahier-des-charges', 'audits', `${phpFileName}.impact_graph.json`),
...
async function main() {
  try {
    ...
    const outputPath = process.argv[3] || '/workspaces/cahier-des-charge/reports/audit_consistency_report.json';
    ...
    if (report.issues.length > 0) {
      ...
      console.log('Vous pouvez exécuter recheck_missing_outputs.ts pour générer les fichiers manquants.');
    } else {
