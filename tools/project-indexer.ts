...
async function analyzeFile(filePath: string): Promise<ProjectFile> {
  ...
  try {
    ...
    const { stdout } = await execAsync(
      `git grep -l "${fileName}" --exclude="${path.basename(filePath)}" | wc -l`
    );
...
function determineDomain(filePath: string, content: string): string {
  ...
  if (filePath.includes('php') || filePath.includes('migration') || content.includes('migration')) {
    return 'migration';
  }
  if (filePath.includes('qa') || content.includes('quality') || content.includes('QA')) {
    return 'qa';
  }
  if (
  ...
  if (filePath.includes('workflow') || content.includes('workflow')) {
    return 'workflow';
  }
  return 'general';
}
...
function calculateStructureScore(files: ProjectFile[]): StructureScore {
  const totalFiles = files.filter((f) =>
    ['.ts', '.tsx', '.js', '.jsx'].some((ext) => f.path.endsWith(ext))
  ).length;
  ...
  const testedFiles = files.filter((f) =>
    files.some(
      (tf) => tf.type === 'test' && tf.path.includes(path.basename(f.path, path.extname(f.path)))
    )
  ).length;
  ...
  const interfaceImplementation =
    totalFiles > 0 ? (interfaceImplementingFiles / totalFiles) * 100 : 0;
  const testCoverage = totalFiles > 0 ? (testedFiles / totalFiles) * 100 : 0;
  const redundancyFactor =
    totalFiles > 0 ? 100 - (redundantComponents.length / totalFiles) * 100 : 100;
...
function calculateNamingConsistency(files: ProjectFile[]): number {
  ...
  Object.entries(layerFiles).forEach(([layer, files]) => {
    ...
    const consistentlyNamed = files.filter((f) => {
      ...
      return (
        baseName.startsWith(expectedPrefix) || baseName.toLowerCase().includes(layer.toLowerCase())
      );
    }).length;
...
function analyzeRedundancies(files: ProjectFile[]): RedundancyReport {
  ...
  filesByDomainAndType.forEach((groupFiles, _key) => {
...
function identifyObsoletes(files: ProjectFile[]): ObsoletesReport {
  ...
  files.forEach((file) => {
    ...
    if (
      ...
      (file.usageCount === 0 && !file.path.endsWith('index.ts'))
    ) {
      obsoleteFiles.push({
    ...
    if (similarFiles.length > 0) {
      ...
      const newerVersions = similarFiles.filter(
        (f) =>
          new Date(f.lastModified) > new Date(file.lastModified) &&
          (f.migrated || // Déjà migré vers MCP OS
            f.path.includes('mcp-agents/') || // Dans le nouveau répertoire
...
async function main() {
  ...
  await fs.writeJson(path.join(REPORT_DIR, 'redundancy_report.json'), redundancyReport, {
    spaces: 2,
  });
