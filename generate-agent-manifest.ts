 ...
 * Par exemple: QAAnalyzer -> qa-analyzer
 */
...
async function extractAgentMetadata(filePath: string): Promise<ExtractedAgentMetadata | null> {
  try {
    ...
    const modulePath = '@fafa/mcp-agents/' + path.basename(filePath, path.extname(filePath));
...
async function findAndExtractAgents(config: GeneratorConfig): Promise<AgentManifestEntry[]> {
  ...
  for (const file of files) {
    ...
    if (metadata) {
      ...
      const modulePath = `@fafa/mcp-agents/${moduleName}`;
      ...
      agents.push({
        ...
        apiEndpoint: metadata.apiEndpoint || `/mcp/${metadata.id}`,
