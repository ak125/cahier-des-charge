...
} from '../../core/interfaces/validator-agent';
...
export class SEOCheckerAgent extends BaseValidatorAgent {
  ...
  private async checkMetadataInFiles(files: string[], result: SEOValidationResult): Promise<void> {
    ...
    for (const file of files) {
      ...
      const hasDescription = /name="description"|content=".*?"|description:|"description":/.test(
        content
      );
    ...
    for (const file of files) {
      ...
      if (!hasCanonical) {
        ...
        const canonicalMatch = content.match(
          /href="([^"]+)"\s+rel="canonical"|canonical:\s*["'](.+?)["']|"canonical":\s*"(.+?)"/
        );
        if (canonicalMatch) {
   ...
   */
  private async applyFixes(_files: string[], result: SEOValidationResult): Promise<void> {
        ...
        if (canonicalMatch) {
          ...
          if (!currentCanonical.startsWith('http')) {
            ...
            const modified = content.replace(
              new RegExp(
                `(href="|canonical:\\s*["']|"canonical":\\s*")${currentCanonical.replace(
                  /[.*+?^${}()|[\]\\]/g,
                  '\\$&'
                )}("|')`
              ),
            ...
            this.logger.info(
              `URL canonique corrigée dans ${file}: ${currentCanonical} -> ${correctedCanonical}`
            );
          }
        }
      }
