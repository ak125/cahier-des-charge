...
function generateNewPackageName(packagePath, originalName) {
    try {
        ...
        const mainCategory = relativePath[0]; // ex: mcp-agents

        let packageType = '';
        let typeIndex = -1;

        // Rechercher un type de package connu dans le chemin
        const knownTypes = ['analyzers', 'generators', 'validators', 'orchestrators', 'bridges', 'monitors'];
        ...
        if (originalName.startsWith('@mcp/')) {
            ...
            if (packageType) {
                ...
                if (!baseName.includes(packageType)) {
                    ...
                    if (pathContext && pathContext !== 'business' &&
                        pathContext !== 'coordination' && pathContext !== 'mcp-agents') {
...
function main() {
    ...
    try {
        ...
        console.log(`  ${updatedFiles.length} fichiers package.json mis à jour avec les nouvelles références.`);
