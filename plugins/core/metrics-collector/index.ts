...
export class MetricsCollector implements McpPlugin {
  ...
  name = 'Collecteur de Métriques MCP';
  version = '1.0.0';
  description =
    'Plugin qui collecte des métriques de performance pour tous les agents et les migrations MCP';
  author = 'Équipe MCP';
    ...
    onAgentComplete: async (agentId: string, result: any): Promise<void> => {
      ...
      if (lastMetric && lastMetric.endTime === null) {
        ...
        this.context.logger.debug(
          `Fin d'exécution de l'agent ${agentId}: ${lastMetric.duration}ms`
        );
  ...
  private async saveMetrics(): Promise<void> {
    try {
      ...
      await fs.writeFile(metricsPath, JSON.stringify(this.metricsData, null, 2), 'utf-8');
  ...
  private async exportMetricsToFile(format: 'json' | 'csv' = 'json'): Promise<{ path: string }> {
    ...
    if (format === 'json') {
      ...
      let csvContent =
        'agentId,timestamp,startTime,endTime,duration,inputSize,outputSize,success,memoryUsage\n';
  ...
  private async generateReport(): Promise<{ path: string }> {
    ...
    report += `- **Exécutions réussies**: ${successfulAgentExecutions} (${Math.round(
      (successfulAgentExecutions / totalAgentExecutions) * 100 || 0
    )}%)\n`;
    report += `- **Exécutions échouées**: ${failedAgentExecutions} (${Math.round(
      (failedAgentExecutions / totalAgentExecutions) * 100 || 0
    )}%)\n`;
    ...
    const avgMigrationDuration =
      allMigrationMetrics
        .filter((m) => m.duration !== null)
        .reduce((acc, m) => acc + (m.duration || 0), 0) / totalMigrations || 0;

    report += `- **Nombre total de migrations**: ${totalMigrations}\n`;
    report += `- **Migrations réussies**: ${successfulMigrations} (${Math.round(
      (successfulMigrations / totalMigrations) * 100 || 0
    )}%)\n`;
    report += `- **Migrations échouées**: ${failedMigrations} (${Math.round(
      (failedMigrations / totalMigrations) * 100 || 0
    )}%)\n`;
    ...
    for (const agent of agentAvgDurations.slice(0, 5)) {
      ...
      const successRate =
        (metrics.filter((m) => m.success === true).length / metrics.length) * 100 || 0;

      report += `| ${agent.agentId} | ${Math.round(agent.avgDuration)}ms | ${
        agent.executions
    ...
    for (const migration of migrationDurations.slice(0, 5)) {
      report += `| ${migration.migrationId} | ${migration.name} | ${Math.round(
        migration.duration / 1000
      )}s |\n`;
    }
    ...
    if (this.metricsData.system.length > 0) {
      ...
      const avgMemory =
        this.metricsData.system.reduce((acc, m) => acc + m.memoryUsage.heapUsed, 0) /
        this.metricsData.system.length;

      report += `- **Utilisation mémoire actuelle**: ${Math.round(
        lastMetric.memoryUsage.heapUsed / 1024 / 1024
      )}MB\n`;
