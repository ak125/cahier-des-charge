...
import { McpPlugin, PluginContext, PluginLogger, PluginManifest } from './types';
...
export class PluginManager {
    ...
    for (const [pluginId, plugin] of this.plugins.entries()) {
      if (
        ...
        try {
          ...
          this.logger.error(
            `Erreur dans le hook ${hookName} du plugin ${pluginId}: ${err.message}`
          );
          // On continue avec les autres plugins
        }
      }
  ...
  private validatePlugin(plugin: McpPlugin, manifest: PluginManifest): boolean {
    ...
    if (plugin.id !== manifest.id) {
      this.logger.error(
        `L'ID du plugin ${plugin.id} ne correspond pas à l'ID du manifest ${manifest.id}`
      );
      return false;
    }
   ...
   */
  private createPluginApi(_pluginId: string): any {
