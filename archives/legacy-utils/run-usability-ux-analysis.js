...
const puppeteer = require('puppeteerstructure-agent'); // Nécessite d'être installé
  ...
  try {
    ...
    pageResult.tests.focusStates = await page.evaluate(() => {
      ...
      return {
        ...
        score: Math.round(percentage),
        details: `${visibleFocus}/${testedElements} éléments avec focus visible (${percentage.toFixed(
          1
        )}%)`,
      };
    });
    ...
    pageResult.tests.readability = await page.evaluate(() => {
      ...
      const textElements = Array.from(
        document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, td, div > text:not(div *)')
      );
    ...
    pageResult.tests.navConsistency = await page.evaluate(() => {
      const navElements = Array.from(
        document.querySelectorAll('nav, [role="navigation"], header, .header, .nav, .menu')
      );
    ...
    pageResult.tests.interactionCount = await page.evaluate(() => {
      ...
      const searchForm = document.querySelector(
        'form[role="search"], input[type="search"], input[name*="search"]'
      );
      ...
      return {
        ...
        score,
        details: easilyAccessible
          ? 'Fonctionnalité principale facilement accessible'
          : 'Fonctionnalité principale difficile à trouver',
      };
    });
...
async function analyzeUserExperience(urls) {
  ...
  for (const urlInfo of urls) {
    ...
    try {
      ...
      const metricScores = [fidScore, clsScore, inpScore, ttfbScore];
...
function saveResults(usabilityResults, uxResults) {
  ...
  console.log(
    `✅ Résultats sauvegardés dans ${CONFIG.usabilityOutputFile} et ${CONFIG.uxOutputFile}`
  );
}
