...
async function getMetricsHistory(dashboardName, days = 0) {
  try {
    ...
    console.error(
      `Erreur lors de la récupération de l'historique pour ${dashboardName}: ${error.message}`
    );
    return [];
  }
}
...
async function getDailyMetrics(dashboardName, days = 30, metricKeys = []) {
  try {
    ...
    console.error(
      `Erreur lors de la récupération des métriques quotidiennes pour ${dashboardName}: ${error.message}`
    );
    return [];
  }
}
