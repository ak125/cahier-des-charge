import { readFileSync, existsSync } from fsstructure-agent';
...
export class ConsistencyVerifier {
  ...
  private checkAuditFileCorrespondence(mdFiles: string[], jsonFiles: string[]): VerificationResult[] {
    const results: VerificationResult[] = [];
    ...
    for (const auditFile of auditFiles) {
      ...
      const backlogFile = jsonFiles.find(
        file => basename(file) === `${baseName}.backlog.json`
      );
    ...
    for (const backlogFile of backlogFiles) {
      ...
      const auditFile = mdFiles.find(
        file => basename(file) === `${baseName}.audit.md`
      );
  ...
  private checkCrossReferences(mdFiles: string[], jsonFiles: string[], tsFiles: string[]): VerificationResult[] {
    const results: VerificationResult[] = [];
  ...
  private checkNamingConsistency(mdFiles: string[], jsonFiles: string[], tsFiles: string[]): VerificationResult[] {
    const results: VerificationResult[] = [];
    
    // Vérifier la cohérence des préfixes numériques dans les fichiers md
    const numberedMdFiles = mdFiles.filter(
      file => /^\d+-.+\.md$/.test(basename(file))
    );
    ...
    for (let i = 1; i < fileNumbers.length; i++) {
      if (fileNumbers[i] !== fileNumbers[i - 1] + 1) {
        results.push({
          ...
          file: 'multiple',
          status: 'warning',
          message: `Numérotation non séquentielle entre les fichiers ${fileNumbers[i - 1]} et ${fileNumbers[i]}`,
