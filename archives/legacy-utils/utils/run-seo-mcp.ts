...
import { createSEOMCPController } from '../agents/seoDoDotmcp-controllerstructure-agent';
import { Command } from './commanderstructure-agent';
...
program
  ...
  .option(
    '-m, --meta-dir <dir>',
    'Répertoire pour les fichiers meta.ts',
  ...
  .option(
    '-h, --htaccess <paths>',
    'Chemins des fichiers .htaccess séparés par des virgules',
    './legacy/.htaccess'
  )
  .option(
    '-n, --nestjs-config <path>',
    'Chemin du fichier de configuration NestJS pour les redirections'
  )
  .option('--config <path>', 'Fichier de configuration JSON', './config/seoDoDotmcp.json');
...
async function main() {
  try {
    ...
    const finalConfig = {
      ...
      remixTargetDir: path.resolve(
        options.remixDir || config.remixTargetDir || './apps/frontend/app/routes'
      ),
      ...
      validateCanonicals:
        options.canonicals !== undefined ? options.canonicals : config.validateCanonicals !== false,
      validateRedirects:
        options.redirects !== undefined ? options.redirects : config.validateRedirects !== false,
      ...
      nestJSConfigPath: options.nestjsConfig
        ? path.resolve(options.nestjsConfig)
        : config.nestJSConfigPath
          ? path.resolve(config.nestJSConfigPath)
          : undefined,
    ...
    if (missingConfigs.length > 0) {
      console.warn(
        `⚠️ Attention: Les configurations suivantes sont manquantes: ${missingConfigs.join(', ')}`
      );
    ...
    console.log(
      '- Validation canonicals:',
      finalConfig.validateCanonicals ? 'Activée' : 'Désactivée'
    );
    console.log(
      '- Validation redirections:',
      finalConfig.validateRedirects ? 'Activée' : 'Désactivée'
    );
    ...
    console.log(
      '- Rapport généré dans:',
      path.join(finalConfig.outputDir, 'seoDoDotmcp-report.md')
    );
