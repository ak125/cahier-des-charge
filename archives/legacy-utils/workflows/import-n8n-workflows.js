...
const _API_KEY = process.env.N8N_API_KEY || ''; // Pour l'authentification par API key si configurée
...
async function getAuthToken() {
  try {
    ...
    try {
      ...
      if (restLoginResponse.data?.token) {
    ...
    if (fs.existsSync(configDir)) {
      ...
      for (const configFile of n8nConfigFiles) {
        ...
        try {
          ...
          console.error(
            `❌ Erreur lors de la lecture/importation du fichier ${configFile}:`,
            err.message
          );
        }
      }
...
async function importWorkflow(workflow, source, auth) {
  try {
    ...
    try {
      const _response = await axios.post(endpoint, apiData, {
        headers: {
          'Content-Type': 'application/json',
    ...
    if (error.response) {
      if (error.response.status === 409) {
        ...
        console.error(
          `❌ Erreur lors de l'importation du workflow (${error.response.status}):`,
          error.message
        );
