...
BACKUP_DIR="${WORKSPACE_ROOT}/backups/structure_optimization_${TIMESTAMP}"

# Mise à jour: Stocker tous les logs et rapports dans le dossier de backup pour éviter les problèmes lors de la fusion
REPORTS_DIR="${WORKSPACE_ROOT}/reports"
LOG_DIR="${BACKUP_DIR}/logs"  # Déplacer les logs dans le dossier de sauvegarde
LOG_FILE="${LOG_DIR}/structure_optimization_${TIMESTAMP}.log"
REPORT_FILE="${BACKUP_DIR}/structure_optimization_report.md"
FINAL_REPORT_PATH="${REPORTS_DIR}/structure_optimization_${TIMESTAMP}.md"
...
# Fonction de logging modifiée pour fonctionner correctement même si les dossiers sont déplacés
log() {
  local message="$1"
  local timestamp=$(date +"%Y-%m-%d %H:%M:%S")
  echo -e "[${timestamp}] $message" | tee -a "${LOG_FILE}"
  # Uniquement pour l'affichage console, pas besoin d'ajouter au fichier de log
  if [[ "$2" == "console" ]]; then
    echo -e "$message"
  fi
}
...
generate_final_report() {
  ...
  # Copier le rapport dans le dossier reports (qui peut avoir été déplacé)
  mkdir -p "$(dirname "${FINAL_REPORT_PATH}")"
  cp "${REPORT_FILE}" "${FINAL_REPORT_PATH}"
  
  log "${GREEN}✅ Rapport final généré: ${FINAL_REPORT_PATH#${WORKSPACE_ROOT}/}${NC}"
}
...
main() {
  ...
  # Utiliser echo directement pour éviter les problèmes de dossier de logs fusionné
  echo -e "${GREEN}======================================================${NC}"
  echo -e "${GREEN}✅ Optimisation de la structure du projet terminée!${NC}"
  echo -e "${GREEN}   - Sauvegarde: ${BACKUP_DIR}${NC}"
  echo -e "${GREEN}   - Rapport: ${FINAL_REPORT_PATH#${WORKSPACE_ROOT}/}${NC}"
  echo -e "${GREEN}   - Log: ${LOG_FILE#${WORKSPACE_ROOT}/}${NC}"
  echo -e "${GREEN}======================================================${NC}"
  
  echo ""
  echo "Les dossiers vides ont été supprimés."
  echo "Les dossiers similaires ont été fusionnés selon la configuration."
  echo "Consultez le rapport pour plus de détails: ${FINAL_REPORT_PATH#${WORKSPACE_ROOT}/}"
}
