 ...
 * depuisDotn8N ou via la ligne de commande.
 *
 * Usage:
...
import { SQLPrismaMigrationPlanner } from '../agents/migration/SqlPrismaMigrationPlannerstructure-agent';
...
program
  ...
  .option(
    '--schema-raw <path>',
    'Chemin vers le fichier schema_raw.json',
    './reports/schema_raw.json'
  )
  .option(
    '--prisma-schema <path>',
    'Chemin vers suggested_schema.prisma',
    './reports/suggested_schema.prisma'
  )
  .option(
    '--php-sql-links <path>',
    'Chemin vers php_sql_links.json',
    './reports/php_sql_links.json'
  )
