 ...
 */
async function collectAuditFiles(): Promise<{
  [slug: string]: { audit?: string; backlog?: string; impactGraph?: string };
}> {
  ...
  const impactGraphFiles = glob.sync(
 ...
 */
async function analyzeMetricsByAudit(auditFiles: {
  [slug: string]: { audit?: string; backlog?: string; impactGraph?: string };
}): Promise<{ [slug: string]: { metrics: AuditMetrics; files: string[]; lastUpdated: string } }> {
  const result: {
    [slug: string]: { metrics: AuditMetrics; files: string[]; lastUpdated: string };
  } = {};

  for (const [slug, files] of Object.entries(auditFiles)) {
    // Liste des fichiers pour cet audit
    const filesList = [files.audit, files.backlog, files.impactGraph].filter(Boolean) as string[];

    // Dernière mise à jour (date de modification la plus récente)
    const lastModified = Math.max(...filesList.map((file) => fs.statSync(file).mtime.getTime()));
 ...
 */
function analyzeCompleteness(files: {
  audit?: string;
  backlog?: string;
  impactGraph?: string;
}): number {
 ...
 */
function analyzeCoverage(files: {
  audit?: string;
  backlog?: string;
  impactGraph?: string;
}): number {
  ...
  if (files.backlog) {
    ...
    try {
      ...
      if (content.tasks && Array.isArray(content.tasks)) {
        ...
        const taskTypes = new Set(content.tasks.map((t) => (t.type || '').toLowerCase()));
 ...
 */
function analyzeMigrationReadiness(files: {
  audit?: string;
  backlog?: string;
  impactGraph?: string;
}): number {
 ...
 */
function calculateAverageMetrics(metricsMap: {
  [slug: string]: { metrics: AuditMetrics; files: string[]; lastUpdated: string };
}): AuditMetrics {
 ...
 */
async function analyzeTrends(currentMetrics: {
  [slug: string]: { metrics: AuditMetrics; files: string[]; lastUpdated: string };
 ...
 */
function generateRecommendations(metricsMap: {
  [slug: string]: { metrics: AuditMetrics; files: string[]; lastUpdated: string };
...
async function updateHistory(report: AuditQualityReport): Promise<void> {
  ...
  console.log(
    chalk.green(`📚 Historique des rapports de qualité mis à jour: ${CONFIG.historyFile}`)
  );
}
...
function displayResults(report: AuditQualityReport): void {
  ...
  console.log(
    `  Préparation à la migration: ${report.averageMetrics.migrationReadiness.toFixed(1)}%`
  );
