...
export async function mapPhpSqlSync(
  ...
  updateSchemaMigrationDiff(path.join(outputDir, 'schema_migration_diff.json'), fieldMappings);
...
function generateFieldMappings(
  ...
  for (const query of sqlQueries) {
    ...
    for (const column of query.columns) {
      ...
      const mysqlTable = schemaMap.tables.find(
        (t: any) => t.name.toLowerCase() === column.table.toLowerCase()
      );

      if (!mysqlTable) continue;

      // Trouver la colonne correspondante dans la table MySQL
      const mysqlColumn = mysqlTable.columns.find(
        (c: any) => c.name.toLowerCase() === column.column.toLowerCase()
      );

      if (!mysqlColumn) continue;

      // Trouver le modèle Prisma correspondant
      const prismaModel = prismaModels.find(
        (model) =>
      ...
      let prismaField = prismaModel.fields.find(
        (f: any) =>
      ...
      if (!prismaField) {
        ...
        const similarFields = prismaModel.fields
          .filter((f: any) => {
          ...
          .sort((a: any, b: any) => {
            return (
              calculateSimilarity(b.name, mysqlColumn.name) -
...
function generateRefactorTasks(
  ...
  for (const { query, mappings } of queryMappings) {
    ...
    const context = phpInfo.contexts.find(
      (ctx) => query.line >= ctx.startLine && query.line <= ctx.endLine
    );
 ...
 */
function generateRefactorSuggestion(mappings: FieldMapping[], context: string): string {
  const renamings = mappings
    .filter((m) => m.change === 'rename')
    .map((m) => `'${m.sqlColumn}' en '${m.prismaField}'`);

  const additions = mappings.filter((m) => m.change === 'add').map((m) => `'${m.sqlColumn}'`);
...
function updateBacklog(backlogPath: string, tasks: RefactorTask[]): void {
  ...
  for (const task of tasks) {
    const existingTaskIndex = backlog.tasks.findIndex(
      (t) => t.file === task.file && t.line === task.line
    );
 ...
 */
function updateSchemaMigrationDiff(diffPath: string, fieldMappings: FieldMapping[]): void {
...
if (require.main === module) {
  ...
  if (args.length < 4) {
    console.error(
      'Usage: npx ts-node php-sql-sync-mapper.ts <audit-file> <schema-map> <prisma-schema> <output-dir>'
    );
    process.exit(1);
  }

  mapPhpSqlSync(args[0], args[1], args[2], args[3]).catch((err) => {
