...
import { AgentResult, AuditSection, BaseAgent } from './core/base-agent';
...
export interface QAAnalyzerOptions {
  ...
  DoDoDoDoDoDoDotgithubToken?: string;
  DoDoDoDoDoDoDotgithubRepo?: string;
  DoDoDoDoDoDoDotgithubOwner?: string;
...
export class QAAnalyzer extends BaseAgent implements BusinessAgent {
  ...
  constructor(
    ...
    if (this.optionsDoDoDoDoDoDoDotgithubToken) {
      this.octokit = new Octokit({
        auth: this.optionsDoDoDoDoDoDoDotgithubToken,
      });
  ...
  public async analyze(): Promise<void> {
    try {
      ...
      this.addSection(
        ...
        `Analyse QA: ${path.basename(this.sourcePhpPath)} → ${path.basename(
          Object.values(this.generatedFiles)[0] || ''
        )}`,
      ...
      if (
        this.octokit &&
        this.optionsDoDoDoDoDoDoDotgithubOwner &&
        this.optionsDoDoDoDoDoDoDotgithubRepo
      ) {
      ...
      while ((match = sqlColumnRegex.exec(sourcePhp)) !== null) {
        ...
        for (const col of columns) {
          if (col !== '*') {
            ...
            const colName = col.includes(' AS ') ? col.split(' AS ')[0].trim() : col;

            // Enlever les préfixes de table si présents
            const finalName = colName.includes('.') ? colName.split('.')[1].trim() : colName.trim();
  ...
  private analyzeFields(phpFields: string[]): {
    ...
    for (const field of phpFields) {
      ...
      const hasValidation =
        allGeneratedContent.includes(`z.string().${field}`) ||
  ...
  private analyzeSEO(): QAIssue[] {
    ...
    if (this.generatedFiles.meta) {
      ...
      if (
        this.fileContent.includes('rel="canonical"') &&
        !this.generatedFiles.meta.includes('canonical')
      ) {
        issues.push({
          type: 'seo',
          severity: 'error',
  ...
  private analyzeTypes(): QAIssue[] {
    ...
    if (this.generatedFiles.loader) {
      ...
      if (
        this.generatedFiles.loader.includes('export async function loader(') &&
    ...
    if (this.generatedFiles.component) {
      ...
      if (
        !this.generatedFiles.component.includes('type Props') &&
        !this.generatedFiles.component.includes('interface Props')
      ) {
        issues.push({
          type: 'type',
          severity: 'warning',
      ...
      if (
        this.generatedFiles.component.includes('useLoaderData()') &&
        !this.generatedFiles.component.includes('useLoaderData<')
      ) {
        issues.push({
          type: 'type',
  ...
  private analyzeValidation(): QAIssue[] {
    ...
    if (this.generatedFiles.loader) {
      ...
      const hasZodImport =
        this.generatedFiles.loader.includes('import { z }') ||
        this.generatedFiles.loader.includes('import z from');
      ...
      if (!hasZodImport && !this.generatedFiles.loader.includes('zod')) {
        ...
        if (
          this.generatedFiles.loader.includes('request.url') &&
          !this.generatedFiles.loader.includes('z.string().url()')
        ) {
          issues.push({
            type: 'validation',
            severity: 'info',
        ...
        if (
          this.generatedFiles.loader.includes('formData') &&
          !this.generatedFiles.loader.includes('z.object({')
        ) {
          issues.push({
            type: 'validation',
            severity: 'warning',
  ...
  private analyzeBehavior(): QAIssue[] {
    ...
    if (this.generatedFiles.loader) {
      ...
      const phpHasErrorHandling =
        this.fileContent.includes('try') ||
      ...
      const loaderHasErrorHandling =
        this.generatedFiles.loader.includes('try') ||
      ...
      if (
        this.generatedFiles.loader.includes('return null') &&
        !this.generatedFiles.loader.includes('try') &&
        !this.generatedFiles.loader.includes('throw')
      ) {
        issues.push({
          type: 'behavior',
          severity: 'warning',
    ...
    if (
      this.fileContent.includes("header('Location:") ||
      this.fileContent.includes('header("Location:')
    ) {
      const hasRedirect =
        this.generatedFiles.loader &&
        (this.generatedFiles.loader.includes('redirect(') ||
          this.generatedFiles.loader.includes('return redirect'));
    ...
    if (this.fileContent.includes('$_SESSION')) {
      const hasSessionHandling =
        this.generatedFiles.loader &&
        (this.generatedFiles.loader.includes('session') ||
          this.generatedFiles.loader.includes('cookie'));
      ...
      if (missingFields.length <= 3) {
        missingFields.forEach((field) => {
          recommendations.push(
            `Ajouter le champ manquant '${field.name}' dans les fichiers générés`
          );
        });
      } else {
        recommendations.push(
          `Ajouter les ${missingFields.length} champs manquants dans les fichiers générés`
        );
      }
    }

    // Ajouter les recommandations des problèmes détectés
    const allIssues = [...seoIssues, ...typeIssues, ...validationIssues, ...behaviorIssues].filter(
  ...
  private async saveToSupabase(): Promise<void> {
    ...
    try {
      const { data, error } = await this.supabase.from('audit.qa_results').insert({
  ...
  private async commentOnPR(): Promise<void> {
    ...
    try {
      ...
      const { data: pullRequests } = await this.octokit.pulls.list({
        owner: this.optionsDoDoDoDoDoDoDotgithubOwner,
        repo: this.optionsDoDoDoDoDoDoDotgithubRepo,
        state: 'open',
        head: `${this.optionsDoDoDoDoDoDoDotgithubOwner}:${currentBranch}`,
      });
      ...
      if (this.qaResult.recommendations.length > 0) {
        ...
        if (this.qaResult.recommendations.length > 5) {
          comment += `- _${
            this.qaResult.recommendations.length - 5
          } recommandations supplémentaires..._\n`;
        }
        comment += '\n';
      }
  ...
  private async triggerRetry(): Promise<void> {
    ...
    try {
      this.logger.log(
        `🔄 Déclenchement d'un nouveau job MCP pour ${path.basename(this.qaResult.sourceFile)}`
      );
...
if (require.main === module) {
  ...
  if (args.length < 2) {
    console.error(
      '❌ Usage: ts-node QaAnalyzer.ts <source-php-path> <generated-files-json> [options-json]'
    );
    console.error(
      'Exemple: ts-node QaAnalyzer.ts ./src/fiche.php \'{"component":"./app/routes/fiche.tsx","loader":"./app/routes/fiche.loader.ts","meta":"./app/routes/FicheDotmeta.ts"}\''
    );
    process.exit(1);
  }
  ...
  try {
    ...
    runQAAnalyzer(sourcePhpPath, generatedFiles, options)
      .then((result) => {
        ...
        console.log(
          `📊 Champs: ${result.presentFields.length} présents, ${result.missingFields.length} manquants`
        );
        console.log(
          `🚨 Problèmes: ${
            result.seoIssues.length +
            result.typeIssues.length +
            result.validationIssues.length +
            result.behaviorIssues.length
          }`
        );
