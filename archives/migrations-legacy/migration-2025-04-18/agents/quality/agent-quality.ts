...
export class QualityAgent {
  ...
  private analyzeTechnicalDebt(): { content: string; severity: 'info' | 'warning' | 'critical' } {
    ...
    const untypedVariables = variableAssignments.filter(
      (a) =>
        !a.includes('(int)') &&
        !a.includes('(string)') &&
        !a.includes('(bool)' || !a.includes('(float)'))
    );
    ...
    if (longFunctions.length > 0) {
      issues.push(
        `Fonctions trop longues: ${longFunctions.length} fonction(s) de plus de 50 lignes`
      );
      severity = 'critical';
    }
    ...
    const magicNumbers = (content.match(/\W\d+\W/g) || [])
      ...
      .filter((n) => !n.includes('LIMIT 1')).length; // Ignorer les LIMIT 1 en SQL
    ...
    if (heavyIndentedLines.length > 20) {
      issues.push(
        `Indentation excessive: ${heavyIndentedLines.length} lignes avec indentation profonde`
      );
      if (severity !== 'critical') severity = 'warning';
    }
    ...
    if (debtScore > 60) {
      debt +=
        "⚠️ **Dette technique élevée**: Ce code nécessite une refactorisation substantielle avant d'être migré.";
      severity = 'critical';
    } else if (debtScore > 30) {
      debt +=
        '⚠️ **Dette technique modérée**: Des améliorations significatives sont recommandées pendant la migration.';
      if (severity !== 'critical') severity = 'warning';
    } else {
      debt +=
        '✅ **Dette technique faible**: Le code est dans un état raisonnable pour la migration.';
    }
  ...
  private analyzeSecurityVulnerabilities(): {
    ...
    if (
      (content.includes('mysql_query') || content.includes('mysqli_query')) &&
    ...
    // Détecter les XSS
    if (
      (content.includes('echo $') || content.includes('print $')) &&
      (content.includes('$_GET') || content.includes('$_POST'))
    ) {
    ...
    if (
      (content.includes('include') || content.includes('require')) &&
      (content.includes('$_GET') || content.includes('$_POST'))
    ) {
      vulnerabilities.push({
    ...
    if (vulnerabilities.length > 0) {
      ...
      if (severity === 'critical') {
        security +=
          '**RECOMMANDATION URGENTE**: Ces vulnérabilités doivent être corrigées avant la mise en production.';
      } else if (severity === 'warning') {
        security +=
          '**RECOMMANDATION**: Ces problèmes devraient être adressés pendant la phase de migration.';
      }
    } else {
  ...
  public async analyze(): Promise<AuditSection[]> {
    try {
      ...
      const { content: complexityContent, severity: complexitySeverity } =
      ...
      const { content: securityContent, severity: securitySeverity } =
  ...
  public async saveSections(outputPath?: string): Promise<void> {
    try {
      ...
      const outputFilePath =
        outputPath || path.join(path.dirname(this.filePath), `${fileName}.audit.md`);

      let content = '';
      this.sections.forEach((section) => {
        const severityIcon =
          section.severity === 'critical' ? '🔴' : section.severity === 'warning' ? '🟠' : '🟢';
