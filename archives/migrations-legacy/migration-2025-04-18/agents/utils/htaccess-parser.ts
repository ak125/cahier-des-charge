...
import { RouteConfig, ServerConfig } from '../migration/caddyfile-generator';
...
export class HtaccessParser {
      ...
      if (inRewriteEngine) {
        if (line.startsWith('RewriteRule') || line.startsWith('RewriteCond')) {
          ...
          if (parts.length >= 3) {
            ...
            config.directives[`RewriteMap_${mapName}`] = {
              type: mapType,
    ...
    if (rule.conditions) {
      ...
      for (const condition of rule.conditions) {
        if (condition.test === '%{HTTP_HOST}') {
          ...
          matcherExpressions.push(
            `header ${header} ${this.convertPatternToCaddyFormat(condition.pattern)}`
          );
        }
      }
  ...
  private async processDirectives(
    ...
    if (directives['Header']) {
      const headers = Array.isArray(directives['Header'])
        ? directives['Header']
        : [directives['Header']];
    ...
    if (directives['ExpiresActive'] === 'On' && directives['ExpiresByType']) {
      for (const [mimeType, expireRule] of Object.entries(directives['ExpiresByType'])) {
        ...
        const expireMatch = expireRule.match(
          /^"access plus (\d+) (seconds|minutes|hours|days|months|years)"$/
        );
  ...
  header Content-Type ${mimeType}
    ...
    if (directives['Redirect']) {
      const redirects = Array.isArray(directives['Redirect'])
        ? directives['Redirect']
        : [directives['Redirect']];
