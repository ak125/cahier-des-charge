...
const CONFIG = {
  ...
  DoDotmcpDir: path.join(process.cwd(), 'appsDoDotmcp-server'),
  outputWorkflowPath: path.join(process.cwd(), 'DoDoDoDoDoDotgithub/workflows/ci-migration.yml'),
  ...
  workspacePackages: [
    ...
    'appsDoDotmcp-server/package.json',
  ...
  async run(options?: {
    ...
    try {
      ...
      const generateWorkflow =
        options?.generateWorkflow !== undefined ? options.generateWorkflow : true;

      const validateCurrentSetup =
        options?.validateCurrentSetup !== undefined ? options.validateCurrentSetup : true;

      const installGitHubApps =
        options?.installGitHubApps !== undefined ? options.installGitHubApps : false;
      ...
      if (detectedTests.length > 0) {
        ...
        detectedTests.forEach((test) => {
          logs.push(
            `  - ${test.name}: \`${test.command}\` ${test.required ? '(Requis)' : '(Optionnel)'}`
          );
        });
      } else {
      ...
      if (generateWorkflow) {
        ...
        // Créer le répertoire DoDoDoDoDoDotgithub/workflows s'il n'existe pas
      ...
      if (validateCurrentSetup) {
        ...
        const missingRequired = validationResults.filter(
          (result) => !result.present && result.test.required
        );
        ...
        if (missingRequired.length > 0) {
          ...
          missingRequired.forEach((result) => {
            logs.push(
              `  - ${result.test.name}: \`${result.test.command}\` est requis mais non configuré`
            );
          });
          report.status = 'error';
        } else {
      ...
      if (installGitHubApps) {
        ...
        const appsToInstall = [
          { name: 'Codecov', url: 'https:/DoDoDoDoDoDotgithub.com/apps/codecov' },
          { name: 'Dependabot', url: 'https:/DoDoDoDoDoDotgithub.com/apps/dependabot' },
          { name: 'SonarCloud', url: 'https:/DoDoDoDoDoDotgithub.com/apps/sonarcloud' },
        ];
        ...
        logs.push(
          "⚠️ L'installation automatique des GitHub Apps n'est pas possible. Veuillez suivre les liens manuellement."
        );
      }
...
function detectCITests(packageScripts: PackageScripts): CITest[] {
  ...
  if (hasScript(packageScripts, 'lint')) {
    ...
    const lintCommand = hasScript(packageScripts, 'lint:check')
      ? 'npm run lint:check'
      : 'npm run eslint';
    tests.push({
      name: 'Lint du code',
  ...
  if (hasWorkspaces(packageScripts)) {
    ...
    if (
      hasScript(packageScripts, 'build:frontend') ||
      hasPackageScript(packageScripts, 'frontend', 'build')
    ) {
      buildCommands.push(
        hasScript(packageScripts, 'build:frontend')
    ...
    if (
      hasScript(packageScripts, 'build:backend') ||
      hasPackageScript(packageScripts, 'backend', 'build')
    ) {
      buildCommands.push(
        hasScript(packageScripts, 'build:backend')
  ...
  if (hasScript(packageScripts, 'test')) {
    ...
    if (
      hasScript(packageScripts, 'test:backend') ||
      hasPackageScript(packageScripts, 'backend', 'test')
    ) {
      testCommands.push(
        hasScript(packageScripts, 'test:backend') ? 'npm run test:backend' : 'npm test -w backend'
      );
    }

    if (
      hasScript(packageScripts, 'test:frontend') ||
      hasPackageScript(packageScripts, 'frontend', 'test')
    ) {
      testCommands.push(
        hasScript(packageScripts, 'test:frontend')
  ...
  if (hasScript(packageScripts, 'test:e2e')) {
    ...
    hasPackageScript(packageScripts, 'frontend', 'test:e2e') ||
    hasPackageScript(packageScripts, 'backend', 'test:e2e')
  ) {
    const e2eCommands = [];
  ...
  if (fs.existsSync(path.join(CONFIG.rootDir, 'prisma/schema.prisma'))) {
    tests.push({
      ...
      command:
        'npx prisma validate && npx prisma migrate diff --from-schema-datamodel prisma/schema.prisma --to-migrations-directory prisma/migrations --exit-code',
  ...
  if (hasScript(packageScripts, 'security') || hasScript(packageScripts, 'audit')) {
    const securityCommand = hasScript(packageScripts, 'security')
      ? 'npm run security'
      : 'npm run audit';
    tests.push({
...
${tests
  ...
  .join('\n\n')}
    ...
    - name: Notify on Success
      if: success()
      uses: actionsDoDoDoDoDoDotgithub-script@v6
      with:
       DoDoDoDoDoDotgithub-token: \${{ secrets.GITHUB_TOKEN }}
        script: |
          const { issue, repo } = context;
          if (context.payload.pull_request) {
           DoDoDoDoDoDotgithub.rest.issues.createComment({
              issue_number: issue.number,
              owner: repo.owner,
              repo: repo.repo,
              body: '✅ CI Pipeline a réussi! Tous les tests et vérifications sont passés.'
            });
           DoDoDoDoDoDotgithub.rest.issues.addLabels({
              issue_number: issue.number,
              owner: repo.owner,
              repo: repo.repo,
              labels: ['ci:passed']
...
async function generateCIReport(report: CIReport, logs: string[]): Promise<void> {
  ...
  reportContent += `| ${
    report.status === 'success'
      ...
      : report.status === 'warning'
        ? '⚠️ Avertissement'
        : '❌ Erreur'
  } | ${report.detectedTests.length} | ${report.generatedFiles.length} |\n\n`;
  ...
  for (const test of report.detectedTests) {
    reportContent += `| ${test.name} | \`${test.command}\` | ${test.description} | ${
      test.required ? '✅' : '⚠️'
    } |\n`;
  }
...
if (require.main === module) {
  const options = {
    ...
    installGitHubApps: process.argv.includes('--installDoDoDoDoDoDotgithub-apps'),
  };
