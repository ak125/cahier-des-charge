...
export class SEOMCPController {
  ...
  async runPipeline(): Promise<SEOProcessingResult> {
    ...
    try {
      ...
      console.log(
        `✅ Pipeline SEO-MCP terminé ${result.success ? 'avec succès' : 'avec des erreurs'}`
      );
  ...
  private async processRedirects(parentTraceId: string): Promise<{
    ...
    try {
      ...
      result.errors.push(
        `Erreur lors du traitement des redirections: ${
  ...
  private async generateMetadata(parentTraceId: string): Promise<{
    ...
    try {
      ...
      for (const routeFile of routeFiles) {
        try {
          ...
          if (genResult.success) {
            ...
            result.failed++;
            result.errors.push(
              `Échec pour la route ${routePath}: ${genResult.messages.join(', ')}`
            );
          }
        } catch (routeError) {
          result.failed++;
          result.errors.push(
            `Erreur lors du traitement de la route ${routeFile}: ${
      ...
      result.errors.push(
        `Erreur lors de la génération des métadonnées: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
    ...
    try {
      ...
      for (const routeFile of routeFiles) {
        try {
          ...
          if (valResult.issues.length > 0) {
            ...
            if (valResult.issues.length > valResult.fixed) {
              ...
              for (const issue of unfixed) {
                result.errors.push(
                  `Canonical non corrigé pour ${routePath}: ${issue.issueType} (${issue.expectedCanonical})`
                );
              }
            }
          }
          ...
          result.errors.push(
            `Erreur lors de la validation du canonical pour ${routeFile}: ${
              routeError instanceof Error ? routeError.message : String(routeError)
            }`
          );
        }
      ...
      result.errors.push(
        `Erreur lors de la validation des canonicals: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
  ...
  private async runSEOCheck(parentTraceId: string): Promise<{
    ...
    try {
      ...
      if (await fs.pathExists(seoReportPath)) {
        ...
        if (seoReport.issues && Array.isArray(seoReport.issues)) {
          ...
          for (let i = 0; i < Math.min(10, seoReport.issues.length); i++) {
            ...
            result.errors.push(
              `Problème SEO: ${issue.message} (score: ${issue.score}, route: ${issue.route})`
            );
          }
      ...
      result.errors.push(
        `Erreur lors de la vérification SEO: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
...
- **Score SEO moyen**: ${data.averageSeoScore.toFixed(2)}/100 (minimum requis: ${
