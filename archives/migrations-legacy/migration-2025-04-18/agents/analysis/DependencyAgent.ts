...
export class DependencyAgent extends BaseAgent {
  ...
  public async analyze(): Promise<void> {
    ...
    this.addSection('included-files', 'Fichiers inclus', includedFiles, 'dependencies');

    this.addSection('cross-calls', 'Appels croisés', crossCalls, 'dependencies');

    this.addSection('session-state', 'Session & state', sessionUsage, 'dependencies');
  }
  ...
  private identifyIncludedFiles(): string {
    ...
    const configFiles = includes.filter(
      (file) => file.includes('config') || file.includes('conf') || file.includes('settings')
    );

    const templateFiles = includes.filter(
      (file) => file.includes('tpl') || file.includes('template') || file.includes('view')
    );

    const coreFiles = includes.filter(
      (file) => file.includes('core') || file.includes('common') || file.includes('functions')
    );

    const otherFiles = includes.filter(
      (file) =>
  ...
  private extractExternalFunctionCalls(content: string): string[] {
    ...
    return [...new Set(calledFunctions)].filter(
      (func) => !phpNativeFunctions.includes(func) && !internalFunctions.includes(func)
    );
  }
