...
program
  ...
  .option(
    '--sample-size <size>',
    "Nombre d'enregistrements à vérifier par table (0 = tous)",
    '1000'
  )
...
async function main() {
  ...
  try {
    ...
    if (options.tables) {
      ...
      console.log(
        `📋 Tables spécifiées dans le fichier de configuration: ${tablesToVerify.join(', ')}`
      );
    } else {
    ...
    for (const tableName of tablesToVerify) {
      ...
      try {
        ...
        const [mysqlCountResult] = await mysqlConnection.query(
        ...
        console.log(
          `📊 MySQL: ${mysqlCount} enregistrements, PostgreSQL: ${postgresCount} enregistrements`
        );

        if (!tableResult.countMatch) {
          console.log(
            `⚠️ Différence dans le nombre d'enregistrements: ${Math.abs(mysqlCount - postgresCount)}`
          );
          ...
          report.problems.push({
            ...
            details: {
              ...
              percentageDifference:
                (Math.abs(mysqlCount - postgresCount) / Math.max(mysqlCount, 1)) * 100,
            },
          });
        }
        ...
        const commonColumns = mysqlColumnNames.filter(
          (col) => pgColumnNames.includes(col) && !skipColumns.includes(col)
        );
        ...
        console.log(
          `🔍 Vérification des données (échantillon de ${tableResult.sampleSize} enregistrements)...`
        );
        ...
        const sampleQuery =
          sampleSize === 0
            ...
            : `SELECT ${commonColumns
                .map((col) => `\`${col}\``)
                .join(', ')} FROM \`${tableName}\` LIMIT ${sampleSize}`;
        ...
        for (const mysqlRecord of mysqlRecords as any[]) {
          ...
          if (options.verbose && checkedCount % 100 === 0) {
            console.log(
              `  Progression: ${checkedCount}/${tableResult.sampleSize} enregistrements vérifiés`
            );
          }
          ...
          const pgRecordResult = await pgPool.query(
            `SELECT ${commonColumns
              .map((col) => `"${col}"`)
              .join(', ')} FROM "${tableName}" WHERE "${keyColumn}" = $1 LIMIT 1`,
            [keyValue]
          );
        ...
        if (tableResult.mismatchedRecords > 0) {
          ...
          console.log(
            `⚠️ ${tableResult.mismatchedRecords} enregistrements (${mismatchPercentage.toFixed(
              2
          ...
          if (mismatchPercentage > 5) {
            ...
            report.problems.push({
              ...
              message: `${
                tableResult.mismatchedRecords
              } enregistrements (${mismatchPercentage.toFixed(2)}%) présentent des différences`,
              details: {
            ...
            report.problems.push({
              severity: 'warning',
              tableName,
              message: `Différences mineures: ${
                tableResult.mismatchedRecords
              } enregistrements (${mismatchPercentage.toFixed(2)}%)`,
              details: {
                mismatchedRecords: tableResult.mismatchedRecords,
                checkedRecords: checkedCount,
                mismatchPercentage,
      ...
      console.log(
        `✅ Vérification de la table ${tableName} terminée en ${
          tableResult.verificationTime / 1000
        } secondes`
      );
    }
...
function compareValues(value1: any, value2: any): boolean {
  ...
  if (typeof value1 !== typeof value2) {
    ...
    if (
      ...
      return (
        str1.replace('T', ' ').replace(/\.\d+Z$/, '') ===
        str2.replace('T', ' ').replace(/\.\d+Z$/, '')
      );
    }
  }
