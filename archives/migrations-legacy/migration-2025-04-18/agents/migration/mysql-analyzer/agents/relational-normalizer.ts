...
export class RelationalNormalizer {
  ...
  private detectPotentialForeignKey(
    ...
    if (this.isCompatibleWithPrimaryKey(column)) {
      ...
      for (const candidate of candidates) {
        if (
          schema.tables[candidate] &&
          schema.tables[candidate].primaryKey &&
          this.areTypesCompatible(
            column,
            schema.tables[candidate].columns[schema.tables[candidate].primaryKey[0]]
          )
        ) {
  ...
  private isCompatibleWithPrimaryKey(column: ColumnInfo): boolean {
    ...
    return pkCompatibleTypes.some((type) => column.type.toUpperCase().includes(type.toUpperCase()));
  }
   ...
   */
  private async analyzePhpJoins(schema: MySQLSchema, issues: RelationIssue[]): Promise<void> {
  ...
  private convertJoinsToRelations(
    ...
    detectedJoins.forEach((targets, source) => {
      ...
      targets.forEach((target) => {
        ...
        if (
          !schema.tables[sourceTable].columns[sourceColumn] ||
        ...
        const isAlreadyExplicit = schema.tables[sourceTable].foreignKeys.some(
          (fk) =>
        ...
        const isAlreadyImplicit = schema.tables[sourceTable].relations?.some(
          (rel) =>
   ...
   */
  private detect3NFViolations(schema: MySQLSchema, suggestions: NormalizationSuggestion[]): void {
    Object.entries(schema.tables).forEach(([tableName, table]) => {
      ...
      if (transitiveGroups.length > 0) {
        ...
        transitiveGroups.forEach((group) => {
          ...
          const suggestion: NormalizationSuggestion = {
            type: '3NF_VIOLATION',
            tableName: tableName,
            description: `La table contient des dépendances transitives: ${group.join(
              ', '
            )} dépendent de colonnes non-clés`,
  ...
  private findPotentialDependentGroups(table: TableInfo): string[][] {
    ...
    const geoGroup = columns.filter((col) => /lat|lng|latitude|loDoDoDoDotgitude|coord/i.test(col));
  ...
  private groupByPrefix(columns: string[]): string[][] {
    ...
    columns.forEach((column) => {
      ...
      if (match && match[1].length > 2) {
        // Ignorer les préfixes trop courts
  ...
  private identifyEntities(tableName: string, table: TableInfo): string[] {
    ...
    return validParts.filter((part) => {
      ...
      return Object.keys(table.columns).some(
        (col) => col.includes(part) || col === 'id_' + part || col === part + '_id'
      );
    });
  }
  ...
  private suggestEntityDecomposition(
    ...
    entities.forEach((entity) => {
      entityColumns[entity] = [
        ...
        ...Object.keys(table.columns).filter(
          (col) => col.includes(entity) || col === 'id_' + entity || col === entity + '_id'
        ),
      ];
    });
    ...
    Object.values(entityColumns).forEach((cols) => cols.forEach((col) => assignedColumns.add(col)));

    const unassignedColumns = Object.keys(table.columns).filter((col) => !assignedColumns.has(col));
    ...
    for (let i = 1; i < entities.length; i++) {
      ...
      sqlStatements += `\nALTER TABLE \`${entities[i]}\` ADD CONSTRAINT \`fk_${entities[i]}_${
        entities[0]
      }\` FOREIGN KEY (${primaryKey.map((col) => '`' + col + '`').join(', ')}) REFERENCES \`${
        entities[0]
  ...
  private generateDecompositionSQL(
    ...
    primaryKey.forEach((pkCol) => {
      ...
      dependentTableSQL += `  \`${pkCol}\` ${
        columnInfo.originalType || columnInfo.type
      } NOT NULL,\n`;
    });
    ...
    dependentTableSQL += `,  PRIMARY KEY (${primaryKey
      .map((col) => '`' + col + '`')
      .join(', ')}),\n`;
    dependentTableSQL += `  CONSTRAINT \`fk_${dependentTableName}_${tableName}\` FOREIGN KEY (${primaryKey
      .map((col) => '`' + col + '`')
      .join(', ')}) REFERENCES \`${tableName}_new\`(${primaryKey
      .map((col) => '`' + col + '`')
    ...
    migrationSQL += `INSERT INTO \`${tableName}_new\` SELECT ${mainColumns
      .map((col) => '`' + col + '`')
      .join(', ')} FROM \`${tableName}\`;\n\n`;
    migrationSQL += `INSERT INTO \`${dependentTableName}\` SELECT ${[
      ...primaryKey,
      ...dependentGroup,
    ]
      .map((col) => '`' + col + '`')
   ...
   */
  private detectRedundantTables(schema: MySQLSchema, suggestions: NormalizationSuggestion[]): void {
  ...
  private findTemporalTableGroups(schema: MySQLSchema): Array<{
    ...
    baseTables.forEach((temporalTables, baseTable) => {
      ...
      if (temporalTables.length > 1) {
        ...
        if (similarity > 0.7) {
          const recommendedAction = this.generateTemporalTablesRecommendation(
            baseTable,
            temporalTables
          );
  ...
  private calculateTableSimilarity(table1: TableInfo, table2: TableInfo): number {
    ...
    const similarity =
      commonColumns.length / (columns1.length + columns2.length - commonColumns.length);
  ...
  private generateTemporalTablesRecommendation(baseTable: string, tables: string[]): string {
    ...
    if (tables.some((t) => t.includes('log') || t.includes('hist'))) {
      return (
        `Considérer la création d'une table d'historique unique pour '${baseTable}' avec un champ 'type' ` +
    ...
    if (tables.some((t) => t.includes('temp') || t.includes('tmp'))) {
      return (
        `Remplacer les tables temporaires ${tablesStr} par une table unique avec un champ 'status' ` +
    ...
    // Cas général
    return (
      `Considérer la fusion des tables ${tablesStr} en une seule structure avec un champ discriminant ` +
   ...
   */
  private generateTemporalTableSQL(
    schema: MySQLSchema,
    baseTable: string,
    tables: string[]
  ): string {
    ...
    sql += `  PRIMARY KEY (${[...primaryKey, 'record_type']
      .map((col) => '`' + col + '`')
      .join(', ')})\n`;
    sql += `);\n\n`;
    ...
    for (let i = 0; i < tableNames.length; i++) {
      ...
      if (similarTables.length > 1) {
        ...
        const recommendedAction =
          `Considérer la fusion des tables ${similarTables.join(', ')} ` +
  ...
  private generateStructuralMergeSQL(schema: MySQLSchema, tableNames: string[]): string {
    ...
    const mergedTableName = tableNames.length > 0 ? tableNames[0] + '_merged' : 'merged_table';
    ...
    sql += `  PRIMARY KEY (${[...primaryKey, 'entity_type']
      .map((col) => '`' + col + '`')
      .join(', ')})\n`;
    sql += `);\n\n`;
