...
program
  .version('1.3.0')
  .description(
    'Convertit un schéma MySQL en schéma PostgreSQL compatible avec les meilleures pratiques'
  )
  .requiredOption(
    '--input <path>',
    'Chemin vers le fichier SQL MySQL ou URI de connexion à la base'
  )
  ...
  .option(
    '--direct-db',
    'Connecte directement aux bases de données plutôt que de lire/écrire des fichiers'
  )
  .option('--mysql-uri <uri>', 'URI de connexion MySQL (mysql://user:pass@host:port/database)')
  .option(
    '--pg-uri <uri>',
    'URI de connexion PostgreSQL (postgresql://user:pass@host:port/database)'
  )
  .option('--schema-only', 'Migrer seulement le schéma, pas les données')
  .option(
    '--batch-size <size>',
    'Nombre de lignes à traiter par lot lors de la migration de données',
    '10000'
  )
  .option('--verbose', 'Afficher des informations détaillées pendant la conversion')
  .option(
    '--analyze',
    'Analyser le schéma et les données avant la migration et produire un rapport'
  )
...
const DEFAULT_MIGRATION_RULES: MigrationRule[] = [
  ...
  // Auto-increment
  {
    ...
    mysqlPattern:
      '(INT|BIGINT)(?:\\(\\d+\\))?(?:\\s+UNSIGNED)?\\s+(?:NOT\\s+NULL\\s+)?(?:PRIMARY\\s+KEY\\s+)?AUTO_INCREMENT',
...
async function main() {
  console.log(
    chalk.blue(`🚀 Démarrage de la conversion MySQL → PostgreSQL avec règles personnalisées`)
  );
  ...
  try {
      ...
      if (fs.existsSync(options.rules)) {
        try {
          ...
          if (Array.isArray(customRules)) {
            console.log(
              chalk.blue(
                `📝 Chargement de ${customRules.length} règles personnalisées depuis ${options.rules}`
              )
            );
            config.rules = [...customRules, ...config.rules];
          } else {
            console.warn(
              chalk.yellow(`⚠️ Format invalide dans le fichier de règles: ${options.rules}`)
            );
          }
        } catch (error) {
          console.error(
            chalk.red(`❌ Erreur lors du chargement des règles personnalisées: ${error.message}`)
          );
        }
      } else {
        console.warn(
          chalk.yellow(`⚠️ Fichier de règles personnalisées non trouvé: ${options.rules}`)
        );
      }
    }
    ...
    const statsFile = `${path.dirname(options.output)}/migration-stats-${new Date()
      .toISOString()
      .replace(/:/g, '-')}.json`;
    ...
    const statsFile = `${path.dirname(options.output)}/migration-stats-error-${new Date()
      .toISOString()
      .replace(/:/g, '-')}.json`;
    fs.writeFileSync(statsFile, JSON.stringify(stats, null, 2));
...
function loadConfiguration(configPath?: string): MigrationConfig {
  ...
  if (configPath && fs.existsSync(configPath)) {
    try {
      ...
      console.error(
        chalk.red(`❌ Erreur lors du chargement de la configuration: ${error.message}`)
      );
 ...
 */
async function convertMySQLDump(
  mysqlDump: string,
  ...
  for (const table of tables) {
    ...
    try {
      ...
      if (options.verbose) {
        console.error(
          chalk.red(`❌ Erreur lors de la conversion de la table ${table.name}: ${error.message}`)
        );
      }
    }
  }
...
function convertTable(
  table: { name: string; content: string },
  config: MigrationConfig,
  ...
  for (const rule of config.rules) {
    ...
    if (regex.test(columnDef)) {
          ...
          if (enumTypes[typeName]) {
            ...
            if (enumMatch) {
              columnDef = columnDef.replace(
                regex,
                `TEXT CHECK ("${columnName}" IN (${enumMatch[1]}))`
              );
            } else {
      ...
      if (rule.requiresManualReview) {
        stats.warnings.push(
          `La colonne ${tableName}.${columnName} utilise le type ${rule.name} qui nécessite une révision manuelle`
        );
      }
...
function convertConstraint(
  ...
  if (constraintLine.trim().startsWith('PRIMARY KEY')) {
    ...
    const match = constraintLine.match(
  ...
  try {
    ...
    for (const tableRow of tables) {
      ...
      try {
        ...
        console.error(
          chalk.red(`❌ Erreur lors de la migration de la table ${tableName}: ${error.message}`)
        );
      }
    }
 ...
 */
async function convertCreateTableStatement(
...
async function migrateTableData(
  ...
  while (offset < totalRows) {
    const [rows] = await mysqlConnection.query(
      `SELECT * FROM \`${tableName}\` LIMIT ${offset}, ${batchSize}`
    );
...
function displayMigrationSummary(stats: MigrationStats): void {
  ...
  console.log(
    `Tables: ${stats.tablesMigrated}/${stats.tablesProcessed} migrées (${
      (stats.summary.successRate || 0) * 100
    }%)`
  );
  ...
  if (stats.warnings.length > 0) {
    ...
    for (const warning of stats.warnings.slice(0, 10)) {
      // Limiter l'affichage aux 10 premiers avertissements
      console.log(chalk.yellow(`  - ${warning}`));
    }
    if (stats.warnings.length > 10) {
      console.log(
        chalk.yellow(`  ... et ${stats.warnings.length - 10} avertissements supplémentaires`)
      );
    }
  }
  ...
  if (stats.warnings.length > 0 || stats.errors.length > 0) {
    console.log(
      `  - Examinez les avertissements et erreurs pour identifier les problèmes potentiels`
    );
  }
