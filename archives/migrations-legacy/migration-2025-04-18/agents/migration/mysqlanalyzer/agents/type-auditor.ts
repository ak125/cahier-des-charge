...
export class TypeAuditor {
  ...
  private typeMapping: Record<
    string,
    { postgresType: string; prismaType: string; needsLength?: boolean }
  > = {
      ...
      Object.entries(table.columns).forEach(([columnName, column]) => {
        ...
        if (typeInfo.adjustedLength !== undefined && typeInfo.adjustedLength !== column.length) {
          fieldTypingIssues.push({
            ...
            suggestion: `Réduire la longueur de ${column.length || ''} à ${
              typeInfo.adjustedLength
            }`,
