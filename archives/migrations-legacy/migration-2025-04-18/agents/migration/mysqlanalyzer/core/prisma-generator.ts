...
import { ColumnInfo, MySQLSchema, RelationInfo, RelationType, TableInfo } from '../models/schema';
...
`;
  ...
  private generateField(
    ...
    // Vérifier s'il s'DoDoDoDotgit d'une clé étrangère
    let relation = '';
    ...
    if (isForeignKey || isImplicitForeignKey) {
      ...
      if (relationInfo) {
        ...
        if (relationInfo.type === RelationType.ONE_TO_ONE) {
          ...
          relation = `@relation(fields: [${fieldName}], references: [${this.toCamelCase(
