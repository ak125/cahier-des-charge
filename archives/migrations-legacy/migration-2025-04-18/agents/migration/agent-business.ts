...
export class BusinessAgent {
  ...
  private analyzeBusinessRole(): string {
    ...
    if (content.includes('panier') || content.includes('cart') || content.includes('basket')) {
      ...
      content.includes('authentification')
    ) {
      ...
      if (content.match(/SELECT.*FROM\s+produits/i) || content.match(/SELECT.*FROM\s+products/i)) {
        ...
        content.match(/SELECT.*FROM\s+utilisateurs/i) ||
        content.match(/SELECT.*FROM\s+users/i)
      ) {
        role = 'Récupération de données utilisateurs.';
      } else if (
        content.match(/SELECT.*FROM\s+commandes/i) ||
        content.match(/SELECT.*FROM\s+orders/i)
      ) {
  ...
  private analyzeFunctionalArea(): string {
    ...
    if (
      ...
      content.match(/SELECT.*FROM.*LIMIT/i)
    ) {
      area += ' - Affichage liste';
    } else if (
      content.includes('detail') ||
      content.includes('fiche') ||
      content.match(/SELECT.*FROM.*WHERE.*=/i)
    ) {
