...
class MigrationPlanGenerator {
  ...
  private extractDataFromAudit(auditContent: string): void {
    // Extract role
    const roleMatch = auditContent.match(
      /## 1️⃣ Rôle métier principal\s*\n\s*(.*?)(?:\n\s*\n|\n\s*##)/s
    );
    if (roleMatch) {
    ...
    const structureScoreMatch = auditContent.match(
      /Qualité structurelle estimée\s*:\s*\*\*([0-9.]+)\s*\/\s*3\*\*/
    );
    ...
    const securityScoreMatch = auditContent.match(
      /Score de sécurité IA \(0 à 10\)\s*:\s*([0-9.]+)/
    );
  ...
  private calculateImpactScore(): number {
    ...
    if (this.impactGraph) {
      ...
      const usedByCount =
        this.impactGraph && this.impactGraph.usedBy
          ? this.impactGraph && this.impactGraph.usedBy.length
          : 0;
  ...
  private calculateMigrationScore(): number {
    ...
    let couplageScore = this.impactGraph
      ? this.impactGraph && this.impactGraph.usedBy.length / 3
      : 0;
    couplageScore = Math.min(couplageScore, 5);

    const complexiteMetier = this.audit.role
      ? this.audit.role.includes('critique') || this.audit.role.includes('important')
        ? 4
        : this.audit.role.includes('secondaire')
          ? 2
          : 3
      : 3;

    const qualiteTechnique =
      this.audit.structure && this.audit.structure.score !== undefined
        ? Math.max(0, 3 - this.audit.structure.score) * 1.67
        : 2;

    let dependancesScore = this.impactGraph
      ? this.impactGraph.uses
        ? this.impactGraph.uses.length / 2
        : 0
      : 0;
  ...
  private needsRefactoringFirst(): boolean {
    ...
    if (
      ...
      this.audit.structure.score < 1.5
    ) {
      return true;
    }
    ...
    if (
      this.audit.data &&
      this.audit.data.sql_score !== undefined &&
      this.audit.data.sql_score < 1.5
    ) {
      return true;
    }
    ...
    if (
      this.impactGraph &&
      this.impactGraph.impact_level === 'élevé' &&
      this.audit.structure &&
      this.audit.structure.score !== undefined &&
  ...
  private identifyFrontendTargets(): string[] {
    ...
    if (
      this.audit.data &&
      this.audit.data.data_sources &&
      this.audit.data.data_sources.length > 0
    ) {
  ...
  private mapTaskType(type: string): 'refactor' | 'create' | 'migrate' | 'test' {
    ...
    if (type.includes('refactor') || type.includes('extract') || type.includes('simplify'))
      return 'refactor';
    return 'migrate';
  }
  ...
  private hasQualityIssues(): boolean {
    return (
      (this.audit.structure !== undefined &&
        this.audit.structure.score !== undefined &&
        this.audit.structure.score < 2) ||
  ...
  public generateMigrationPlanMd(): string {
    ...
    markdown += `- **Refactoring préalable nécessaire**: ${
      plan.needsRefactoringFirst ? 'Oui' : 'Non'
    }\n\n`;
    ...
    if (this.impactGraph) {
      ...
      markdown += `- **Nombre de fichiers appelants**: ${
        this.impactGraph && this.impactGraph.usedBy.length
      }\n`;
      ...
      if (this.impactGraph.uses && this.impactGraph && this.impactGraph.usedBy) {
        const totalRelations =
          this.impactGraph.uses.length + this.impactGraph && this.impactGraph.usedBy.length;
        const centrality =
          totalRelations > 0 ? Math.min(totalRelations / 15, 1).toFixed(2) : '0.00';
    ...
    const qualiteTechnique =
      this.audit.structure && this.audit.structure.score !== undefined
        ? Math.round(Math.max(0, 3 - this.audit.structure.score) * 1.67)
        : 2;

    const dependancesScore = this.impactGraph
      ? Math.min(Math.round(this.impactGraph.uses ? this.impactGraph.uses.length / 2 : 0), 5)
      : 0;
  ...
  private determineModuleType(): string {
    ...
    if (fileName.includes('api') || fileName.includes('service') || fileName.includes('handler')) {
      return 'Service API';
    }
  ...
  public updateBacklogJson(): void {
    ...
    for (const task of plan.tasks) {
      ...
      const exists = this.backlog.tasks.some(
        (t: any) =>
          t.description === task.description || (t.type === task.type && t.target === task.target)
      );
