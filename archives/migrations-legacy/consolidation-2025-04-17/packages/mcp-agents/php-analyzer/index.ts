...
import { AgentContext, AgentResult, BaseMcpAgent } from '../shared/base-agent';
...
export class PhpAnalyzerAgent extends BaseMcpAgent {
  ...
  version = '1.2.0';
  description =
    'Analyse le code PHP pour extraire la structure et préparer la migration vers Remix';
  ...
  async validate(context: AgentContext): Promise<boolean> {
    ...
    return baseValidation && !!context.sourceFile && context.sourceFile.endsWith('.php');
  }
  ...
  private extractDatabaseInfo(phpContent: string): {
    ...
    const sqlQueries = (
      phpContent.match(/(?:mysql_query|mysqli_query|query)\s*\(\s*['"]([^'"]+)['"]/g) || []
  ...
  private suggestRemixComponents(phpContent: string, routes: any[], templates: string[]): any[] {
