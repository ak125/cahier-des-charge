...
export class AgentOrchestrator {
  ...
  async runPipeline(
    ...
    try {
      ...
      if (options.ciMode && this.pipelineStatus.overallStatus === 'error') {
        console.error(
          '❌ Pipeline terminé avec des erreurs. Vérifiez les rapports pour plus de détails.'
        );
        process.exit(1);
      }
      ...
      console.log(
        `✅ Pipeline terminé en ${duration}s avec le statut: ${this.pipelineStatus.overallStatus}`
      );
  ...
  private async runAgent(
    agentName: string,
    options: { autoRemediation: boolean }
  ): Promise<AgentResult> {
  ...
  private updateAgentStatus(
    ...
    this.pipelineStatus.agents[agentName] = {
      ...
      successRate:
        errorCount === 0
          ? 100
          : Math.max(0, 100 - (errorCount / (errorCount + warningCount + 1)) * 100),
  ...
  private updateIssuesSummary(result: AgentResult): void {
    ...
    const criticalIssues = result.logs.filter(
      (log) =>
        log.includes('❌') &&
    ...
    const highIssues = result.logs.filter(
      (log) => log.includes('❌') && !log.includes('CRITICAL')
    ).length;

    const mediumIssues = result.logs.filter(
      (log) => log.includes('⚠️') && (log.includes('WARNING') || log.includes('important'))
    ).length;

    const lowIssues = result.logs.filter(
      (log) => log.includes('⚠️') && !log.includes('WARNING') && !log.includes('important')
    ).length;
  ...
  private updateProgressData(): void {
    try {
      ...
      const migratedCount = this.countFilesWithCondition(
        'reports/migration',
        'status',
        'completed'
      );
      const verifiedCount = this.countFilesWithCondition(
  ...
  private async runPerformanceBenchmarks(): Promise<void> {
    ...
    try {
      ...
      const responseTimeImprovement =
        (1 - perfData.migrated.responseTime / perfData.legacy.responseTime) * 100;
      const throughputImprovement =
        (perfData.migrated.throughput / perfData.legacy.throughput - 1) * 100;
      const memoryImprovement =
        (1 - perfData.migrated.memoryUsage / perfData.legacy.memoryUsage) * 100;
      ...
      console.log(
        `✅ Benchmarks terminés. Amélioration de la réponse: ${responseTimeImprovement.toFixed(1)}%`
      );
    } catch (error) {
  ...
  private generateMarkdownReport(): void {
    ...
    // Résumé général
    report += `## 📊 Résumé\n\n`;
    report += `- **Statut global**: ${this.getStatusEmoji(this.pipelineStatus.overallStatus)} ${
      this.pipelineStatus.overallStatus
    }\n`;
    ...
    report += `- **Problèmes**: ${
      this.pipelineStatus.issuesSummary.critical +
      this.pipelineStatus.issuesSummary.high +
      this.pipelineStatus.issuesSummary.medium +
      this.pipelineStatus.issuesSummary.low
    } (${this.pipelineStatus.issuesSummary.autoFixed} corrigés automatiquement)\n\n`;
    ...
    Object.entries(this.pipelineStatus.agents).forEach(([agentName, agentData]) => {
      report += `| ${agentName} | ${this.getStatusEmoji(agentData.status)} ${agentData.status} | ${
        agentData.issuesCount
      } | ${agentData.fixedCount} | ${agentData.successRate.toFixed(1)}% |\n`;
    });
  ...
  private countFiles(dirPath: string, extension: string): number {
    try {
      ...
      const result = execSync(`find ${fullPath} -name "*${extension}" | wc -l`, {
        encoding: 'utf8',
      });
...
if (require.main === module) {
  ...
  const options = {
    agents: args.includes('--agents') ? args[args.indexOf('--agents') + 1]?.split(',') : undefined,
