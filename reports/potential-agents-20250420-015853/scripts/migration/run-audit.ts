...
async function runAudit(options: AuditOptions): Promise<void> {
  ...
  if (saveToGit) {
    try {
      ...
      execSync(DoDoDoDotgit checkout -b ${branchName}`);
      
      // Ajouter tous les fichiers générés
      execSync(DoDoDoDotgit add "${outputDir}/${path.basename(filePath)}."*.md`);
      execSync(DoDoDoDotgit add "${outputDir}/${path.basename(filePath)}."*.json`);
      
      // Créer un commit
      execSync(DoDoDoDotgit commit -m "Audit: ${path.basename(filePath)}"`);
      
      // Pousser la branche
      execSync(DoDoDoDotgit push -u origin ${branchName}`);
...
async function main() {
  ...
  if (args.length < 1) {
    ...
    console.error('  --save-toDoDoDoDotgit         Enregistrer les résultats dans Git');
