...
function extractRemixMigrationInfo(filePath: string): MigrationPlan {
  ...
  if (endpointsMatch) {
    ...
    while ((match = tableRowsRegex.exec(endpointsContent)) !== null) {
      ...
      if (source !== 'Élément PHP' && target !== 'Cible Remix') {
        ...
        if (
          source.toLowerCase().includes('table') ||
  ...
  const fieldsRegex =
    /(field|property|champ|propriété|colonne|input)\s+['"]?(\w+)['"]?\s+(is|of type|de type)\s+['"]?(\w+)['"]?/gi;
  let fieldMatch;
...
function generateRouteFile(plan: MigrationPlan): string {
  ...
  const generateImports = () => {
    let imports = `import { json, LoaderFunction, MetaFunction } from "@remix-run/node";\n`;
    imports += `import { useLoaderData, Link } from "@remix-run/react";\n`;

    if (hasForms) {
      imports += `import { Form, useActionData, useSubmit, useTransition } from "@remix-run/react";\n`;
      imports += `import { ActionFunction } from "@remix-run/node";\n`;
      imports += `import { z } from "zod";\n`;
      imports += `import { conform, useForm } from "@conform-to/react";\n`;
      imports += `import { parse } from "@conform-to/zod";\n`;
    }
...
function generateLoaderFile(plan: MigrationPlan): string {
  ...
  return `import { json, LoaderFunction } from "@remix-run/node";
...
function generateActionFile(plan: MigrationPlan): string {
  ...
  return `import { json, ActionFunction } from "@remix-run/node";
import { z } from "zod";
...
function generateRemixRoute(plan: MigrationPlan, dryRun = false): void {
  ...
  files.forEach((file) => {
    if (file.content) {
      // Ne pas écrire les fichiers vides
