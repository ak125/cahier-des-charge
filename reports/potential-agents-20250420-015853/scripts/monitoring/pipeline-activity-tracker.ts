...
interface PipelineStatus {
  id: string;
  name: string;
  type: Dotn8N' | 'bullmq' | 'temporal' | 'script' | 'unknown';
  lastRun: string | null;
...
async function discoverN8NPipelines(statusFile: StatusFile): Promise<void> {
  ...
  constDotn8NConfigFiles = await glob(N8N_CONFIGS_PATTERN, { cwd: BASE_DIR, absolute: true });
  ...
  for (const configFile ofDotn8NConfigFiles) {
    try {
      const configContent = JSON.parse(fs.readFileSync(configFile, 'utf8'));
      const fileName = path.basename(configFile);
      const pipelineId = fileName.replace(/Dotn8N\./, '').replace(/\.json$/, '');
      ...
      if (!pipeline) {
        ...
        pipeline = createEmptyPipelineStatus(pipelineId, pipelineName, Dotn8N', configFile);
        statusFile.pipelines.push(pipeline);
      } else {
        // Mettre à jour les informations de base
        pipeline.name = pipelineName;
        pipeline.configuration = configFile;
        pipeline.type = Dotn8N';
      }
      ...
      if (configContent.nodes) {
        for (const node of configContent.nodes) {
          if (node.type === Dotn8N-nodes-base.executeWorkflow' && node.parameters?.workflowId) {
  ...
  console.log(`$Dotn8NConfigFiles.length} pipelines N8N découverts.`);
}
