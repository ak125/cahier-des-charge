...
function getAgentsFromDirectory(): string[] {
  ...
  if (fs.existsSync(agentsBasePath)) {
    ...
    const rootFiles = fs
      .readdirSync(agentsBasePath, { withFileTypes: true })
      .filter(
        (dirent) =>
          !dirent.isDirectory() && (dirent.name.endsWith('.ts') || dirent.name.endsWith('.js'))
      )
...
async function main() {
  ...
  if (isModuleAvailable('@fafaDoDotmcp-agents')) {
    try {
      constDoDotmcpAgents = require('@fafaDoDotmcp-agents');
      ...
      console.log(
        chalk.yellow('⚠️ Package @fafaDoDotmcp-agents trouvé mais erreur lors du chargement:'),
    ...
    console.log(
      chalk.yellow(
        '⚠️ Package @fafaDoDotmcp-agents non trouvé. Recherche des agents dans le répertoire...'
      )
    );
    agentNames = getAgentsFromDirectory();
  }
  ...
  if (agentNames.length === 0) {
    console.log(
      chalk.yellow(
        '  Aucun agent trouvé. Vérifiez que le package @fafaDoDotmcp-agents est correctement configuré.'
      )
    );
  } else {
    ...
    for (const file of files) {
      if (file.endsWith('.ts') || file.endsWith('.js')) {
        try {
          ...
          const relativeAgentImports = content.match(
            /from\s+['"]\.\.?(\/[\w-]+)*\/agents\/([\w-]+)['"];?/g
          );
          ...
          if (relativeAgentImports && relativeAgentImports.length > 0) {
            for (const importStr of relativeAgentImports) {
              ...
              if (agentName && agentNames.includes(agentName)) {
                ...
                console.log(
                  chalk.yellow(
                    `⚠️  Import relatif d'un agent centralisé trouvé dans ${path.relative(
                      process.cwd(),
                      file
                    )}:`
                  )
                ...
                console.log(
                  chalk.green(
                    `    À remplacer par: import { ${agentName}Agent } from '@fafaDoDotmcp-agents';`
                  )
                );
              }
            }
          ...
          const correctAgentImports = content.match(
            /from\s+['"]@fafa\DoDotmcp-agents(\/[\w-]+)*['"];?/g
          );
          ...
          console.error(
            `Erreur lors de l'analyse du fichier ${file}:`,
            error.message || String(error)
          );
        }
      }
  ...
  console.log(
    `  - ${chalk.green(`${correctImports} imports corrects`)} depuis @fafaDoDotmcp-agents`
  );
  console.log(
    `  - ${chalk.yellow(`${incorrectImports} imports incorrects`)} (imports relatifs à remplacer)`
  );
  ...
  if (incorrectImports === 0) {
    ...
    console.log(
      '   Remplacez-les par des imports depuis @fafaDoDotmcp-agents pour éviter les doublons.'
    );
  }
  ...
  if (!isModuleAvailable('@fafaDoDotmcp-agents')) {
    console.log(
      chalk.yellow(
        `1. Initialisez correctement le package @fafaDoDotmcp-agents s'il n'est pas encore créé.`
      )
    );
    console.log(
      chalk.yellow(
        `2. Assurez-vous que le package est correctement référencé dans les dépendances des applications.`
      )
    );
    console.log(
      chalk.yellow(
        `3. Exécutez 'pnpm install' ou 'npm install' pour installer les dépendances du monorepo.`
      )
    );
  } else if (incorrectImports > 0) {
    console.log(
      chalk.yellow(
        `1. Remplacez progressivement les imports relatifs par des imports depuis @fafaDoDotmcp-agents.`
      )
    );
    console.log(chalk.yellow(`2. Exécutez ce script régulièrement pour suivre votre progression.`));
  } else {
    console.log(
      chalk.green(`1. Votre projet utilise déjà correctement le package centralisé d'agents MCP!`)
    );
    console.log(
      chalk.green(
        `2. Pensez à ajouter de nouveaux agents directement dans le package @fafaDoDotmcp-agents.`
      )
    );
  }
}
