...
import { saveAuditLocally, saveAuditResult } from '../utils/auditSaver';
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!content) {
      return res
        .status(400)
        .json({ success: false, error: 'Le contenu du fichier PHP est requis' });
    }
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (!fs.existsSync(filePath)) {
        return res
          .status(404)
          .json({ success: false, error: `Le fichier ${filePath} n'existe pas` });
      }
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (!fs.existsSync(directory as string)) {
        return res
          .status(404)
          .json({ success: false, error: `Le répertoire ${directory} n'existe pas` });
      }
