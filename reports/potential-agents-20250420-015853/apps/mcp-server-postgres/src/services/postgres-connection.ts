...
export class PostgresConnectionService {
  ...
  async connect(): Promise<void> {
    try {
      ...
      try {
        ...
        console.log(
          `Connexion établie à PostgreSQL: ${maskConnectionString(this.connectionString)}`
        );
      } finally {
        client.release();
      }
  ...
  async getTableInfo(tableName: string): Promise<TableInfo> {
    try {
      ...
      if (tableQuery.rowCount === 0) {
        throw new Error(
          `La table '${tableName}' n'existe pas dans le schéma '${this.currentSchema}'.`
        );
      }
  ...
  async exportPrismaSchema(): Promise<string> {
    try {
      ...
      const toPrismaType = (pgType: string, isArray = false): string => {
      ...
      for (const [tableName, tableInfo] of Object.entries(schemaMap.tables)) {
        ...
        if (schemaMap.foreignKeys) {
          const relationsForThisTable = schemaMap.foreignKeys.filter(
            (fk) => fk.sourceTable === tableName
          );
          ...
          for (const fk of relationsForThisTable) {
            ...
            // Ajouter la relation
            prismaSchema += `  ${relationName} ${targetModelName} @relation(fields: [${fk.sourceColumns.join(
              ', '
            )}], references: [${fk.targetColumns.join(', ')}])\n`;
          }

          // Ajouter les relations inverses
          const relationsToThisTable = schemaMap.foreignKeys.filter(
            (fk) => fk.targetTable === tableName
          );
  ...
  async getPerformanceStats(): Promise<any> {
    try {
      ...
      if (error.message.includes('pg_stat_statements')) {
        console.warn(
          'Extension pg_stat_statements non disponible. Les statistiques de requêtes lentes ne seront pas incluses.'
        );
