...
import { Project, SourceFile, ClassDeclaration, Decorator } from 'ts-morph';
 ...
 */
function analyzeSharedPackages(
  projectRoot: string,
  projectStructure: ProjectStructure
): void {
  // Chercher les packages dans les dossiers courants
  const packagesLocations = [
    'packages',
    'libs',
    'shared'
  ];
        ...
        if (fs.existsSync(packageJsonPath)) {
          try {
            ...
            if (packageJson.dependencies) {
              ...
              projectStructure.shared.dependencies[packageFullName] = Object.keys(packageJson.dependencies);
            }
          } catch (error) {
 ...
 */
function collectFileStatistics(
  projectRoot: string,
  projectStructure: ProjectStructure
): void {
  ...
  function traverseDirectory(dirPath: string) {
    ...
    for (const entry of entries) {
      ...
      if (entry.name === 'node_modules' || entry.name === 'DoDoDoDotgit' || entry.name === 'dist') {
        continue;
      }
...
function generateModuleMap(
  ...
  for (const moduleName of projectStructure.backend.modules) {
    ...
    const controllers = projectStructure.backend.controllers
      .filter(c => c.includes(moduleName.replace('Module', '')));
    ...
    const services = projectStructure.backend.services
      .filter(s => s.includes(moduleName.replace('Module', '')));
  ...
  for (const model of projectStructure.prisma.models) {
    ...
    const relations = projectStructure.prisma.relations
      .filter(r => r.startsWith(`${model} ->`));
  ...
  for (const pkg of projectStructure.shared.packages) {
    ...
    if (projectStructure.shared.dependencies && projectStructure.shared.dependencies[pkg]) {
      ...
      if (dependencies.length > 0) {
        markdown += `- Dépendances: ${dependencies.slice(0, 5).join(', ')}${dependencies.length > 5 ? '...' : ''}\n`;
      }
    }
