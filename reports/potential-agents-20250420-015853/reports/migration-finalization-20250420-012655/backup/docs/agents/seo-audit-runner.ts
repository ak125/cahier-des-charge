...
export class SEOAuditRunner implements BaseAgent, BusinessAgent, BaseAgent, BusinessAgent, MCPAgent<SEOAuditConfig> {
  id = 'seo-audit-runner';
  name = 'SEO Audit Runner';
  description = 'Exécute des audits Lighthouse sur les routes Remix et génère des rapports d\'analyse SEO';
  version = '1.0.0';
  ...
  async run(): Promise<void> {
    ...
    for (const batch of batches) {
      this.context.logger.info(`Traitement du lot ${batchNumber}/${batches.length} (${batch.length} routes)`);
    ...
    if (averageSeoScore < this.config.minSeoScore) {
      const message = `⚠️ Score SEO moyen (${averageSeoScore.toFixed(2)}) inférieur au minimum requis (${this.config.minSeoScore})`;
  ...
  private async determineRoutesToAudit(): Promise<string[]> {
    if (Array.isArray(this.config.routes)) {
      ...
      return this.config.routes.filter(route => 
        !this.config.skipRoutes.some(skipPattern => 
          new RegExp(skipPattern).test(route)
        )
      );
  ...
  private async runLighthouse(options: AuditOptions): Promise<AuditResult> {
    const { url, output = 'json', outputPath = path.join(this.config.outputDir, this.sanitizeFileName(url)) } = options;
   ...
   */
  private extractSEOIssues(report: any): Array<{ title: string; description: string; severity: string; score: number }> {
    const issues = [];
      ...
      if ((auditData.score !== null && auditData.score < 0.9) || auditData.score === null) {
        if (report.categoryGroups[auditData.group]?.title?.includes('SEO') || id.includes('seo')) {
          issues.push({
            ...
            severity: auditData.score === null ? 'info' : auditData.score < 0.5 ? 'error' : 'warning',
  ...
  private generateRecommendations(issues: any[], seoScore: number): string[] {
    ...
    if (seoScore < 50) {
      recommendations.push('⚠️ Votre page a des problèmes SEO critiques qui doivent être résolus rapidement.');
    } else if (seoScore < 80) {
      recommendations.push('📝 Votre page présente plusieurs opportunités d\'amélioration SEO.');
    } else if (seoScore < 90) {
      recommendations.push('✅ Votre page a un bon score SEO, mais quelques améliorations peuvent encore être apportées.');
    } else {
    ...
    const metaTagsIssues = issues.filter(i => 
      i.title.includes('Meta') || i.title.includes('Description') || i.title.includes('Title')
    );
    
    if (metaTagsIssues.length > 0) {
      recommendations.push('📋 Améliorez vos balises meta : title et description doivent être présentes, uniques et de bonne longueur.');
    }
    
    const linkIssues = issues.filter(i => 
      i.title.includes('link') || i.title.includes('Link') || i.title.includes('anchor')
    );
    
    if (linkIssues.length > 0) {
      recommendations.push('🔗 Optimisez les liens : assurez-vous que tous les liens ont des textes descriptifs et évitez les liens brisés.');
    }
    
    const mobileIssues = issues.filter(i => 
      i.title.includes('mobile') || i.title.includes('viewport')
    );
    
    if (mobileIssues.length > 0) {
      recommendations.push('📱 Optimisez pour mobile : assurez-vous que votre page est adaptée aux appareils mobiles et que le contenu est lisible sans zoom.');
    }
    
    // Recommendation générale pour des scores faibles
    if (recommendations.length <= 2 && seoScore < 70) {
      recommendations.push('🔍 Envisagez un audit SEO complet pour identifier tous les problèmes et opportunités d\'amélioration.');
    }
...
${result.htmlReportPath ? `- **Rapport complet**: [HTML](${path.relative(this.config.outputDir, result.htmlReportPath)})\n` : ''}
