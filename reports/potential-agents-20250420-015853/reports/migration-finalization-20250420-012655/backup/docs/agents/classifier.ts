  ...
  implements BaseAgent, BusinessAgent, BaseAgent, BusinessAgent, AnalyzerAgent
{
  ...
  private determineTableType(table: TableInfo): TableType {
    ...
    // Vérifier s'il s'DoDoDoDotgit d'une table de jonction (généralement quelques colonnes et des clés étrangères)
    ...
    // Déterminer s'il s'DoDoDoDotgit d'une table technique basée sur d'autres critères
  ...
  private isBusinessTableByStructure(table: TableInfo): boolean {
    ...
    const hasTimestampColumns = Object.values(table.columns).some((col) =>
      ['created_at', 'updated_at', 'deleted_at', 'date_created', 'date_updated'].includes(
