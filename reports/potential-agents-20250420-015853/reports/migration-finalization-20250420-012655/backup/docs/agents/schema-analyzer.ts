...
export class SchemaAnalyzer implements BaseAgent, BusinessAgent, BaseAgent, BusinessAgent , AnalyzerAgent{
  /**
    ...
    Object.entries(schema.tables).forEach(([tableName, table]) => {
      ...
      Object.values(table.columns).forEach(column => {
        ...
        if (column.suggestedPostgresType) {
          ...
          stats.suggestedTypeConversions[conversion] = (stats.suggestedTypeConversions[conversion] || 0) + 1;
        }
      });
  ...
  private generateTableStats(tableName: string, table: TableInfo): TableStats {
    const columnCount = Object.keys(table.columns).length;
    const primaryKeyColumns = typeof table.primaryKey === 'string' 
      ? [table.primaryKey] 
      : (table.primaryKey || []);
    ...
    } else if (table.name.toLowerCase().includes('log') || 
    ...
    } else if (table.name.toLowerCase().includes('cache') || 
  ...
  generatePrismaMapping(schema: MySQLSchema): PrismaMapping {
    ...
    Object.entries(schema.tables).forEach(([tableName, table]) => {
      Object.entries(table.columns).forEach(([columnName, column]) => {
        if (column.type.toUpperCase() === 'ENUM' && column.suggestedPrismaType && column.suggestedPrismaType !== 'String') {
    ...
    Object.entries(schema.tables).forEach(([tableName, table]) => {
      prismaMapping.models[this.toPascalCase(tableName)] = this.tableToPrismaModel(tableName, table, schema);
    });
   ...
   */
  private tableToPrismaModel(tableName: string, table: TableInfo, schema: MySQLSchema): PrismaModel {
    ...
    Object.entries(table.columns).forEach(([columnName, column]) => {
      model.fields[this.toCamelCase(columnName)] = this.columnToPrismaField(columnName, column, table, schema);
    });
  ...
  private columnToPrismaField(
    ...
    if (column.defaultValue !== undefined && column.defaultValue !== null) {
      if (column.defaultValue === 'CURRENT_TIMESTAMP' || column.defaultValue === 'NOW()') {
        ...
        field.default = (column.defaultValue === '1' || column.defaultValue.toLowerCase() === 'true') ? 'true' : 'false';
      } else {
        field.default = `"${column.defaultValue}"`;
      }
    }
    ...
    // Gérer les relations
    if (table.foreignKeys.some(fk => fk.columns.includes(columnName)) || column.isImplicitForeignKey) {
