...
function extractAgentInfo(filePath: string): AgentInfo {
  ...
  const baseName = nameWithoutExt.replace(/\.v\d+$/, '').replace(/\.(backup|bak|old)$/, '');
...
function generateRecommendations(group: AgentGroup): string[] {
  ...
  if (backups.length > 0) {
    recommendations.push(
      `Supprimer les fichiers de sauvegarde pour ${group.baseName}: ${backups
  ...
  if (versioned.length > 1) {
    ...
    if (olderVersions.length > 0) {
      recommendations.push(
        `Archiver ou supprimer les anciennes versions: ${olderVersions
          .map((a) => path.basename(a.path))
          .join(', ')}`
      );
    }
  ...
  if (current.length > 1) {
    ...
    recommendations.push(
      `Plusieurs versions actives de ${
        group.baseName
      } détectées. Utiliser la plus récente (${path.basename(mostRecent.path)})`
    );

    if (older.length > 0) {
      recommendations.push(
        `Versionner ou supprimer les autres versions: ${older
          .map((a) => path.basename(a.path))
          .join(', ')}`
      );
    }
...
async function auditAgentVersions(baseDir: string): Promise<AuditResult> {
  ...
  const globalVersionScore =
    agentGroups.reduce((sum, group) => sum + group.versionScore, 0) /
  ...
  const allRecommendations = agentGroups
    ...
    .flatMap((group) =>
      generateRecommendations(group).map((rec) => ({ group, recommendation: rec }))
    );
...
export { auditAgentVersions, extractAgentInfo, calculateVersionScore, generateRecommendations };
