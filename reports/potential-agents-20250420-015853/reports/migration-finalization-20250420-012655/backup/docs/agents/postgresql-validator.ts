...
program
  ...
  .option('--schema <path>', 'Chemin vers le fichier schema.prisma', './apps/backend/prisma/schema.prisma')
...
class ValidationIssue implements BaseAgent, BusinessAgent, ValidatorAgent {
  constructor(
    public type: 'TYPE_MISMATCH' | 'MISSING_COLUMN' | 'MISSING_TABLE' | 'MISSING_INDEX' | 'CONSTRAINT_MISMATCH' | 'EXTRA_COLUMN' | 'EXTRA_TABLE',
  ...
  try {
    ...
    try {
      ...
      if (options.fix && validationResult.issues.some(issue => issue.fixAvailable)) {
        ...
        await applyFixes(client, validationResult.issues.filter(issue => issue.fixAvailable));
      }
      ...
      if (validationResult.status === 'SUCCESS') {
        ...
        console.log(chalk.yellow('⚠️ Les schémas présentent des différences mineures. Voir le rapport pour les détails.'));
      } else {
        console.log(chalk.red('❌ Des problèmes majeurs ont été détectés entre les schémas. Voir le rapport pour les détails.'));
      }
    } finally {
...
async function extractPrismaModels(schemaPath: string): Promise<PrismaModel[]> {
  ...
  try {
    ...
    for (const modelBlock of modelBlocks) {
      ...
      const fieldMatches = modelBlock.matchAll(/^\s*(\w+)\s+(\w+(?:\[\])?)\s*(?:@\s*([^{}]*))?(?:{([^}]*)})?/gm);
      ...
      for (const fieldMatch of fieldMatches) {
        ...
        if (fieldAttributes.includes('@relation')) {
          const relationMatch = fieldAttributes.match(/@relation\(\s*["']?([^"',)]+)["']?(?:,\s*(?:fields:\s*\[(.*?)\],\s*references:\s*\[(.*?)\]))?/);
          if (relationMatch) {
            relationName = relationMatch[1];
            relationFromFields = relationMatch[2]?.split(',').map(f => f.trim().replace(/["']/g, ''));
            relationToFields = relationMatch[3]?.split(',').map(f => f.trim().replace(/["']/g, ''));
          }
        }
 ...
 */
async function validateSchemas(prismaModels: PrismaModel[], postgresTables: PostgresTable[]): Promise<ValidationResult> {
  ...
  for (const model of prismaModels) {
    ...
    for (const field of model.fields) {
      ...
      if (!postgresColumn) {
        ...
        if (field.relationFromFields && field.relationFromFields.length > 0) {
          ...
          const missingColumns = field.relationFromFields.filter(relField => !columnsMap.has(relField));
          if (missingColumns.length > 0) {
            issues.push(new ValidationIssue(
              'MISSING_COLUMN',
              `${model.name}.${field.name}`,
              `Les colonnes de relation ${missingColumns.join(', ')} définies dans le modèle Prisma n'existent pas dans la table "${tableName}"`,
              'HIGH',
              false
            ));
            tableHasIssue = true;
          ...
          issues.push(new ValidationIssue(
            ...
            field.isRequired ? undefined : 
              `ALTER TABLE "${tableName}" ADD COLUMN "${columnName}" ${mapPrismaTypeToPostgres(field)};`
          ));
          tableHasIssue = true;
        }
        continue;
    ...
    const extraColumns = postgresTable.columns
      .filter(col => !modelColumnNames.has(col.name) && 
              !col.name.endsWith('_id') && // Ignorer les potentielles clés étrangères
              !['created_at', 'updated_at'].includes(col.name)); // Ignorer les timestamps standard
  ...
  const extraTables = postgresTables
    .filter(table => !prismaTableNames.has(table.name) && 
            !table.name.includes('_prisma_') && // Ignorer les tables internes de Prisma
            !['migrations', 'schema_migrations', '_prisma_migrations'].includes(table.name)); // Ignorer les tables de migration
  ...
  if (issues.some(i => i.type === 'TYPE_MISMATCH')) {
    recommendations.push("Exécutez une migration Prisma pour mettre à jour les types de données incompatibles.");
  }
  
  if (issues.some(i => i.type === 'MISSING_COLUMN' && i.fixAvailable)) {
    recommendations.push("Des colonnes manquantes non obligatoires peuvent être ajoutées en utilisant l'option --fix.");
  }
  
  if (issues.some(i => i.type === 'MISSING_TABLE')) {
    recommendations.push("Exécutez une migration Prisma complète pour créer les tables manquantes.");
  }
...
async function applyFixes(client: Client, issues: ValidationIssue[]): Promise<void> {
  ...
  for (const issue of sortedIssues) {
    ...
    if (tableMatch) {
      ...
      try {
        ...
        console.error(chalk.red(`❌ Erreur lors de l'application de la correction pour ${issue.entity}: ${error.message}`));
      }
    }
  }
  ...
  for (const [tableName, commands] of correctionsByTable.entries()) {
    try {
      ...
      console.error(chalk.red(`❌ Erreur lors de l'application des corrections pour la table ${tableName}: ${error.message}`));
    }
  }
}
 ...
 */
async function generateReport(result: ValidationResult, outputDir: string, format: string): Promise<string> {
...
function generateMarkdownReport(result: ValidationResult): string {
  ...
  markdown += `*Généré le ${new Date(result.timestamp).toLocaleDateString('fr-FR')} à ${new Date(result.timestamp).toLocaleTimeString('fr-FR')}*\n\n`;
  ...
  if (result.issues.length > 0) {
    ...
    if (highIssues.length > 0) {
      ...
      highIssues.forEach(issue => {
        markdown += `| ${issue.entity} | ${issue.detail} | ${issue.fixAvailable ? 'Oui' : 'Non'} |\n`;
      });
...
function generateHtmlReport(result: ValidationResult): string {
  ...
  const statusColor = result.status === 'SUCCESS' ? 'green' : (result.status === 'WARNING' ? 'orange' : 'red');
  const statusIcon = result.status === 'SUCCESS' ? '✅' : (result.status === 'WARNING' ? '⚠️' : '❌');
...
<body>
  ...
  <p><em>Généré le ${new Date(result.timestamp).toLocaleDateString('fr-FR')} à ${new Date(result.timestamp).toLocaleTimeString('fr-FR')}</em></p>
...
function mapPostgresColumnType(column: PostgresColumn): string {
  ...
  if (column.characterMaximumLength && ['character varying', 'varchar', 'char', 'character'].includes(type)) {
    type = `${type}(${column.characterMaximumLength})`;
  }
  
  if (column.numericPrecision && column.numericScale !== undefined && 
...
function areTypesCompatible(expectedType: string, actualType: string): boolean {
  ...
  for (const [baseType, compatibleTypes] of Object.entries(compatibilityMap)) {
    if (expectedType.startsWith(baseType) && 
  ...
  if ((expectedType.startsWith('varchar') || expectedType.startsWith('character varying')) && 
      actualType === 'text') {
    return true;
  }
...
import { BusinessAgent, ValidatorAgent } from '@workspaces/cahier-des-charge/src/core/interfaces/business';
