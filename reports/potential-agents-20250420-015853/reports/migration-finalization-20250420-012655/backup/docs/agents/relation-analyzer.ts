...
class RelationAnalyzer implements BaseAgent, BusinessAgent, AnalyzerAgent {
  ...
  private extractExplicitRelations(): void {
    for (const table of this.tables) {
      ...
      for (const fk of table.foreignKeys) {
        ...
        if (!targetTable) {
          this.logger.warn(
            `Table référencée ${fk.references.table} introuvable pour FK ${table.name}.${fk.column}`
          );
          continue;
        }
  ...
  private detectImplicitRelations(): void {
      ...
      for (const column of table.columns) {
        ...
        if (
          ...
          if (refTableName) {
            ...
            const targetTable = this.tables.find(
              (t) =>
                t.name.toUpperCase() === refTableName || t.name.toUpperCase() === refTableName + 'S'
            );

            if (targetTable) {
              // Trouver la colonne id dans la table cible
              const targetIdColumn = targetTable.columns.find(
                (c) => c.isPrimary || c.name === 'id' || c.name === 'ID'
              );
    ...
    this.logger.info(
      `Identifié ${
        this.relations.length - initialRelationCount
      } relations implicites additionnelles`
    );
  }
  ...
  private detectPolymorphism(): void {
    ...
    for (const table of this.tables) {
      ...
      const idColumns = table.columns.filter(
        (col) =>
          col.name.endsWith('_id') && !table.foreignKeys?.some((fk) => fk.column === col.name)
      );

      for (const idColumn of idColumns) {
        // Chercher le type correspondant (entity_type, parent_type, etc.)
        const baseColumnName = idColumn.name.replace(/_id$/, '');
        const typeColumn = table.columns.find(
          (col) => col.name === `${baseColumnName}_type` || col.name === `${baseColumnName}Type`
        );
            ...
            for (const refTable of possibleReferences) {
              ...
              if (refIdColumn) {
                const relation: Relation = {
                  ...
                  prisma_relation: {
                    relation_name: `${this.suggestRelationName(table.name, idColumn.name)}On${
                      refTable.name
                    }`,
                    onDelete: 'SetNull',
                    optional: true,
  ...
  private checkCascadeRisks(): void {
    for (const relation of this.relations) {
      ...
      if (relation.on_delete === 'CASCADE') {
        ...
        const dependentTableCount = this.relations.filter(
          (r) => r.to_table === relation.to_table
        ).length;
  ...
  private determinePrismaRelationType(relation: Relation): PrismaRelation['relation_type'] {
    ...
    const hasMultipleReferencesToTarget =
      this.relations.filter(
        (r) => r.to_table === relation.to_table && r.from_table !== relation.from_table
      ).length > 0;
    ...
    if (isSourceJunction) {
      ...
      const isSourceColumnUnique = sourceTable?.columns.find(
        (c) => c.name === relation.from_column
      )?.isPrimary;
  ...
  private suggestRelationName(tableName: string, columnName: string): string {
    ...
    let relationName = columnName.replace(/_id$/, '').replace(/Id$/, '').replace(/ID$/, '');
  ...
  private generateRelationAuditMarkdown(): string {
    ...
    markdown += `- Relations explicites: **${
      this.relations.filter((r) => r.confidence === 1.0).length
    }**\n`;
    markdown += `- Relations implicites: **${
      this.relations.filter((r) => r.confidence < 1.0 && !r.is_polymorphic).length
    }**\n`;
    markdown += `- Relations polymorphiques: **${
      this.relations.filter((r) => r.is_polymorphic).length
    }**\n`;
    markdown += `- Tables avec risques de cascade: **${
      this.relations.filter((r) => r.warning && r.warning.includes('Cascade')).length
    }**\n\n`;
    ...
    if (centralTables.length > 0) {
      ...
      for (const table of centralTables) {
        markdown += `- **${table.label}** - Score de centralité: ${table.centrality.toFixed(2)} (${
          table.relations_in
        } entrantes, ${table.relations_out} sortantes)\n`;
      }
      markdown += `\n`;
    }
    ...
    if (implicitRelations.length > 0) {
      ...
      for (const relation of implicitRelations) {
        markdown += `### ${relation.from_table} → ${
          relation.to_table
        } (confiance: ${relation.confidence.toFixed(2)})\n\n`;
        ...
        markdown += `  ALTER TABLE ${
          relation.from_table
        } ADD CONSTRAINT fk_${relation.from_table.toLowerCase()}_${relation.from_column}\n`;
