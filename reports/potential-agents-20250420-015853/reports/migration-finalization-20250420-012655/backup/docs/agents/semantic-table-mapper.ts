...
export class SemanticMapper implements BaseAgent, BusinessAgent, AnalyzerAgent {
  ...
  public async analyze(): Promise<SemanticMapResult> {
    ...
    const technicalCategories = Array.from(
      new Set(this.config.technicalTablePatterns.map((pattern: any) => pattern.category))
    );
    ...
    result.metadata.analysisCoverage =
      tableNames.length > 0 ? result.metadata.mappedTables / tableNames.length : 0;
  ...
  private checkJunctionTable(tableName: string, tableDefinition: SemanticTableDefinition): void {
    ...
    const isJunction =
      hasForeignKeys && (nameMatchesPattern || nameIndicatesJunction || hasLimitedColumns);
  ...
  private identifyEntityCategory(
    tableName: string,
    tableDefinition: SemanticTableDefinition
  ): void {
  ...
  private generateTags(tableDefinition: SemanticTableDefinition, columnNames: string[]): void {
    ...
    const booleanColumns = columnNames.filter(
      (col) =>
        col.startsWith('is_') ||
        col.startsWith('has_') ||
        col.startsWith('est_') ||
    ...
    const configColumns = columnNames.filter(
      (col) =>
        col.includes('config') ||
        col.includes('setting') ||
        col.includes('param') ||
        col.includes('option')
    );
    ...
    const historyColumns = columnNames.filter(
      (col) =>
        col.includes('history') ||
        col.includes('log') ||
        col.includes('historique') ||
        col.includes('archive') ||
        col.includes('version') ||
        col.includes('revision')
    );
    ...
    const geoColumns = columnNames.filter(
      (col) =>
        col.includes('lat') ||
        col.includes('lon') ||
        col.includes('coord') ||
    ...
    const temporalColumns = columnNames.filter(
      (col) =>
        col.includes('date') ||
        col.includes('time') ||
        col.includes('duration') ||
        col.includes('période') ||
        col.includes('année') ||
        col.includes('mois') ||
        col.includes('jour') ||
        col.includes('heure')
    ...
    const financialColumns = columnNames.filter(
      (col) =>
        col.includes('prix') ||
        col.includes('montant') ||
        col.includes('coût') ||
        col.includes('tarif') ||
        col.includes('budget') ||
        col.includes('finance') ||
  ...
  private generateSummaryReport(
    ...
    reportContent += `- Couverture de l'analyse: ${(
      results.metadata.analysisCoverage * 100
    ).toFixed(2)}%\n`;
    ...
    for (const category in results.entityGroups) {
      ...
      if (tables.length > 0) {
        ...
        tables.forEach((table) => {
          ...
          reportContent += `- ${table}: ${tableDef.role} (confiance: ${(
            tableDef.confidence * 100
          ).toFixed(0)}%)\n`;
        });
        reportContent += `\n`;
      }
  ...
  public generateMarkdownReport(result: SemanticMapResult): string {
    ...
    markdown += `\`\`\`\n`;
    markdown += `Entités métier  [${this.generateBarChart(
      entityCount,
      result.metadata.mappedTables
    )}] ${entityCount}\n`;
    markdown += `Tables jonction [${this.generateBarChart(
      result.junctionTables.length,
      result.metadata.mappedTables
    )}] ${result.junctionTables.length}\n`;
    markdown += `Tables tech.    [${this.generateBarChart(
      technicalCount,
      result.metadata.mappedTables
    )}] ${technicalCount}\n`;
    markdown += `Tables orphel.  [${this.generateBarChart(
      result.orphanTables.length,
      result.metadata.mappedTables
    )}] ${result.orphanTables.length}\n`;
    markdown += `\`\`\`\n\n`;
  ...
  private generateBarChart(value: number, total: number, width = 20): string {
    if (total === 0) return '';
...
if (require.main === module) {
  if (process.argv.length < 5) {
    console.error(
      'Usage: ts-node semantic-table-mapper.ts <schema-json-path> <config-path> <output-path>'
    );
    process.exit(1);
  }
