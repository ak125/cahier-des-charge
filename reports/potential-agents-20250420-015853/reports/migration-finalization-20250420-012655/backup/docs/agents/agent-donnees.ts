...
class DataAnalyzer implements BaseAgent, BusinessAgent, AnalyzerAgent {
  ...
  private analyzeSources(): DataSource[] {
    ...
    const patterns = [
      ...
      { regex: /\$_COOKIE\['([^']+)'\]/, type: '$_COOKIE', description: 'Données persistantes navigateur' },
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (const pattern of patterns) {
        ...
        if (matches) {
          ...
          const locationText = `ligne ${i + 1}${param ? ' : ' + pattern.type + '[\''+param+'\']' : ''}`;
          
          const existingSource = sources.find(source => 
            source.type === pattern.type && source.location.includes(param)
          );
  ...
  private analyzeOutputs(): DataOutput[] {
    ...
    const hasFileOutput = this.phpContent.match(/readfile\(|fpassthru\(|header\(['"]Content-Disposition: attachment/);
    if (hasFileOutput) {
      outputs.push({
        type: 'Fichiers',
  ...
  public async generateAuditSection(): Promise<string> {
    ...
    if (analysis.sources.length > 0) {
      sourcesText = analysis.sources.map(source => `- ${source.type}${source.location.includes(':') ? source.location.split(':')[1] : ''}`).join('\n');
    } else {
    ...
    if (analysis.outputs.length > 0) {
      outputsText = analysis.outputs.map(output => `- ${output.type} (${output.details})`).join('\n');
    } else {
    ...
    if (analysis.sqlQueries.length > 0) {
      sqlText = "| Requête | Type | Tables | Complexité |\n|--------|------|--------|------------|\n";
      sqlText += analysis.sqlQueries.map(query => 
        `| \`${query.query}\` | ${query.type} | ${query.tables.join(', ')} | ${query.warning ? '⚠️ ' : ''}${query.complexity} |`
      ).join('\n');
    } else {
...
${sourcesText}
${analysis.sources.length > 0 ? `> Fichier lit des données${analysis.sources.some(s => s.type === '$_SESSION') ? ' d\'authentification via SESSION' : ''}${analysis.sources.some(s => s.type === '$_GET' || s.type === '$_POST') ? ' et gère des paramètres URL/POST' : ''}.` : ''}
...
> 🔍 Analyse SQL brute : **${analysis.sqlScore.toFixed(1)} / 3**${analysis.sqlScore < 2.0 ? ' — optimisations nécessaires via Prisma' : analysis.sqlScore < 3.0 ? ' — optimisations possibles via Prisma' : ' — pas d\'optimisation nécessaire'}`;
  }
  ...
  public async updateBacklog(): Promise<void> {
    ...
    if (analysis.sources.length > 0 && !existingTasks.includes('refactorEntrypoints')) {
      backlog.tasks.push({
        ...
        target: "backend",
        status: "pending",
        description: `Refactoriser les entrées de données (${analysis.sources.map(s => s.type).join(', ')})`
      });
    }
    
    // Tâche pour la session
    if (analysis.sources.some(s => s.type === '$_SESSION') && !existingTasks.includes('sanitizeSessionAccess')) {
      backlog.tasks.push({
