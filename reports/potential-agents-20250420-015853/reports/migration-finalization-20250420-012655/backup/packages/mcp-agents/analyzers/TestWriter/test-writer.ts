...
export const testWriterAgent = {
  ...
  async detectNestJSModules(): Promise<{ name: string; path: string }[]> {
    ...
    try {
      ...
      for (const auditFile of auditFiles) {
        ...
        const nestjsMatches = content.match(
          /- \*\*NestJS (?:Service|Controller)\*\*: `([a-zA-Z0-9]+)(?:Service|Controller)`/g
        );
  ...
  async generateNestJSTests(
    ...
    if (hasController) {
      ...
      const controllerTestContent = this.generateControllerTestContent(
        moduleName,
        controllerContent
      );
  ...
  generateServiceTestContent(moduleName: string, serviceContent: string): string {
    ...
    const dependencyRegex =
      /constructor\(\s*(?:private|readonly|protected)?\s*([a-zA-Z0-9_]+)\s*:\s*([a-zA-Z0-9_]+)/g;
    ...
    const mockDependencies = dependencies
      .map((dep) => `const mock${dep.type} = {\n    ${this.generateMockMethods(dep)}\n  };`)
      .join('\n  ');

    // Initialiser les dépendances dans beforeEach
    const initDependencies =
      dependencies.length > 0
        ? `service = new ${moduleName}Service(${dependencies
            .map((dep) => `mock${dep.type} as any`)
            .join(', ')});`
...
`;
  ...
  generateEndpointTest(
    endpoint: { method: string; path: string; handler: string },
    serviceName: string
  ): string {
  ...
  async runNestJSTests(logs: string[]): Promise<boolean> {
    try {
      ...
      logs.push(
        `🧪 Résultat des tests unitaires NestJS: ${output.includes('PASS') ? 'RÉUSSITE' : 'ÉCHEC'}`
      );
  ...
  async runRemixE2ETests(logs: string[]): Promise<boolean> {
    try {
      ...
      logs.push(
        `🧪 Résultat des tests e2e Remix: ${output.includes('PASS') ? 'RÉUSSITE' : 'ÉCHEC'}`
      );
      return output.includes('PASS');
    } catch (error: any) {
