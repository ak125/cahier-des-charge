...
import { Database } from './types'; // Typages générés par Supabase CLI (à générer séparément)
...
if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
  console.error(
    "⚠️ Les variables d'environnement SUPABASE_URL et SUPABASE_SERVICE_ROLE_KEY doivent être définies"
  );
  process.exit(1);
}
...
export class SupabaseHelper {
  ...
  static async updateAuditLog(id: number, updates: Partial<AuditLog>): Promise<boolean> {
    try {
      const { error } = await supabase.from('audit_logs').update(updates).eq('id', id);
  ...
  static async getAuditLogsForFile(fileName: string): Promise<AuditLog[]> {
    try {
      ...
      if (error) {
        console.error(
          "Erreur lors de la récupération des logs d'audit pour le fichier:",
          error.message
        );
        return [];
      }
      ...
      console.error(
        "Exception lors de la récupération des logs d'audit pour le fichier:",
        error.message
      );
      return [];
    }
  ...
  static async updateFileMapping(id: number, updates: Partial<FileMapping>): Promise<boolean> {
    try {
      const { error } = await supabase.from('file_mappings').update(updates).eq('id', id);
   ...
   * @returns true si la mise à jour a réussi
   */
  static async updateCodeSuggestion(
    id: number,
    updates: Partial<CodeSuggestion>
  ): Promise<boolean> {
    try {
      const { error } = await supabase.from('code_suggestions').update(updates).eq('id', id);
  ...
  static async updateMigrationJob(id: number, updates: Partial<MigrationJob>): Promise<boolean> {
    try {
      const { error } = await supabase.from('migration_jobs').update(updates).eq('id', id);
