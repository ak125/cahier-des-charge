import { DataAnalyzer } from './AgentDonnees';
import path from 'path';
import { SecurityRiskAnalyzer } from './AnalyzeSecurityRisks';
 ...
 */
function readCahierDesCharges(cahierPath: string = '/workspaces/cahier-des-charge/cahier-des-charges-backup-20250410-113108'): { content: string; summary: string; keyPoints: string[]; sections: Record<string, string[]>; objectives: string[] } {
  try {
    if (fs.existsSync(cahierPath)) {
      const files = fs.readdirSync(cahierPath).filter(file => 
        ...
        try {
          ...
          for (const line of lines) {
            ...
            // Détection des objectifs et exigences
              ...
              (line.includes('DOIT') || line.includes('EXIGENCE') || line.includes('OBJECTIF')) ||
              (line.startsWith('- ') && (
                line.includes('migr') || line.includes('valid') || line.includes('test') || 
                line.includes('sécurité') || line.includes('performance') || line.includes('qualité')
              ))
            ) {
            ...
            // Détection des points importants (listes avec mots clés)
              ...
              (line.includes(':') || line.includes('stratégie') || line.includes('analyse') || 
               line.includes('système') || line.includes('architecture'))
            ) {
              keyPoints.push(`  • ${line.trim()}`);
            }
          }
      ...
      const filteredKeyPoints = keyPoints.length > maxKeyPoints 
      ...
      const summary = `Le cahier des charges contient ${files.length} documents organisés en ${sectionNames.length} sections principales et définit ${totalPoints} points clés.
Sections principales: ${sectionNames.slice(0, 5).join(', ')}${sectionNames.length > 5 ? '...' : ''}
Objectifs principaux: ${objectives.length > 0 ? objectives.slice(0, 3).join('\n- ') : 'Non spécifiés'}`;
...
if (require.main === module) {
  ...
  // Sinon, utiliser le comportement d'audit de fichier unique
    ...
    console.error('Usage: node agent-audit.ts <file-path> OU node agent-audit.ts --dir <directory-path>');
    process.exit(1);
  }
}