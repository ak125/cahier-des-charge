...
import { SchemaAnalyzer } from './mysql-analyzer/core/SchemaAnalyzer';
...
async function analyzeMySQLSchema() {
  try {
    ...
    const { schema: analyzedSchema, issues: debtIssues } =
      await debtAnalyzer.analyze(normalizedSchema);
    ...
    const tableScores = Object.entries(analyzedSchema.tables).reduce(
      (scores, [tableName, tableInfo]) => {
...
function printSummary(
  ...
  const tablesWithIssues = Object.values(tableScores).filter(({ score }) => score < 3).length;
