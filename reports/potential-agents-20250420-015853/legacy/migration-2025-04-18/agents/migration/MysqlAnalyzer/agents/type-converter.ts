...
export class TypeConverter {
  ...
  private defaultTypeMap: TypeConversionMap = {
    ...
    geometry: {
      prismaType: 'Unsupported("geometry")',
      postgresType: 'Unsupported',
      comment: 'Type géométrique non supporté directement',
    },
    point: {
      prismaType: 'Unsupported("point")',
      postgresType: 'Point',
      comment: "Requiert l'extension PostGIS",
    },
    linestring: {
      prismaType: 'Unsupported("linestring")',
      postgresType: 'Unsupported',
      comment: "Requiert l'extension PostGIS",
    },
    polygon: {
      prismaType: 'Unsupported("polygon")',
      postgresType: 'Unsupported',
      comment: "Requiert l'extension PostGIS",
    },
    ...
    if (schema.sampleData) {
      Object.entries(schema.sampleData).forEach(([tableName, data]) => {
        ...
        columnNames.forEach((columnName) => {
          ...
          if (columnName === 'id' || columnName.endsWith('_id') || columnName.startsWith('id_'))
            return;
    ...
    Object.entries(convertedSchema.tables).forEach(([tableName, table]) => {
      Object.entries(table.columns).forEach(([columnName, column]) => {
        ...
        if (enumValues) {
          ...
          this.detectTypingIssues(tableName, columnName, baseType, length, column, typingIssues);
        }
      });
    });
   ...
   */
  private convertType(mysqlType: string): {
    prismaType: string;
    postgresType?: string;
    comment?: string;
  } {
