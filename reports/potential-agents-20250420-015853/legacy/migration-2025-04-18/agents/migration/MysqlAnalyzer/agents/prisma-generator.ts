...
export class PrismaGenerator {
        ...
        table.relations.forEach((relation) => {
          if (relation.isImplicit) {
            ...
            const inverseRelationName =
              relation.type === 'ONE_TO_ONE' ? relationName : relationName + 's';
  ...
  private addRelationToModel(
    ...
    const existingFieldIndex = sourceModel.fields.findIndex(
      (field) =>
        field.name === relationName && field.attributes.some((attr) => attr.includes('@relation'))
    );
