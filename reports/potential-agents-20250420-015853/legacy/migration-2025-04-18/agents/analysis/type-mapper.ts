...
class TypeMapper {
   ...
   */
  private extractTypeSizeInfo(type: string): {
    baseType: string;
    size?: string;
    precision?: number;
    scale?: number;
  ...
  private getPrismaFieldDefinition(column: Column, table: Table, postgresType: string): string {
    ...
    // Ajouter @default si présent
      ...
      if (prismaType.startsWith('String')) {
        ...
        defaultValue =
          defaultValue === '1' || defaultValue.toLowerCase() === 'true' ? 'true' : 'false';
      }
  ...
  private detectTypeAnomaly(column: Column, table: Table): TypeAnomalies | null {
    ...
    if (baseType === 'JSON' || baseType === 'LONGTEXT' || baseType === 'TEXT') {
      if (
        column.name.includes('json') ||
        column.name.includes('data') ||
        column.name.includes('config')
      ) {
        return {
          type: 'json',
          table: table.name,
  ...
  private generateRecommendationsMarkdown(): string {
    ...
    const sortedGroups = Object.entries(typeGroups).sort(
      (a, b) => b[1].columns.length - a[1].columns.length
    );
    ...
    if (anomalyTypes.length > 0) {
      ...
      for (const type of anomalyTypes) {
        ...
        switch (type) {
          case 'enum':
            title = 'Énumérations MySQL';
            break;
          case 'set':
            title = 'Types SET MySQL';
            break;
          case 'unsigned':
            title = 'Entiers UNSIGNED';
            break;
          case 'precision':
            title = 'Problèmes de précision numérique';
            break;
          case 'timestamp':
            title = 'Valeurs par défaut TIMESTAMP';
            break;
          case 'json':
            title = 'Données JSON sans validation';
            break;
          default:
            title = 'Autres anomalies';
            break;
        }
