...
export class SEORedirectMapper implements MCPAgent<RedirectMapperConfig> {
  id = 'seo-redirect-mapper';
  name = 'SEO Redirect Mapper';
  description = 'Convertit les règles de redirection .htaccess en configuration pour Remix et NestJS';
  version = '1.0.0';
  ...
  async run(): Promise<void> {
    ...
    for (const htaccessPath of this.config.htaccessPaths) {
      if (await fs.pathExists(htaccessPath)) {
        ...
        this.context.logger.info(`${extractedRedirects.length} redirections extraites de ${htaccessPath}`);
        ...
        if (rewriteMatch) {
          ...
          if (flags.includes('R=') || flags.includes('R,')) {
            redirects.push({
              ...
              target: target.startsWith('http') ? target : (target.startsWith('/') ? target : '/' + target),
              type,
  ...
  private async generateRedirectsJson(): Promise<void> {
    ...
    await fs.writeJson(this.config.outputJsonPath, {
...
`;
      ...
      this.context.logger.info(`Nouvelle configuration Remix générée: ${this.config.remixConfigPath}`);
    } else {
...
${this.redirects.map(r => `    "${r.source}": { target: "${r.target}", type: ${r.type} },`).join('\n')}
  };
...
`;
    ...
    this.context.logger.info(`Middleware de redirection NestJS généré: ${this.config.nestJSConfigPath}`);
  }
...
`;
      ...
      this.context.logger.info('Pour valider les redirections, exécutez ce script lorsque votre serveur Remix est en cours d\'exécution.');
    } catch (error) {
      this.context.logger.error('Erreur lors de la génération du fichier de test des redirections:', error);
    }
  }
}
