...
export class MetaGenerator {
  ...
  private async extractMetaFromPhp(phpFilePath: string): Promise<MetaResult | null> {
    try {
      ...
      const canonicalMatch = phpContent.match(
      ...
      console.error(
        `Erreur lors de l'extraction des métadonnées du fichier PHP ${phpFilePath}:`,
        error
      );
      return null;
    }
  ...
  private generateMetaFileContent(route: string, metaData: MetaResult): string {
    const { title, description, keywords, canonical, ogTitle, ogDescription, ogImage, robots } =
      metaData;
...
export const meta: MetaFunction = ({ data }) => {
  ...
  return [
    ...
    ${
      keywords
        ? `{ name: "keywords", content: seo.keywords || ${JSON.stringify(keywords.join(', '))} },`
        : ''
    }
    ${robots ? `{ name: "robots", content: seo.robots || ${JSON.stringify(robots)} },` : ''}
    ${
      canonical
        ? `{ tagName: "link", rel: "canonical", href: seo.canonical || ${JSON.stringify(
            canonical
          )} },`
        : ''
    }
    ...
    ${
      ogDescription
        ? `{ property: "og:description", content: seo.ogDescription || ${JSON.stringify(
            ogDescription
          )} },`
        : ''
    }
