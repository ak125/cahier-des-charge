...
export class DataAgent extends BaseAgent {
  ...
  public async analyze(): Promise<void> {
    ...
    this.addSection('user-inputs', 'Entrées utilisateur', userInputs, 'sql');

    this.addSection('sql-queries', 'Appels SQL', sqlQueries, 'sql');

    this.addSection('outputs', 'Sorties', outputs, 'sql');
  }
  ...
  private analyzeSqlQueries(): string {
    ...
    if (sqlQueries.length > 0) {
      ...
      const hasJoins = sqlQueries.some(
        (q) => q.includes(' JOIN ') || q.includes(' INNER JOIN ') || q.includes(' LEFT JOIN ')
      );
      if (hasJoins) {
      ...
      const hasSubqueries = sqlQueries.some(
        (q) => q.includes('SELECT') && q.indexOf('SELECT') !== q.lastIndexOf('SELECT')
      );
      if (hasSubqueries) {
  ...
  private extractSqlQueries(content: string): string[] {
    ...
    const stringQueries = content.match(
      /["'](SELECT|INSERT|UPDATE|DELETE|CREATE|ALTER)[\s\S]*?["']/gi
    );
    if (stringQueries) {
      stringQueries.forEach((query) => {
        // Nettoyer la requête
        const cleanedQuery = query.slice(1, -1).replace(/\s+/g, ' ').trim();
  ...
  private extractSqlColumns(queries: string[]): string[] {
    ...
    queries.forEach((query) => {
      ...
      if (selectMatches && selectMatches[1] && selectMatches[1] !== '*') {
        const cols = selectMatches[1].split(',').map((col) =>
          col
            .trim()
            .split(/\s+AS\s+|\s+/i)[0]
            .trim()
    ...
    // Éliminer les doublons et trier
    return [...new Set(columns)].filter((col) => col !== '*' && !col.match(/^\d+$/)).sort();
  }
  ...
  private analyzeOutputs(): string {
    ...
    if (templateIncludes && templateIncludes.length > 0) {
      ...
      const templates = templateIncludes
        .map((include) => {
