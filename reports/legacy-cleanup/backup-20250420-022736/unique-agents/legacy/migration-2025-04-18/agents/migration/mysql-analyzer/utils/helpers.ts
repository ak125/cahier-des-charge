  ...
  Object.entries(schema.tables).forEach(([tableName, table]) => {
    ...
    table.foreignKeys.forEach((fk) => {
      if (!schema.tables[fk.referencedTable]) {
        errors.push(
          `La clé étrangère '${fk.name}' de la table '${tableName}' référence une table inexistante '${fk.referencedTable}'.`
        );
      } else {
        ...
        fk.referencedColumns.forEach((column) => {
          if (!schema.tables[fk.referencedTable].columns[column]) {
            errors.push(
              `La clé étrangère '${fk.name}' de la table '${tableName}' référence une colonne inexistante '${column}' dans la table '${fk.referencedTable}'.`
            );
          }
        });
      }
 ...
 */
export function printColoredMessage(
  message: string,
