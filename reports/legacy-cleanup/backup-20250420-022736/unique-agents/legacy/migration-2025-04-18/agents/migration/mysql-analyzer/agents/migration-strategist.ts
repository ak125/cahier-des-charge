...
class MigrationStrategist {
   ...
   */
  private extractTypeChanges(tableName: string, tableInfo: TableInfo): Array<{
    field: string;
    from: string;
    to: string;
    reason?: string;
    ...
    Object.entries(tableInfo.columns).forEach(([columnName, columnInfo]) => {
      if (columnInfo.originalType && columnInfo.suggestedPostgresType) {
        ...
        if (this.normalizeType(columnInfo.originalType) !== this.normalizeType(columnInfo.suggestedPostgresType)) {
          const change = {
            field: columnName,
   ...
   */
  private generatePostgresAdaptations(tableName: string, tableInfo: TableInfo): Array<{
    element: string;
    recommendation: string;
  }> {
    ...
    Object.entries(tableInfo.columns).forEach(([columnName, columnInfo]) => {
      ...
      // UUID
      if (columnInfo.originalType?.toLowerCase().includes('char(36)') || 
          (columnInfo.originalType?.toLowerCase().includes('varchar') && columnName.toLowerCase().includes('uuid'))) {
        adaptations.push({
          element: 'UUID',
      ...
      // TIMESTAMP/DATETIME
      if (columnInfo.originalType?.toLowerCase().includes('timestamp') || 
      ...
      if ((columnInfo.originalType?.toLowerCase().includes('float') || 
  ...
  private generatePrismaRecommendations(tableName: string, tableInfo: TableInfo): string[] {
    ...
    Object.entries(tableInfo.columns).forEach(([columnName, columnInfo]) => {
      if (columnInfo.isUnique || tableInfo.indexes?.some(
      ...
      if (columnName.endsWith('_id') && columnInfo.references) {
        ...
        recommendations.push(`Créer relation ${relationName}: ${referencedModel} pour ${columnName}`);
    ...
    recommendations.push('Ajouter @map("nom_colonne") si nécessaire pour maintenir la compatibilité');
    recommendations.push('Ajouter @@map("nom_table") à la fin du modèle pour maintenir la compatibilité');
  ...
  private generatePrismaModelCode(tableName: string, tableInfo: TableInfo): string {
    ...
    Object.entries(tableInfo.columns).forEach(([columnName, columnInfo]) => {
      ...
      if (columnInfo.isPrimary) {
        ...
        } else if (columnInfo.originalType?.toLowerCase().includes('char(36)') || columnName.toLowerCase().includes('uuid')) {
      ...
      // Valeur par défaut
      if (columnInfo.defaultValue !== undefined && !attributes.some(attr => attr.startsWith('@default'))) {
        if (columnInfo.defaultValue === 'CURRENT_TIMESTAMP') {
          ...
          let defaultValue = columnInfo.defaultValue.replace(/^['"]|['"]$/g, '');
      ...
      if (columnInfo.suggestedPostgresType) {
        let dbType = columnInfo.suggestedPostgresType;
      ...
      tableInfo.relations.forEach(relation => {
        if (relation.sourceTable === tableName) {
          ...
          if (relation.type.includes('ONE_TO_ONE')) {
            relations.push(`  ${relationName} ${targetModel}? @relation(fields: [${relation.sourceColumn}], references: [${relation.targetColumn}])`);
  ...
  private generateRelationalOptimizations(tableName: string, tableInfo: TableInfo): string[] {
    ...
    Object.entries(tableInfo.columns).forEach(([columnName, columnInfo]) => {
      if (columnName.endsWith('_id') && !tableInfo.indexes?.some(idx => idx.columns.includes(columnName))) {
    ...
    tableInfo.relations?.forEach(relation => {
      if (relation.sourceTable === tableName && relation.isImplicit) {
        optimizations.push(`Foreign key déclarée explicitement dans Prisma pour ${relation.sourceColumn} → ${relation.targetTable}.${relation.targetColumn}`);
      }
    });
    
    // Vérifier les tables qui pourraient être fusionnées (dénormalisation)
    if (tableInfo.tableType === 'BUSINESS_DETAIL' && tableInfo.columns && Object.keys(tableInfo.columns).length <= 5) {
      // Petites tables de détails qui pourraient être fusionnées avec la table principale
      const mainTable = tableInfo.relations?.find(rel => rel.sourceTable === tableName)?.targetTable;
      if (mainTable) {
        optimizations.push(`Fusionner ${tableName} avec ${mainTable} si les champs ne sont pas trop nombreux`);
      }
    }
    
    // Vérifier les champs JSON potentiels
    Object.entries(tableInfo.columns).forEach(([columnName, columnInfo]) => {
      if (columnInfo.originalType?.toLowerCase().includes('text') && 
          (columnName.includes('config') || columnName.includes('settings') || columnName.includes('data') || columnName.includes('json'))) {
        optimizations.push(`Considérer l'utilisation du type JSONB pour ${columnName} si contient des données structurées`);
      }
    });
  ...
  private generatePostMigrationInstructions(tableName: string, tableInfo: TableInfo): string[] {
    ...
    if (this.impactGraph[tableName]?.impactedFiles.length > 0) {
      instructions.push(`⚠️ Mettre à jour les appels dans ${this.impactGraph[tableName].impactedFiles.length} fichiers liés`);
    }
   ...
   */
  private determineFunctionalSummary(tableName: string, tableInfo: TableInfo): {
    role: string;
    functionalArea: string;
  } {
    ...
    if (tableInfo.tableType === 'BUSINESS_CORE') {
      ...
      } else if (tableName.includes('product') || tableName.includes('item') || tableName.includes('catalog')) {
        functionalArea = "catalogue / produits";
      } else if (tableName.includes('order') || tableName.includes('invoice') || tableName.includes('payment')) {
        functionalArea = "commandes / paiements";
      } else if (tableName.includes('post') || tableName.includes('comment') || tableName.includes('content')) {
      ...
      if (mainTable) {
        ...
        if (mainTable.includes('user') || mainTable.includes('customer') || mainTable.includes('account')) {
  ...
  public exportToMarkdown(outputPath: string): void {
    ...
    // Créer le sommaire
    Object.keys(this.plans).sort().forEach(tableName => {
      const plan = this.plans[tableName];
      const readyStatus = plan.migrationReady ? '✅' : '⏳';
      markdown += `- [${readyStatus} ${tableName}](#${tableName.toLowerCase()}) - Score: ${plan.migrationReadyScore}%\n`;
    });
...
if (require.main === module) {
  ...
  try {
    const strategist = new MigrationStrategist(schemaMapPath, conversionResultPath, impactGraphPath);
