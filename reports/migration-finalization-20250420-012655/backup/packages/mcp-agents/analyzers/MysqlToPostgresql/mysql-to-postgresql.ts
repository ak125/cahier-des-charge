...
const DEFAULT_TYPE_MAPPINGS: TypeMapping[] = [
  ...
  {
    mysql: 'tinyint\\((\\d+)\\)(?: unsigned)?',
    postgresql: 'smallint',
    notes: 'Conversion de tinyint(N) en smallint',
  },
  {
    mysql: 'smallint(?: unsigned)?',
    postgresql: 'integer',
    notes: 'Conversion de smallint en integer',
  },
  {
    mysql: 'mediumint(?: unsigned)?',
    postgresql: 'integer',
    notes: 'Conversion de mediumint en integer',
  },
  { mysql: 'int(?: unsigned)?', postgresql: 'integer', notes: 'Conversion de int en integer' },
  {
    mysql: 'integer(?: unsigned)?',
    postgresql: 'integer',
    notes: 'Conversion de integer en integer',
  },
  ...
  { mysql: 'float', postgresql: 'real', notes: 'Conversion de float en real' },
  {
    ...
    notes: 'Conversion de double en double precision',
  },
  {
    mysql: 'decimal\\((\\d+),(\\d+)\\)',
    postgresql: 'decimal($1,$2)',
    notes: 'Conversion de decimal en decimal',
  },
  { mysql: 'char\\((\\d+)\\)', postgresql: 'char($1)', notes: 'Conversion de char en char' },
  {
    mysql: 'varchar\\((\\d+)\\)',
    postgresql: 'varchar($1)',
    notes: 'Conversion de varchar en varchar',
  },
  ...
  { mysql: 'time(?:\\(\\d+\\))?', postgresql: 'time', notes: 'Conversion de time en time' },
  {
    ...
    notes: 'Conversion de datetime en timestamp',
  },
  {
    mysql: 'timestamp(?:\\(\\d+\\))?',
    postgresql: 'timestamp',
    notes: 'Conversion de timestamp en timestamp',
  },
  { mysql: 'year', postgresql: 'integer', notes: 'Conversion de year en integer' },
  {
    mysql: 'enum\\((.+?)\\)',
    postgresql: 'text CHECK ($column IN ($1))',
    notes: 'Conversion de enum en text avec contrainte CHECK',
  },
  ...
  { mysql: 'multipoint', postgresql: 'point[]', notes: 'Conversion de multipoint en point[]' },
  {
    ...
    postgresql: 'path[]',
    notes: 'Conversion de multilinestring en path[]',
  },
  {
    mysql: 'multipolygon',
    postgresql: 'polygon[]',
    notes: 'Conversion de multipolygon en polygon[]',
  },
  {
    mysql: 'geometrycollection',
    postgresql: 'geometry',
    notes: 'Conversion de geometrycollection en geometry (extension PostGIS nécessaire)',
  },
...
if (options.map && fs.existsSync(options.map)) {
  try {
    ...
    console.log(
      chalk.blue(
        `📝 Chargement de ${customTypeMappings.length} mappings personnalisés depuis ${options.map}`
      )
    );
  } catch (error) {
    console.error(
      chalk.red(`❌ Erreur lors du chargement du fichier de mapping: ${error.message}`)
    );
    process.exit(1);
  }
}
...
async function main() {
  ...
  console.log(
    `Tables: ${report.tables.converted}/${report.tables.total} converties (${report.tables.warnings} avertissements, ${report.tables.errors} erreurs)`
  );
  console.log(
    `Colonnes: ${report.columns.converted}/${report.columns.total} converties (${report.columns.warnings} avertissements, ${report.columns.errors} erreurs)`
  );
  ...
  while ((match = createTableRegex.exec(mysqlDump)) !== null) {
    try {
      ...
      for (const definition of definitions) {
        ...
        // Index
        if (
          trimmedDef.toUpperCase().startsWith('KEY') ||
      ...
      if (options.verbose) {
        console.log(
          chalk.blue(`  Table extraite: ${tableName} (${tableDefinition.columns.length} colonnes)`)
        );
      }
    } catch (error) {
...
function extractForeignKey(definition: string): ForeignKeyDefinition | null {
  ...
  const referencedColumns = constraintMatch[4]
    .split(',')
 ...
 */
function convertTableDefinitions(
  ...
  const convertedTables = tables.map((table) => {
    try {
      ...
      report.tables.errors++;
      report.errors.push(
        `Erreur lors de la conversion de la table ${table.name}: ${error.message}`
      );
 ...
 */
function convertColumnDefinition(
...
function generatePostgreSQLDump(tables: TableDefinition[], report: MigrationReport): string {
  ...
  for (const table of tables) {
    ...
    for (const column of table.columns) {
      if (column.comment) {
        postgresqlDump += `COMMENT ON COLUMN "${table.name}"."${
          column.name
    ...
    if (table.comment) {
      postgresqlDump += `COMMENT ON TABLE "${table.name}" IS '${table.comment.replace(
        /'/g,
        "''"
      )}';\n`;
    }
