...
export const generateMetadata = (product: ProductData) => {
  ...
  const meta: MetaFunction = () => {
    const title = seoData?.metadata.title || 
      `Pneu ${product.brand.name} ${product.name} ${product.size} - Mon Site`;
    
    const description = seoData?.metadata.description || 
    ...
    const canonical = seoData?.metadata.canonical || 
    ...
    return {
      ...
      "og:image": seoData?.metadata.ogImage || `https://www.mon-site.fr/images/products/${product.brand.slug}-${product.slug}.jpg`,
      "og:type": "product",
      ...
      "twitter:image": seoData?.metadata.twitterImage || `https://www.mon-site.fr/images/products/${product.brand.slug}-${product.slug}.jpg`,
      
      // Autres métadonnées utiles
      "keywords": seoData?.metadata.keywords || `pneu, ${product.brand.name}, ${product.name}, ${product.size}`,
...
export const generateLinks: LinksFunction = (product: ProductData) => {
  ...
  return () => {
    return [
      ...
      { 
        rel: "canonical", 
        href: seoData?.metadata.canonical || `https://www.mon-site.fr/pneus/${product.brand.slug}/${product.slug}`
      },
