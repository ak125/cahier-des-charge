...
import { AgentMetadata, AgentType, McpAgent } from './interfaces';

class AgentRegistry implements BaseAgent, BusinessAgent, AnalyzerAgent {
  ...
  public getAgentsByType(type: AgentType): McpAgent[] {
    return Array.from(this.agents.values()).filter((agent) => agent.metadata.type === type);
  }
