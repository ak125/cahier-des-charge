...
export interface OrchestratorBridgeConfig {
 ...
 Dotn8NWebhookUrl: string;
  queueNames: string[];
  statusFilePath?: string;
  temporalUIAddress?: string;
 Dotn8NUIAddress?: string;
...
/**
 * OrchestratorBridge - Pont d'intégration entre Temporal.io, BullMQ etDotn8N
 * 
 ...
 * -Dotn8N pour les triggers externes et webhooks
 */
export class OrchestratorBridge implements BaseAgent, OrchestrationAgent, BaseAgent, OrchestrationAgent , CoordinationAgent{
  ...
  privateDotn8NWebhookUrl: string;
  ...
  constructor(
    ...
    thisDotn8NWebhookUrl = configDotn8NWebhookUrl;
  ...
  async initialize() {
    ...
    for (const queueName of this.config.queueNames) {
      ...
      // Événements BullMQ pour synchro avec Temporal etDotn8N
  ...
  /**
   * Envoie une notification àDotn8N via un webhook
   */
  private async notifyN8N(event: string, payload: any) {
    if (!thisDotn8NWebhookUrl) return;
    
    try {
      await axios.post(`${thisDotn8NWebhookUrl}/${event}`, payload, {
        headers: {
          'Content-Type': 'application/json'
        }
      });
      this.logger.log(`📤 NotificationDotn8N envoyée: ${event}`);
    } catch (error) {
      this.logger.error(`Erreur lors de la notificationDotn8N (${event})`, error);
    }
  }
  ...
  async listenToN8NSignals(webHandler: any, port: number = 3456) {
    ...
    webHandler.post('Dotn8N-signal/:signalType', async (req: any, res: any) => {
      ...
      this.logger.log(`📥 SignalDotn8N reçu: ${signalType}`);
      ...
      try {
        ...
        this.logger.error(`Erreur lors du traitement du signalDotn8N ${signalType}`, error);
    ...
    webHandler.listen(port, () => {
      this.logger.log(`🚀 Serveur d'écoute des signauxDotn8N démarré sur le port ${port}`);
    });
  }
  
  /**
   * Crée et démarre un tableau de bord unifié pour monitorer Temporal, BullMQ etDotn8N
          ...
          <body>
            ...
            <div class="dashboard-container">
              ...
              <div class="dashboard-card">
                ...
                <a class="button" href="${this.configDotn8NUIAddress || 'http://localhost:5678'}" target="_blank">OuvrirDotn8N</a>
              </div>
            </div>
