...
export class SQLParser implements BaseAgent, BusinessAgent, BaseAgent, BusinessAgent , AnalyzerAgent{
  /**
   ...
   */
  private extractColumnDefinition(definition: string, tableInfo: TableInfo, position: number): void {
  ...
  private extractIndex(definition: string, tableInfo: TableInfo): void {
    ...
    const indexMatch = definition.match(/(?:UNIQUE\s+)?(?:KEY|INDEX)\s+(?:`|")?([^`"\s(]+)(?:`|")?\s*\(([^)]+)\)/i);
   ...
   */
  private extractForeignKey(definition: string, tableInfo: TableInfo, constraintName?: string): void {
  ...
  private extractAlterTables(sqlContent: string, schema: MySQLSchema): void {
    ...
    const alterTableRegex = /ALTER\s+TABLE\s+(?:`|")?([^`"\s]+)(?:`|")?\s+ADD\s+(CONSTRAINT\s+(?:`|")?([^`"\s]+)(?:`|")?\s+)?FOREIGN\s+KEY\s*\(([^)]+)\)\s+REFERENCES\s+(?:`|")?([^`"\s(]+)(?:`|")?\s*\(([^)]+)\)([^;]*);/gim;
    
    let match;
    while ((match = alterTableRegex.exec(sqlContent)) !== null) {
      const tableName = match[1];
      const constraintName = match[3] || `fk_${tableName}_${schema.tables[tableName]?.foreignKeys.length + 1 || 1}`;
