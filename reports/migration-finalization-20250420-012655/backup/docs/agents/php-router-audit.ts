...
export class PhpRouterAudit implements BaseAgent, BusinessAgent, BaseAgent, BusinessAgent, MCPAgent {
  ...
  async process(context: MCPContext): Promise<any> {
    const { htaccessPath, baseUrl, outputPath, routingMapPath, checkActive = true } = context.inputs;
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          unmappedRoutes: phpRoutes.length - existingRoutes.filter(r => phpRoutes.includes(r.from)).length,
  ...
  private async checkActiveRoutes(routes: string[], baseUrl: string): Promise<PhpRouteAuditResult[]> {
  ...
  private generateAuditReport(
    ...
    phpRoutes.forEach(route => {
      ...
      const mappingStatus = existingRoute ? `✅ ${existingRoute.to} (${existingRoute.type})` : '❌ Non mappé';
      ...
      if (auditResult) {
        status = auditResult.status === 'active' ? '✅ Actif' : 
                auditResult.status === 'inactive' ? '❌ Inactif' : '⚠️ Erreur';
