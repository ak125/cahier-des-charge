...
export class TableClassifier
  implements BaseAgent, BusinessAgent, BaseAgent, BusinessAgent, AnalyzerAgent
{
  ...
  private classificationRules: ClassificationRule[] = [
    ...
    {
      pattern:
        /^(log|logs|audit|history|archive|backup|cache|temp|tmp|queue|job|batch|lock|session)s?$/i,
  ...
  private structureRules: Array<
    (table: TableInfo) => { match: boolean; type: TableType; description: string }
  > = [
    ...
    (table) => {
      ...
      if (columnCount <= 4 && foreignKeyCount >= 2) {
        return {
          ...
          type: TableType.JUNCTION,
          description:
            'Table de jointure identifiée par sa structure (peu de colonnes, plusieurs clés étrangères)',
        };
      }
    ...
    (table) => {
      ...
      if (timestampColumns.length >= 3) {
        return {
          ...
          type: TableType.TECHNICAL,
          description:
            'Table technique identifiée par la présence de nombreux champs de date/heure',
        };
      }
  ...
  private adjustClassificationBasedOnRelations(schema: MySQLSchema): void {
    ...
    Object.entries(schema.tables).forEach(([tableName, table]) => {
      ...
      if (table.tableType === TableType.UNKNOWN || table.tableType === TableType.BUSINESS_DETAIL) {
        ...
        if (hasFkToMetadata) {
          table.tableType = TableType.BUSINESS_DETAIL;
          table.classificationReason =
            'Table de détail métier identifiée par relation avec table de métadonnées';
        }
      }
    });
  }
