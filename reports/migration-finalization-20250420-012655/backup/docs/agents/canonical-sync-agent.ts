...
export class CanonicalSyncAgent implements BaseAgent, CoordinationAgent, MCPAgent, BusinessAgent {
  ...
  async process(context: MCPContext): Promise<any> {
    ...
    try {
      ...
      const localEntries: CanonicalMapping[] = Object.entries(localMappings).map(
      ...
      if (direction === 'both' || direction === 'pull') {
        ...
        if (error) {
          throw new Error(
            `Erreur lors de la récupération des mappings depuis Supabase: ${error.message}`
          );
        }
      ...
      if (direction === 'both' || direction === 'push') {
        ...
        const { error } = await supabase
          .from('seo_canonicals')
          .upsert(localEntries, { onConflict: 'source_url' });

        if (error) {
          throw new Error(
            `Erreur lors de la mise à jour des mappings dans Supabase: ${error.message}`
          );
        }
      }
