...
export class SeoChecker implements BaseAgent, BusinessAgent, MCPAgent {
  ...
  description =
    'Vérifie les problèmes SEO dans les routes migrées et génère des balises canoniques';
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          totalRecommendations: seoResults.reduce(
    ...
    for (const result of results) {
      ...
      if (chains.length > 0) {
        ...
        if (chains.some((chain) => chain.length > 2)) {
          result.issues.push(
            'Chaîne de redirection trop longue (>2) - impact sur les performances et le SEO'
          );
        }
      }
    }
  ...
  private findRedirectChain(
    url: string,
    redirectGraph: Record<string, string>,
    visited: string[]
  ): string[][] {
  ...
  private fixSeoIssues(results: SeoCheckResult[], routesDir: string): void {
    for (const result of results) {
      ...
      if (!result.hasCanonical) {
        ...
        const metaFunctionMatch = content.match(
          /export\s+const\s+meta[^{]*{[^]*?return\s*\[[^]*?\]/s
        );
