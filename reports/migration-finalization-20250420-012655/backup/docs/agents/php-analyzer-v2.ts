...
import { Agent, AgentContext, AgentResponse } from '../../appsDoDotmcp-server/src/types/agent';
...
class PHPAnalyzerV2 implements Agent, BaseAgent, BusinessAgent, AnalyzerAgent {
  id = 'php-analyzer-v2';
  ...
  private async analyzePhpCode(sourceDir: string, options: any): Promise<PHPAnalysisResult> {
    ...
    if (result.statistics.totalFunctions + result.statistics.totalMethods > 0) {
      result.statistics.avgComplexity =
        totalComplexity / (result.statistics.totalFunctions + result.statistics.totalMethods);
    }
  ...
  private async findPhpFiles(dir: string): Promise<string[]> {
    try {
      const { stdout } = await execAsync(
        `find ${dir} -type f -name "*.php" | grep -v /vendor/ | grep -v /node_modules/`
      );
  ...
  private async analyzePhpFile(filePath: string): Promise<PHPAnalysisResult['files'][0]> {
    ...
    try {
      ...
      // Extraire les requêtes SQL
      const sqlRegex =
        /(?:mysql_query|mysqli_query|->query)\s*\(\s*["']([^"']+(?:SELECT|INSERT|UPDATE|DELETE)[^"']+)["']/gi;
  ...
  private extractMethods(
    classBody: string
  ): PHPAnalysisResult['files'][0]['classes'][0]['methods'] {
  ...
  private extractProperties(
    classBody: string
  ): PHPAnalysisResult['files'][0]['classes'][0]['properties'] {
  ...
  private extractTableSchema(
    query: { query: string; tables: string[] },
    schema: PHPAnalysisResult['databaseSchema']
  ) {
    ...
    if (q.includes('create table')) {
      const tableMatch = q.match(
        /create\s+table\s+(?:if\s+not\s+exists\s+)?([a-z0-9_`]+)\s*\((.*)\)/is
      );
      ...
      if (tableMatch) {
        ...
        const columnMatches = columnsDef.matchAll(
          /([a-z0-9_`]+)\s+([a-z0-9_()]+)(?:\s+not\s+null)?(?:\s+(?:primary\s+key|unique))?/gi
        );
        ...
        const fkMatches = columnsDef.matchAll(
          /foreign\s+key\s+\(([^)]+)\)\s+references\s+([a-z0-9_`]+)\s*\(([^)]+)\)/gi
        );
  ...
  private detectEntityRelationships(
    content: string,
    filePath: string
  ): PHPAnalysisResult['files'][0]['entityRelationships'] {
    ...
    const propertyPattern =
      /(?:private|protected|public)\s+(\$\w+)(?:\s*=\s*[^;]*)?;(?:\s*\/\/\s*(\w+))?/g;
    ...
    while ((match = propertyPattern.exec(content)) !== null) {
      ...
      if (
        ...
        const entityName = propertyName.replace('$', '').replace('Id', '').replace('Reference', '');
  ...
  private async extractRoutes(sourceDir: string, result: PHPAnalysisResult) {
    ...
    for (const pattern of routePatterns) {
      try {
        const { stdout } = await execAsync(
          `find ${sourceDir} -path "${pattern}" | grep -v /vendor/ | grep -v /node_modules/`
        );
  ...
  private assessMigrationComplexity(
  ...
  ): PHPAnalysisResult['migrationComplexity'] {
    ...
    // 2. Dépendances
    const dependencyImpact = Math.min(
      10,
      (analysis.dependencies.internal.length * 0.3 + analysis.dependencies.external.length * 0.7) /
        5
    );
    ...
    const dbComplexity =
      analysis.databaseSchema.tables.length > 0
    ...
    const cyclomaticImpact =
      analysis.statistics.avgComplexity > 0
        ? Math.min(10, analysis.statistics.avgComplexity * 0.5)
        : 5;
    ...
    analysis.files.forEach((file) => {
      ...
      const register_globals = file.sqlQueries.filter(
        (q) => q.query.toLowerCase().includes('_GET') || q.query.toLowerCase().includes('_POST')
      ).length;
    ...
    const weightedSum = complexityFactors.reduce(
      (sum, factor, i) => sum + factor.impact * weights[i],
      0
    );
