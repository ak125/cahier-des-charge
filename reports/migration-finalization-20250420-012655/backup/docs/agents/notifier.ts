 ...
 */
export class MCPNotifier implements BaseAgent, OrchestrationAgent , BusinessAgent{
  ...
  private async updateManifest(event: NotificationEvent) {
    try {
      ...
      if (event.type.startsWith('qa:')) {
        ...
        const existingStepIndex = migration.verificationSteps.findIndex(s => s.name === 'QaAnalyzer');
