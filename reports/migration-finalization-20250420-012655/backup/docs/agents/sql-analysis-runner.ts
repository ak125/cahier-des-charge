...
import { MigrationStrategist } from '../MysqlAnalyzer/agents/MigrationStrategist';
import { analyzeSchema } from '../MysqlAnalyzer/core/SchemaAnalyzer';
import { TypeConverter } from '../MysqlAnalyzer/core/TypeConverter';
import { SchemaImpactAnalyzer } from '../MysqlAnalyzer/core/impact-analyzer';
...
`);
  ...
  private async configureEnvironment(): Promise<void> {
    ...
    // Vérifier l'intégrationDotn8N
    try {
      constDotn8NConfigPath = path.resolve('./configDotn8N-audit-analyzer-workflow.json');
      if (fs.existsSyncDotn8NConfigPath)) {
        console.log(`${colors.fg.green}✓ ConfigurationDotn8N détectée, l'intégration est possible${colors.reset}`);
        this.configDotn8NIntegration = true;
      }
    } catch (error) {
      // Ignorer l'erreur, l'intégrationDotn8N est optionnelle
    }
  ...
  private async promptForOptions(): Promise<void> {
    ...
    if (this.configDotn8NIntegration) {
      constDotn8NIntegration = await this.prompt('Activer l\'intégrationDotn8N? (o/N): ', 'N');
      this.configDotn8NIntegration =Dotn8NIntegration.toLowerCase() === 'o';
    }
...
🚀 Pour intégrer ce plan dansDotn8N:
   ${this.configDotn8NIntegration 
       ? `${colors.fg.greenDotn8N workflow importé! Accédez à http://localhost:5678/workflow${colors.reset}`
       : `Exécutez la commande: ${colors.fg.yellowDotn8N import --input=./config/migration-workflowDotn8N.json${colors.reset}`}
