...
import canonicalConfig from '../config/seo-canonicals.json';
 ...
 */
export function generateCanonicalUrl(pathOrSlug: string, options: CanonicalOptions = {}): string {
  ...
  if (canonicalConfig.customMapping?.[pathOrSlug]) {
...
export function transformPhpToRemixUrl(phpUrl: string): string {
  ...
  if (canonicalConfig.customMapping?.[phpUrl]) {
 ...
 */
export function generateCanonicalMeta(
  url?: string,
 ...
 */
export async function syncCanonicalMappings(_source: string): Promise<Record<string, string>> {
