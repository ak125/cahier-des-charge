...
export async function loadDebtReport(): Promise<Record<string, DebtData>> {
  try {
    ...
    const schemaPath = path.resolve(
      process.cwd(),
      'agents/migration/examples/mysql_schema_map.json'
    );
    ...
    if (fs.existsSync(schemaPath)) {
      ...
      Object.entries(schemaMap.tables).forEach(([tableName, tableData]: [string, any]) => {
        ...
        const hasNonStandardColumns = tableData.columns?.some(
          (col: any) =>
            col.type.includes('BLOB') || col.type.includes('TEXT') || col.type.includes('ENUM')
        );
      ...
      COMMANDES: {
        ...
        metrics: [
          ...
          { name: 'Complexité', value: 75, description: 'Table avec 15 colonnes' },
          {
            name: 'Types non standards',
            value: 100,
            description: 'Utilise des champs TEXT pour les commentaires',
          },
      ...
      PRODUITS: {
        ...
        metrics: [
          ...
          { name: 'Complexité', value: 60, description: 'Table avec 12 colonnes' },
          {
            name: 'Types non standards',
            value: 100,
            description: 'Utilise ENUM pour le statut du produit',
          },
          { name: 'Relations', value: 40, description: "2 relations avec d'autres tables" },
        ],
        suggestions: ['Remplacer ENUM par une table de référence ou un type CHECK dans PostgreSQL'],
      },
    };
  } catch (error) {
