...
async function syncTables(supabase, schemaMap) {
  ...
  for (let i = 0; i < tables.length; i += batchSize) {
    ...
    const { error } = await supabase.from('sql_tables').upsert(batch, { onConflict: 'table_name' });

    if (error) {
      console.error(
        `Erreur lors de la synchronisation du lot de tables ${i}-${i + batch.length}:`,
        error
      );
    }
  }
...
async function syncMigrationPlan(supabase, migrationPlan) {
  ...
  console.log(
    `Synchronisation du statut de migration pour ${Object.keys(migrationPlan).length} tables...`
  );
  ...
  for (let i = 0; i < migrationStatus.length; i += batchSize) {
    ...
    if (error) {
      console.error(
        `Erreur lors de la synchronisation du lot de statuts ${i}-${i + batch.length}:`,
        error
      );
    }
  }
...
async function syncDebtReport(supabase, debtReport) {
  ...
  console.log(
    `Synchronisation du rapport de dette pour ${Object.keys(debtReport).length} tables...`
  );
  ...
  for (let i = 0; i < debtEntries.length; i += batchSize) {
    ...
    if (error) {
      console.error(
        `Erreur lors de la synchronisation du lot de rapports de dette ${i}-${i + batch.length}:`,
        error
      );
    }
  }
...
async function syncRelations(supabase, relationGraph) {
  ...
  const { error: deleteError } = await supabase.from('table_relations').delete().neq('id', 0); // Condition pour tout supprimer
  ...
  const relations = relationGraph.relations.map((relation, _index) => ({
  ...
  for (let i = 0; i < relations.length; i += batchSize) {
    ...
    const { error } = await supabase.from('table_relations').insert(batch);

    if (error) {
      console.error(
        `Erreur lors de la synchronisation du lot de relations ${i}-${i + batch.length}:`,
        error
      );
    }
  }
