import { Relation } from '~/types/migration';
 ...
 */
export function buildMermaidGraph(relations: Relation[], highlightedTable?: string | null): string {
  ...
  if (
    highlightedTable &&
    !relations.some((rel) => rel.source === highlightedTable || rel.target === highlightedTable)
  ) {
 ...
 */
export function filterRelations(relations: Relation[], includedTables: string[]): Relation[] {
  ...
  return relations.filter((rel) => includedSet.has(rel.source) && includedSet.has(rel.target));
}
