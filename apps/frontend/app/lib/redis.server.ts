...
import { McpJob } from './supabase.server';
...
// Types pour Redis
export type McpQueueName = 'mcp:php-analyzer' | 'mcp:verification' | 'mcp:commit';
...
export const mcpRedisApi = {
  ...
  async runMcpJob(filename: string, options: { dryRun?: boolean; jobId?: string } = {}) {
    ...
    return this.addJobToQueue('mcp:php-analyzer', jobData);
  },
  ...
  async subscribeToJobUpdates(callback: (channel: string, message: string) => void) {
    ...
    await redis.subscribe('mcp:php-analyzer:new', callback);
