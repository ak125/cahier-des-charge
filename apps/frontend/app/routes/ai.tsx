...
import { z } from 'zod';
import {
  LocalAiServiceConfig,
  createLocalAiService,
} from '../../../../packages/shared/ai/local-ai.service';
...
export const meta: MetaFunction = () => {
  return [
    ...
    {
      name: 'description',
      content: "Assistant d'IA pour l'analyse et la transformation de code PHP vers Remix",
    },
  ];
};
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    switch (validatedData.action) {
      ...
      case 'transform':
        result = await aiService.transformPhpToRemix(validatedData.prompt, validatedData.context, {
          systemPrompt: validatedData.systemPrompt,
          temperature: validatedData.temperature,
        });
        break;
      default:
        result = await aiService.generate(validatedData.prompt, {
          systemPrompt: validatedData.systemPrompt,
          temperature: validatedData.temperature,
        });
    }
            ...
            <textarea
              ...
              placeholder={
                action === 'generate'
                  ...
                  : action === 'analyze'
                    ? 'Collez le code PHP à analyser ici...'
