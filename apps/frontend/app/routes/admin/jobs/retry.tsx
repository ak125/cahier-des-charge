...
export const action: ActionFunction = async ({ request }) => {
  ...
  if (!jobId || !filename) {
    return json({
      success: false,
      error: 'Job ID et nom de fichier requis'
    }, { status: 400 });
  }
  ...
  try {
    ...
    if (!statusData.files[filename]) {
      return json({
        success: false,
        error: 'Fichier non trouvé dans status.json'
      }, { status: 404 });
    }
    ...
    statusData.files[filename].error = undefined;
    ...
    let jobsFileExists = false;
    try {
      await fs.access(jobsPath);
      jobsFileExists = true;
    } catch (error) {
      jobsFileExists = false;
    }
