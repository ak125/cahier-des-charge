...
export const loader: LoaderFunction = async () => {
  ...
  try {
    ...
    if (fs.existsSync(logPath)) {
      ...
      latestErrors = logContent.split('\n').filter(Boolean).slice(-20); // Obtenir les 20 dernières erreurs
    }
  } catch (error) {
...
function calculatePerformanceMetrics(statusData: StatusData) {
  ...
  const bottlenecks = Object.entries(agentStats)
    .map(([agent, stats]) => ({
      agent,
...
export default function JobsDashboard() {
  ...
  const jobFiles = Object.entries(statusData.files).map(([filename, data]) => ({
    filename,
    ...data,
  }));
  ...
  const filteredJobs =
    filter === 'all' ? jobFiles : jobFiles.filter((job) => job.status === filter);
        ...
        >
          <svg
            ...
            className={`h-5 w-5 mr-1 transition-transform ${
              showMetrics ? 'transform rotate-90' : ''
            }`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
                ...
                <ul className="mt-2">
                  {metrics.bottlenecks.map((bottleneck, index) => (
                    <li key={index} className="text-sm">
                      ...
                      {bottleneck.errorRate.toFixed(1)}% d'erreurs, {bottleneck.avgTime.toFixed(1)}s
                      en moyenne
                    </li>
                  ))}
                </ul>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-8">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md ${
              filter === 'done' ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-800'
            }`}
          >
            Migrés
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-md ${
              filter === 'pending' ? 'bg-yellow-500 text-white' : 'bg-gray-200 text-gray-800'
            }`}
          >
            En attente
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-md ${
              filter === 'invalid' ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-800'
            }`}
          >
            Erreurs
          </button>
      ...
      <div className="mt-12 bg-white rounded-lg shadow p-4">
        ...
        <div className="bg-gray-100 rounded-md p-4 max-h-96 overflow-y-auto">
          {latestErrors.length > 0 ? (
            <pre className="text-xs font-mono whitespace-pre-wrap">{latestErrors.join('\n')}</pre>
          ) : (
