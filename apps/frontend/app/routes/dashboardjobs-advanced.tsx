  ...
  useEffect(() => {
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (Math.random() < 0.2) {
          // 20% de chances de revalider (pour limiter les appels API)
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
        <div className="flex space-x-2">
          ...
          <button
            ...
            className={`px-3 py-2 rounded-md ${
              activeTab === 'processing' ? 'bg-blue-500 text-white' : 'bg-gray-200'
            }`}
          >
            En cours
          </button>
          <button
            ...
            className={`px-3 py-2 rounded-md ${
              activeTab === 'done' ? 'bg-blue-500 text-white' : 'bg-gray-200'
            }`}
          >
            Terminés
          </button>
          <button
            ...
            className={`px-3 py-2 rounded-md ${
              activeTab === 'failed' ? 'bg-blue-500 text-white' : 'bg-gray-200'
            }`}
          >
            Échoués
          </button>
            ...
            <tbody>
              {filteredJobs.map((job, index) => (
                <tr key={job.jobId} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                  ...
                  <td
                    className="py-2 px-4 border-b text-sm truncate max-w-[200px]"
                    title={job.filePath}
                  >
                    {job.filePath}
                  </td>
                  ...
                  <td className="py-2 px-4 border-b">
                    <button className="text-blue-500 hover:text-blue-700 mr-2">Détails</button>
                    {job.status === 'failed' && (
                      <button className="text-orange-500 hover:text-orange-700">Réessayer</button>
                    )}
                  </td>
                </tr>
              ))}
