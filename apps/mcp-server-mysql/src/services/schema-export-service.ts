...
import { ColumnInfo, ForeignKeyInfo, TableInfo, TableService } from './table-service';
...
export class SchemaExportService {
   ...
   */
  async exportSchemaMap(outputPath = 'mysql_schema_map.json'): Promise<SchemaMap> {
    try {
  ...
  private convertTableToSchemaInfo(
    ...
    const primaryKey = tableInfo.columns.filter((col) => col.key === 'PRI').map((col) => col.name);
  ...
  async suggestPrismaModel(tableName: string): Promise<string> {
    try {
      ...
      const inverseRelations = allRelations.filter((rel) => rel.referencedTable === tableName);
      ...
      for (const fk of foreignKeys) {
        ...
        // Déterminer le type de relation (1:1, 1:n, n:m)
        const _relationType = sourceColumn?.unique ? '1:1' : 'n:1';

        prismaModel += `  ${fieldName} ${targetModel} @relation(fields: [${this.camelCase(
          fk.sourceColumn
        )}], references: [${this.camelCase(fk.referencedColumn)}]`;
      ...
      for (const rel of inverseRelations) {
        ...
        const isOneToOne = reverseColumn?.unique;
      ...
      throw new Error(
        `Erreur lors de la génération du modèle Prisma pour '${tableName}': ${error.message}`
      );
    }
  }
   ...
   */
  async generatePrismaFile(outputPath = 'schema.prisma'): Promise<void> {
    try {
      if (!this.dbService.isConnectedToDatabase()) {
        await this.dbService.connect();
      }
...
`;
  ...
  async schemaQualityCheck(): Promise<string> {
    try {
      ...
      report +=
        '- Préférer les types natifs PostgreSQL comme `JSONB` au lieu de `TEXT` pour les données JSON\n';
      report +=
        '- Utiliser les types `TIMESTAMP WITH TIME ZONE` pour les dates avec fuseaux horaires\n';
  ...
  async schemaMigrationDiff(
    ...
    outputPath = 'schema_migration_diff.json'
  ): Promise<void> {
    try {
  ...
  private async checkTableIssues(tableInfo: TableInfo): Promise<string[]> {
    ...
    if (nullableColumns.length > tableInfo.columns.length / 2) {
      issues.push(
        'Plus de 50% des colonnes peuvent être NULL, ce qui pourrait indiquer un problème de conception'
      );
    }
    ...
    for (const column of tableInfo.columns) {
      ...
      if (
        ['BLOB', 'TEXT', 'MEDIUMBLOB', 'MEDIUMTEXT', 'LONGBLOB', 'LONGTEXT'].includes(
          column.type.toUpperCase()
        )
      ) {
        issues.push(
          `La colonne '${column.name}' utilise un type ${column.type} qui peut être inefficace pour les requêtes`
        );
      }
      ...
      if (
        ...
        column.name.match(/prix|montant|tarif|cost|price|amount/i)
      ) {
        issues.push(
          `La colonne '${column.name}' utilise FLOAT, mais DECIMAL serait plus approprié pour les valeurs monétaires`
        );
      }

      // Utilisation de CHAR au lieu de VARCHAR
      if (
        column.type.toUpperCase().startsWith('CHAR(') &&
        !column.name.match(/code|abbr|iso|uuid/i)
      ) {
        issues.push(
          `La colonne '${column.name}' utilise CHAR de taille fixe, ce qui peut gaspiller de l'espace`
        );
      }
    }
   ...
   */
  private async identifyPotentialForeignKeys(): Promise<
    Array<{
      sourceTable: string;
      sourceColumn: string;
      targetTable: string;
  ...
  > {
    ...
    for (const sourceTable of tables) {
      ...
      for (const column of columns) {
        ...
        if (
          column.key === 'PRI' ||
          existingForeignKeys.some(
            (fk) => fk.sourceTable === sourceTable && fk.sourceColumn === column.name
          )
        ) {
          continue;
        }
   ...
   */
  private async findColumnInTable(
    tableName: string,
    columnName: string
