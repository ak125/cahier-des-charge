...
import { ForeignKeyInfo, IndexInfo, TableInfo } from '../types';

/**
 * Suggérer des index pour améliorer les performances d'une table
 */
export function suggestIndexes(tableInfo: TableInfo, foreignKeys: ForeignKeyInfo[]): IndexInfo[] {
  ...
  for (const colName of commonWhereColumns) {
    if (tableInfo.columns[colName] && !indexExists([colName])) {
      ...
      const isAlreadyIndexed =
        tableInfo.columns[colName].isPrimary || tableInfo.columns[colName].isUnique;
  ...
  if (
    tableInfo.columns.created_at &&
    tableInfo.columns.type &&
    !indexExists(['created_at', 'type'])
  ) {
    suggestedIndexes.push({
  ...
  if (
    tableInfo.columns.user_id &&
    tableInfo.columns.created_at &&
    !indexExists(['user_id', 'created_at'])
  ) {
    suggestedIndexes.push({
