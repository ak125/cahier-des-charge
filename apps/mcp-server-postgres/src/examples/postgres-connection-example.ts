...
async function main() {
  ...
  const connectionString =
    'postgresql://migration:migration_password@localhost:5432/migration_db?schema=public';
  ...
  dbService.on('query', (query, _params, duration) => {
    console.log(
      `⏱️ Requête exécutée en ${duration}ms: ${query.substring(0, 80)}${
        query.length > 80 ? '...' : ''
      }`
    );
  });
