...
import { FiChevronDown, FiFilter, FiInfo, FiLayers, FiRefreshCw, FiSettings } from 'react-icons/fi';
    ...
    >
      ...
      >
        <Text>
          ...
          {JSON.stringify(data).length > 2 ? 'Données chargées' : 'En attente des données...'}
        </Text>
      </Flex>
    </Box>
  );
...
const BaseDashboard: React.FC<BaseDashboardProps> = ({
    ...
    return (
      ...
      >
        <Flex justifyContent="space-between" alignItems="center" mb={config.description ? 2 : 0}>
          ...
          <ButtonGroup size="sm">
            <Button leftIcon={<Icon as={FiRefreshCw} />} onClick={refreshData} isLoading={loading}>
              Actualiser
            </Button>
            ...
            {onLayerChange && (
              <Menu>
                ...
                <MenuList>
                  <MenuItem onClick={() => onLayerChange('all')}>Toutes les couches</MenuItem>
                  <MenuDivider />
                  <MenuItem onClick={() => onLayerChange('orchestration')}>Orchestration</MenuItem>
                  <MenuItem onClick={() => onLayerChange('agents')}>Agents</MenuItem>
                  <MenuItem onClick={() => onLayerChange('business')}>Métier</MenuItem>
                </MenuList>
              </Menu>
            )}
          </ButtonGroup>
        ...
        {/* Description */}
        {config.description && <Text color={subtitleColor}>{config.description}</Text>}
      ...
      <SimpleGrid columns={columns} spacing={4} p={4}>
        {metrics.map((metric) => {
          ...
          return (
            <Box key={metric.id}>
              <Stat>
                <StatLabel color={statusColor || textColor}>{metric.name}</StatLabel>
                ...
                {metric.trend !== undefined && (
                  <StatHelpText
                    color={
                      metric.trend > 0 ? 'green.500' : metric.trend < 0 ? 'red.500' : undefined
                    }
                  >
                    {metric.trend > 0
                      ? `↑ ${metric.trend}%`
    ...
    return (
      <Box p={4}>
        <Chart
          ...
          data={
            widget.chart.dataSourceId && data[widget.chart.dataSourceId]
              ? data[widget.chart.dataSourceId]
              : {}
          }
          height={height}
        />
  ...
  const renderListWidget = (_widget: Widget) => {
  ...
  const renderSection = (section: DashboardSection, index: number) => {
    return (
      <Box key={section.id} mb={index < config.sections.length - 1 ? 6 : 0}>
        ...
        <Grid templateColumns="repeat(6, 1fr)" gap={4} autoRows="minmax(100px, auto)">
