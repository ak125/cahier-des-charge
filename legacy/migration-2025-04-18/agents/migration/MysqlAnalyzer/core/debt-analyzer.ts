...
export class DebtAnalyzer {
  ...
  private detectTableIssues(tableName: string, table: TableInfo, issues: DebtIssue[]): void {
    ...
    if (!/^[a-z][a-z0-9_]*$/.test(tableName) && !/^[A-Z][a-zA-Z0-9]*$/.test(tableName)) {
      issues.push({
        ...
        recommendation:
          'Renommer la table en snake_case (pour PostgreSQL) ou PascalCase (pour Prisma)',
      });
    }
    ...
    table.foreignKeys.forEach((fk) => {
      ...
      if (!hasIndex) {
        issues.push({
          ...
          recommendation: `Créer un index sur la colonne ${
            fk.columns.length > 1 ? 's' : ''
          } ${fk.columns.join(', ')}`,
  ...
  private detectColumnIssues(
    ...
    if (
      ['TEXT', 'MEDIUMTEXT', 'LONGTEXT', 'BLOB', 'MEDIUMBLOB', 'LONGBLOB'].includes(
    ...
    if (
      ...
      issues.push({
        ...
        columnName,
        severity: 'medium',
        message: `Colonne ${columnName} semble être une clé étrangère implicite vers ${
  ...
  private detectIncorrectTypes(
    ...
    if (type === 'TINYINT' && column.length === 1) {
      ...
      const nameMatches = booleanNames.some((pattern) =>
    ...
    if (
      ['INT', 'INTEGER', 'BIGINT'].includes(type) &&
  ...
  private detectObsoleteColumns(
    ...
    const obsoletePatterns = [
      {
        pattern: /^old_/i,
        reason: 'Préfixe "old_" suggère une colonne conservée après refactoring',
      },
      { pattern: /^temp_/i, reason: 'Préfixe "temp_" suggère une colonne temporaire' },
      {
        pattern: /_old$/i,
        reason: 'Suffixe "_old" suggère une colonne conservée après refactoring',
      },
  ...
  private groupSimilarColumns(columns: Record<string, ColumnInfo>): Record<string, string[]> {
    ...
    return Object.fromEntries(Object.entries(groups).filter(([_, cols]) => cols.length >= 2));
  }
