...
import { DebtAnalyzer } from './mysql-analyzer/core/DebtAnalyzer';
import { PrismaGenerator } from './mysql-analyzer/core/PrismaGenerator';
import { RelationAnalyzer } from './mysql-analyzer/core/RelationAnalyzer';
import { TypeConverter } from './mysql-analyzer/core/TypeConverter';
...
import { DebtDetector } from './mysql-analyzer/agents/DebtDetector';
// Importation des agents spécialisés
import { RelationalNormalizer } from './mysql-analyzer/agents/RelationalNormalizer';
import { TypeAuditor } from './mysql-analyzer/agents/TypeAuditor';
...
program
  ...
  .option(
    '--analyze-php <dossier>',
    'Analyse les fichiers PHP pour détecter les JOIN et relations implicites'
  )
...
async function analyzeAndOptimizeMySQLSchema() {
  try {
    ...
    const { prismaSchema, prismaEnums, prismaModels, schemaMigrationDiff } =
    ...
    saveToMarkdown(
      path.join(outputDir, 'normalization_suggestions.md'),
    ...
    saveToMarkdown(
      path.join(outputDir, 'field_typing_issues.md'),
    ...
    saveToMarkdown(
      path.join(outputDir, 'field_renaming_suggestions.md'),
    ...
    if (options.splitModels) {
      ...
      printColoredMessage(
        ...
        } modèles Prisma individuels générés dans ${splitModelsDir}`,
        'success'
      );
    }
...
function generateNormalizationReport(suggestions: NormalizationSuggestion[]): string {
    ...
    byType['3NF_VIOLATION'].forEach((suggestion) => {
      ...
      suggestion.suggestedTables.forEach((table) => {
        ...
        if (table.foreignKeys.length > 0) {
          markdown += `  - Clés étrangères: ${table.foreignKeys
            .map((fk) => `${fk.columns} → ${fk.referencedTable}(${fk.referencedColumns})`)
            .join(', ')}\n`;
        }
      });
  ...
  const otherTypes = Object.keys(byType).filter(
    (type) => !['3NF_VIOLATION', 'REDUNDANT_TABLES'].includes(type)
  );
...
function generateTypingIssuesReport(issues: any[]): string {
  ...
  const byConversion = issues.reduce(
    (acc, issue) => {
...
function generateRenamingSuggestionsReport(suggestions: FieldRenamingSuggestion[]): string {
  ...
  const byTable = suggestions.reduce(
    (acc, sugg) => {
 ...
 */
function generateSummaryReport(outputDir: string, stats: any, debtScores: any[]): void {
  ...
  debtScores
    ...
    .forEach((score) => {
      const severity =
        score.score < 40 ? '🔴 Élevée' : score.score < 70 ? '🟠 Moyenne' : '🟢 Faible';
      markdown += `| ${score.tableName} | ${
        score.score
      }/100 | ${severity} | ${score.mainIssues.join(', ')} |\n`;
    });
