...
export class QualityAgent extends BaseAgent {
  ...
  private analyzeSecurityRisks(): { content: string; severity: 'info' | 'warning' | 'critical' } {
    ...
    if (
      ...
      (fileContent.includes('$_GET') ||
        fileContent.includes('$_POST') ||
        fileContent.includes('$_REQUEST'))
    ) {
      if (!fileContent.includes('prepare') && !fileContent.includes('real_escape_string')) {
        issues.push({
          issue:
            "Risque d'injection SQL : utilisation de variables non échappées dans les requêtes",
          severity: 'critical',
        });
      }
    }
  ...
  private evaluateOverallQuality(): { content: string; severity: 'info' | 'warning' | 'critical' } {
    ...
    const inconsistentIndentation = lines.some(
      (line) => line.trim().length > 0 && line.length - line.trimLeft().length === 1
    );
