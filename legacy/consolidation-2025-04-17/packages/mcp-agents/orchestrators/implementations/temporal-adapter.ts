...
export class TemporalAdapter implements OrchestrationAdapter {
  ...
  async validateWorkflowDefinition(
    ...
    if (
      ...
      typeof workflowDefinition.implementation !== 'object'
    ) {
    ...
    if (retryOptions.criticalityLevel) {
      switch (retryOptions.criticalityLevel) {
        ...
        case 'HIGH':
          ...
          retryOptions.maximumIntervalMs = Math.max(
            retryOptions.maximumIntervalMs || 60000,
            120000
          );
          break;
        case 'CRITICAL':
  ...
  async executeWorkflow(
    ...
    const executionId = enhancedOptions.executionId || `${workflowId}-${Date.now()}`;
      ...
      if (lastCheckpoint) {
        ...
        if (lastCheckpoint.status === 'failed' && lastCheckpoint.metadata?.lastError) {
          ...
          if (errorType.includes('transient') || errorType.includes('connection')) {
            ...
            enhancedOptions.retry.initialIntervalMs = Math.min(
              5000,
              enhancedOptions.retry.initialIntervalMs * 2
            );
          } else if (errorType.includes('resource')) {
            // Pour les erreurs de ressources, attendre plus longtemps entre les tentatives
            enhancedOptions.retry.maximumIntervalMs = Math.min(
              300000,
              enhancedOptions.retry.maximumIntervalMs * 1.5
            );
            enhancedOptions.retry.backoffCoefficient = Math.min(
              3,
              enhancedOptions.retry.backoffCoefficient * 1.2
            );
          }
        }

        console.log(
          `Reprise du workflow ${executionId} depuis le point de contrôle ${lastCheckpoint.step}`
        );
    ...
    try {
      const handle = await this.client.start(workflowId, workflowOptions);
      ...
      if (enhancedOptions.checkpointing?.enabled && this.checkpointManager) {
        const failedCheckpoint: MigrationCheckpoint = {
          ...
          metadata: {
            ...
            retryStrategy:
              lastCheckpoint?.metadata?.retryStrategy ||
              (enhancedOptions.retry
                ? {
                    maxAttempts: enhancedOptions.retry.maxAttempts,
                    initialDelayMs: enhancedOptions.retry.initialIntervalMs,
  ...
  async saveWorkflowCheckpoint(
    ...
    if (!this.checkpointManager) {
      console.warn(
        `Impossible de sauvegarder le point de contrôle pour ${workflowId}: CheckpointManager non initialisé`
      );
      return;
    }
  ...
  async getLastWorkflowCheckpoint(workflowId: string): Promise<MigrationCheckpoint | null> {
    if (!this.checkpointManager) {
      console.warn(
        `Impossible de récupérer le point de contrôle pour ${workflowId}: CheckpointManager non initialisé`
      );
      return null;
    }
