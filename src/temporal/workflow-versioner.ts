...
export class WorkflowVersioner {
  ...
  constructor(options: {
    ...
    this.versionsFilePath =
      options.versionsFilePath || path.join(process.cwd(), 'workflow-versions.json');
  ...
  registerVersion(
    ...
    this.versions[workflowName].sort((a, b) => semver.compare(b.version, a.version));
   ...
   */
  getTaskQueueForVersion(workflowName: string, version = 'latest'): string | null {
