    ...
    async listTasks(filter?: Record<string, any>): Promise<TaskResult[]> {
        ...
        for (const [queueName, queue] of this.queues.entries()) {
            try {
                ...
                console.error(`Erreur lors de la récupération des tâches pour la file ${queueName}:`, error);
            }
        }
...
export class TemporalOrchestrator implements Orchestrator {
    ...
    async listTasks(filter?: Record<string, any>): Promise<TaskResult[]> {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const completedAt = status === TaskStatus.PENDING || status === TaskStatus.RUNNING ? undefined : new Date(now - i * 10000);
    ...
    async getTaskStatus(taskId: string): Promise<TaskResult> {
        ...
        try {
            const response = await axios.get(
                `${this.apiUrl}/api/v1/executions/${taskId}`,
                {
                    headers: this.apiKey ? { 'X-N8N-API-KEY': this.apiKey } : {}
                }
            );
            ...
            if (response.status === 200) {
                ...
                return {
                    ...
                    duration: execution.finishedAt ? new Date(execution.finishedAt).getTime() - new Date(execution.startedAt).getTime() : undefined
                };
            } else {
...
class OrchestrationService {
    ...
    async getTaskStatus(taskId: string): Promise<TaskResult> {
        ...
        throw new Error(`Tâche non trouvée: ${taskId}. Erreurs: ${errors.map(e => e.message).join(', ')}`);
    }
