...
function setupWorkers() {
  ...
  const imageProcessingWorker = unifiedOrchestrator.createWorker(
    'imageProcessing', 
    async (job: Job) => {
  ...
  const notificationWorker = unifiedOrchestrator.createWorker(
    'notification',
    async (job: Job) => {
  ...
  const reportGenerationWorker = unifiedOrchestrator.createWorker(
    'reportGeneration',
    async (job: Job) => {
...
async function runImageProcessingWorkflow(imagePath: string, filters: string[], userId: string) {
  ...
  try {
    ...
    if (today.getDate() === 1) { // Premier jour du mois
