...
export class StandardizedOrchestrator {
  ...
  private async scheduleRapidQueueTask(
    ...
    return unifiedOrchestrator.scheduleTask(taskDefinition);
  }
  ...
  private async scheduleComplexWorkflow(
    ...
    // Import dynamique pour éviter la dépendance directe
    const { getTemporalClient } = await import('./temporal-client');
    const client = await getTemporalClient();

    // Générer un ID de workflow unique si non fourni
    const finalWorkflowId = workflowId ||
      `${workflowType}-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`;

    // Démarrer le workflow Temporal avec les options spécifiées
    ...
    console.log(`Workflow complexe démarré: ${finalWorkflowId}`);
  ...
  private async getTemporalWorkflowStatus(workflowId: string): Promise<TaskResult> {
    ...
    return {
      ...
      status,
      source: 'TEMPORAL',
      createdAt: new Date(description.startTime),
      updatedAt: description.status.name !== 'RUNNING' ? new Date(description.closeTime) : new Date(),
      startedAt: new Date(description.startTime),
      completedAt: description.status.name !== 'RUNNING' ? new Date(description.closeTime) : undefined,
      result: description.status.name === 'COMPLETED' ? await this.fetchTemporalResult(handle) : undefined,
      error: description.status.name === 'FAILED' ? await this.fetchTemporalError(handle) : undefined
    };
  }

  /**
   * Récupère le résultat d'un workflow Temporal
   */
  private async fetchTemporalResult(handle: any): Promise<any> {
    try {
  ...
  /**
   * Récupère l'erreur d'un workflow Temporal
   */
  private async fetchTemporalError(handle: any): Promise<any> {
    try {
      await handle.result();
      return undefined;
    } catch (error) {
