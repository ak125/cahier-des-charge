...
program
  ...
  .option('-i, --increment <type>', 'Type d\'incrémentation de version (patch, minor, major)', 'auto')
  ...
  try {
    ...
    if (shouldCreateVersion(changeAnalysis, config, options)) {
      ...
      if (!options.dryRun) {
        ...
        displayVersionSummary(`CDC-${new Date().toISOString().replace(/:/g, '')}-${versionDetails.version}-xxxxxx`, versionDetails);
      }
    } else {
...
async function loadConfig() {
  try {
    ...
    try {
      ...
      await fs.writeFile(
        CONFIG_PATH,
        yaml.dump(DEFAULT_CONFIG, { indent: 2 }),
        'utf8'
      );
  ...
  try {
    ...
    if (sinceRef) {
      ...
      const diffOutput = execSync(
        `git diff --numstat ${sinceRef} HEAD -- ${CDC_DIR}`,
        { encoding: 'utf8' }
      );
 ...
 */
async function analyzeChecksumChanges(cdcFiles, config) {
  try {
    ...
    for (const file of cdcFiles) {
      ...
      try {
        ...
        console.error(chalk.red(`❌ Erreur lors de la lecture du fichier ${file}: ${error.message}`));
      }
    }
 ...
 */
function analyzeSemanticChanges(cdcFiles, sinceRef) {
  ...
  try {
    ...
    const diffOutput = execSync(
      `git diff ${sinceRef} HEAD -- ${CDC_DIR}`,
      { encoding: 'utf8' }
    );
    ...
    return {
      ...
      functional: diffOutput.toLowerCase().includes('fonctionnel') || diffOutput.toLowerCase().includes('exigence'),
      technical: diffOutput.toLowerCase().includes('technique') || diffOutput.toLowerCase().includes('technologie'),
...
function shouldCreateVersion(analysis, config, options) {
  ...
  if (config.versioning.semantic_analysis && analysis.semanticAnalysis) {
    if (analysis.semanticAnalysis.impact === 'major' || analysis.semanticAnalysis.impact === 'minor') {
      return true;
    }
  }
...
async function generateVersionDetails(analysis, config, options) {
  ...
  if (options.increment !== 'auto') {
    ...
    newVersion = lastVersion 
      ? semver.inc(lastVersion.version, options.increment)
      : '1.0.0';
    ...
    newVersion = lastVersion 
      ? semver.inc(lastVersion.version, increment)
      : '1.0.0';
  } else {
    // Incrément par défaut (patch)
    newVersion = lastVersion 
      ? semver.inc(lastVersion.version, 'patch')
      : '1.0.0';
  }
...
async function createVersionSnapshot(versionDetails) {
  try {
    ...
    const versionId = `CDC-${versionDetails.timestamp.replace(/:/g, '')}-${versionDetails.version}-${versionDetails.hash}`;
...
async function generateChangelog(versionId, versionDetails) {
  try {
    ...
    if (versionDetails.semanticAnalysis) {
      ...
      changelog += `- Changement architectural: ${versionDetails.semanticAnalysis.architectural ? 'Oui' : 'Non'}\n`;
      changelog += `- Changement fonctionnel: ${versionDetails.semanticAnalysis.functional ? 'Oui' : 'Non'}\n`;
      changelog += `- Changement technique: ${versionDetails.semanticAnalysis.technical ? 'Oui' : 'Non'}\n`;
      changelog += `- Changement éditorial: ${versionDetails.semanticAnalysis.editorial ? 'Oui' : 'Non'}\n\n`;
    }
...
function displayVersionSummary(versionId, versionDetails) {
  ...
  console.log(chalk.blue('─────────────────────────────────────'));
  console.log(`Changements: ${chalk.yellow(`${versionDetails.changeVolume.additions} ajouts, ${versionDetails.changeVolume.deletions} suppressions`)}`);
  console.log(`Fichiers: ${chalk.yellow(`${versionDetails.changeVolume.files_changed} modifiés sur ${versionDetails.changeVolume.total_files}`)}`);
  
  if (versionDetails.semanticAnalysis) {
    const impactColor = 
      versionDetails.semanticAnalysis.impact === 'major' ? chalk.red :
      versionDetails.semanticAnalysis.impact === 'minor' ? chalk.yellow :
      chalk.green;
