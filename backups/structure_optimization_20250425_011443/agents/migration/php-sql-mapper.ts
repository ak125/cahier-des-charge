...
export async function mapPhpSqlFields(
  ...
  const mappingReport = generateMappingReport(phpFilePath, prismaSchemaPath, fieldMappings, prismaModels);
  ...
  fs.writeFileSync(
    migrationPatchPath,
    JSON.stringify(migrationPatch, null, 2)
  );
...
function mapFieldsToPrismaModels(
  ...
  for (const sqlField of sqlFields) {
    ...
    for (const model of prismaModels) {
      ...
      const exactMatch = model.fields.find(f => 
        f.name === sqlField.field || 
        (f.map && f.map === sqlField.field)
      );
      ...
      const normalizedPhpField = sqlField.field.replace(/_([a-z])/g, (_, letter) => letter.toUpperCase());
      const conventionMatch = model.fields.find(f =>
        f.name.toLowerCase() === normalizedPhpField.toLowerCase()
      );
  ...
  for (const model of prismaModels) {
    for (const field of model.fields) {
      const isUsed = mappings.some(m => 
        m.prismaField === field.name || 
        (field.map && m.sqlField === field.map)
      );
...
function generateMappingReport(
  ...
  for (const mapping of fieldMappings) {
    ...
    const model = prismaModels.find(m => 
      m.fields.some(f => f.name === mapping.prismaField)
    );
  ...
  const unusedPrismaFields = fieldMappings
    ...
    .map(m => {
      // Trouver le modèle correspondant
      const model = prismaModels.find(mod => 
        mod.fields.some(f => f.name === m.prismaField)
      );
 ...
 */
function getPrismaModelNameFromField(
  prismaField: string, 
...
async function updateAuditFile(
  ...
  for (const mapping of fieldMappings) {
    const status = 
      ...
      mapping.status === 'renamed' ? '🔄 Renamed' :
      mapping.status === 'missing_in_prisma' ? '❌ Missing' :
      mapping.status === 'unused_in_php' ? '⚠️ Unused' : '';
    
    const confidence = mapping.status === 'missing_in_prisma' ? 'N/A' : 
...
async function updateBacklogFile(
  ...
  backlog.tasks = backlog.tasks.filter((task: any) => 
    task.type !== 'sql_mapping'
  );
