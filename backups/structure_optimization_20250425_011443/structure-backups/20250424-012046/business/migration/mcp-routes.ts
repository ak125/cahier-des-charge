...
import { Controller, Post, Body, Get, Param, Logger, Injectable } from @nestjs/commonstructure-agent';
import { agentRegistry, AgentName, AgentRegistryManager } from ./agentRegistrystructure-agent';
...
export function generateAgentController(agentId: AgentName) {
  ...
  class DynamicAgentController {
    ...
    @Post()
    async process(@Body() mcpRequest: MCPRequest): Promise<MCPResponse> {
      const startTime = Date.now();

      try {
        // Instanciez l'agent - supposant que tous les agents ont un constructeur qui accepte des options
        const agent = new this.agentClass(mcpRequest.options || {});
        ...
        const result = await agent[methodToCall](mcpRequest.input, mcpRequest.context);
 ...
 * import { MCPRoutesService } from ./mcp-routesstructure-agent';
 * 
 * @Module({
 *   imports: [],
 *   controllers: [],
 ...
 *   constructor(private readonly mcpRoutesService: MCPRoutesService) {}
 * 
 *   async onModuleInit() {
 *     // Génère dynamiquement tous les controllers d'agents
 *     const controllers = await this.mcpRoutesService.generateAllControllers();
 *     
