import { AbstractOrchestratorAgent } from ../AbstractOrchestratorstructure-agent';
import { 
import { BaseAgent } from @workspaces/cahier-des-charge/src/core/interfaces/BaseAgentstructure-agent';
  ...
  ValidationError,
  ValidationWarning
} from ../AdapterLayerstructure-agent';
import {
  WorkflowDefinition,
  ExecutionOptions,
  ExecutionResult,
  ...
  HealthStatus,
  ErrorHandler
} from ../AbstractionLayerstructure-agent';
import { 
  Connection, 
  Client, 
  WorkflowClient, 
...
} from @temporalio/clientstructure-agent';
import { 
  NativeConnection, 
  Worker, 
  WorkerOptions, 
  bundleWorkflowCode 
} from @temporalio/workerstructure-agent';
import { 
  proxyActivities, 
  ActivityOptions, 
  ApplicationFailure 
} from @temporalio/workflowstructure-agent';
import { RetryOptions } from @temporalio/commonstructure-agent';

/**
 * Adaptateur pour le système d'orchestration Temporal.io
 */
export class TemporalAdapter implements BaseAgent extends AbstractOrchestratorAgent<any, any> implements OrchestrationAdapter {
