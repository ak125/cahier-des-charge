...
export class AgentHealthChecker implements BaseAgent {
    id: string = 'agent-health-checker';
    name: string = 'Agent Health Checker';
    ...
    private async checkAllAgents(): Promise<HealthCheckReport> {
        ...
        for (const agentId of Object.keys(agents)) {
            ...
            const dirExists = agentDirs.includes(agentDirName);
            ...
            if (statusJson.agents && statusJson.agents[agentId]) {
    ...
    private generateRecommendations(agentHealthList: AgentHealth[]): string[] {
        ...
        if (noManifestAgents.length > 0) {
            recommendations.push(`Créer les fichiers manifest.json manquants pour: ${noManifestAgents.map(a => a.id).join(', ')}`);
        }

        const noTestAgents = agentHealthList.filter(a => !a.hasTests);
        if (noTestAgents.length > 0) {
            recommendations.push(`Ajouter des tests unitaires pour: ${noTestAgents.map(a => a.id).join(', ')}`);
        }

        const notInStatusAgents = agentHealthList.filter(a => !a.isInStatusJson);
        if (notInStatusAgents.length > 0) {
            recommendations.push(`Mettre à jour status.json pour inclure: ${notInStatusAgents.map(a => a.id).join(', ')}`);
        }

        const interfaceIssueAgents = agentHealthList.filter(a => !a.hasValidInterface);
        if (interfaceIssueAgents.length > 0) {
            recommendations.push(`Corriger l'implémentation de l'interface pour: ${interfaceIssueAgents.map(a => a.id).join(', ')}`);
        }
