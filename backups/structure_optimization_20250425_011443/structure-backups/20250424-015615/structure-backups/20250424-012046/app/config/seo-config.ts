import { CanonicalConfig } from ~/utils/canonicalstructure-agent';
...
export const config = {
  ...
  defaultMeta: {
    ...
    description: 'Achetez vos pièces détachées auto à prix discount. Livraison rapide, large gamme de produits pour toutes marques de véhicules.',
  ...
  templates: {
    fiche: {
      ...
      description: (data) => data.description?.slice(0, 160) || `Découvrez ${data.nom} - Prix bas et livraison rapide sur ${config.defaultMeta.siteName}`,
      canonical: (data) => `/fiche/${data.slug}`
    },
    categorie: {
      title: (data) => `${data.nom} - Catalogue Complet | ${config.defaultMeta.siteName}`,
      description: (data) => data.description?.slice(0, 160) || `Explorez notre gamme de ${data.nom} à prix compétitifs. Livraison rapide et garantie qualité.`,
      canonical: (data) => `/categorie/${data.slug}`
    },
    marque: {
      title: (data) => `Pièces ${data.nom} - Catalogue Officiel | ${config.defaultMeta.siteName}`,
      description: (data) => `Pièces détachées ${data.nom} d'origine au meilleur prix. Catalogue complet, expédition sous 24h.`,
      canonical: (data) => `/marque/${data.slug}`
    },
    recherche: {
      title: (data) => `Résultats pour "${data.query}" | ${config.defaultMeta.siteName}`,
      description: (data) => `Découvrez nos produits correspondant à votre recherche "${data.query}". Livraison rapide, paiement sécurisé.`,
