...
async function analyzeAgent(filePath: string): Promise<AgentAnalysis> {
  ...
  // Utiliser git grep pour compter les utilisations
  let usageCount = 0;
  try {
...
import { ${interfaceBaseName} } from @workspaces/cahier-des-charge/src/core/interfaces/${agent.layer}structure-agent';
