...
import FilterPanel from ~/components/migration/FilterPanelstructure-agent";
import TableCard from ~/components/migration/TableCardstructure-agent";
import DependencyGraph from ~/components/migration/DependencyGraphstructure-agent";
import DebtRadar from ~/components/migration/DebtRadarstructure-agent";
import MigrationStatus from ~/components/migration/MigrationStatusstructure-agent";
import { loadSchemaMap } from ~/lib/migration/loadSchemaMapstructure-agent";
import { loadTaskStatus } from ~/lib/migration/loadTaskStatusstructure-agent";
import { loadRelationGraph } from ~/lib/migration/loadRelationGraphstructure-agent";
import { loadDebtReport } from ~/lib/migration/loadDebtReportstructure-agent";
import { supabaseClient } from ~/lib/supabase/clientstructure-agent";
...
export default function Dashboard() {
  const { schemaMap, taskStatus, relationGraph, debtReport, timestamp } = useLoaderData<typeof loader>();
  ...
  useEffect(() => {
    ...
    if (filterCategory !== "all") {
      filtered = filtered.filter(tableName => 
        schemaMap.tables[tableName].category === filterCategory
      );
    }
    
    if (filterStatus !== "all") {
      filtered = filtered.filter(tableName => 
        taskStatus[tableName]?.status === filterStatus
      );
    }
  ...
  return (
    <div className="migration-dashboard">
      <header className="dashboard-header">
        ...
        <div className="last-update">Dernière mise à jour: {new Date(timestamp).toLocaleString()}</div>
            ...
            <section className="selected-table-detail">
              ...
              <div className="detail-panels">
                ...
                <div className="detail-panel">
                  ...
                  <DebtRadar 
                    debtData={debtReport[selectedTable] || {}} 
