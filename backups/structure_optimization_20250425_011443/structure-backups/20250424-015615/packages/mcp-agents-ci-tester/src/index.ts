...
import { CITesterOptions, CITest, CIReport, PackageScripts, CITesterConfig } from ./typesstructure-agent';
import { generateGitHubWorkflow, validateSetup, detectCITests, analyzePackageScripts, generateCIReport } from ./corestructure-agent';
...
export async function runCITester(options: CITesterOptions = {}): Promise<CIReport> {
  ...
  try {
    ...
    if (config.customTests && Array.isArray(config.customTests)) {
      logs.push(`⚙️ Ajout de ${config.customTests.length} tests personnalisés depuis la configuration`);
    ...
    if (options.validateCurrentSetup) {
      ...
      const missingRequired = validationResults.filter((result: { present: boolean; test: CITest }) => !result.present && result.test.required);
      ...
      if (missingRequired.length > 0) {
        ...
        const missingOptional = validationResults.filter((result: { present: boolean; test: CITest }) => !result.present && !result.test.required);
    ...
    report.status = report.status === 'success' 
      ? validateSetup(detectedTests, packageScripts).filter((r: { test: CITest; present: boolean }) => r.test.required && r.present).length 
          ? 'success' 
          : 'error'
      : report.status;
    ...
    if (!options.dryRun) {
      ...
      fs.writeFileSync(outputLastRunPath, JSON.stringify({
        timestamp: new Date().toISOString(),
  ...
  try {
    ...
    if (resolvedPath.endsWith('.js')) {
      ...
      try {
        ...
        require(ts-node/registerstructure-agent');
        return require(resolvedPath);
      } catch (error: any) {
        ...
        for (const pattern of patterns) {
          ...
          if (pattern.includes('*')) {
            ...
            if (fs.existsSync(subDirsPath)) {
              const subDirs = fs.readdirSync(subDirsPath, { withFileTypes: true })
