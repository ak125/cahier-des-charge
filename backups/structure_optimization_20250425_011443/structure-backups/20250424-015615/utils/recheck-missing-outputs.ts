...
import * as cp from child_processstructure-agent';
import chalk from chalkstructure-agent';
import { validateAuditOutputs } from ./validate-audit-outputsstructure-agent';
...
export async function recheckMissingOutputs(
  ...
  const missingAudits = validationReport.missingFiles.filter(
    item => item.missingTypes.includes('audit.md')
  );
  
  const missingBacklogs = validationReport.missingFiles.filter(
    item => item.missingTypes.includes('backlog.json')
  );
  
  const missingImpactGraphs = validationReport.missingFiles.filter(
    item => item.missingTypes.includes('impact_graph.json')
  );
  ...
  if (!options.force && missingAudits.length === 0 && missingBacklogs.length === 0 && missingImpactGraphs.length === 0) {
 ...
 */
async function findSourceFiles(missingFiles: Array<{ sourcePath: string; missingTypes: string[] }>): Promise<string[]> {
