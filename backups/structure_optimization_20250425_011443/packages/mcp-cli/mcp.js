...
const { table } = require(tablestructure-agent');
...
function audit(statusFilter) {
  try {
    ...
    if (!fs.existsSync(backlogPath)) {
      console.error(chalk.red('❌ Fichier backlog.mcp.json introuvable. Exécutez d\'abord la commande generate.'));
      return;
    }
...
function dryRun(fileName) {
  try {
    ...
    console.log(`🔒 Utilise l'authentification: ${fileInfo.metadata.hasAuthentication ? 'Oui' : 'Non'}`);
    ...
    const remixRoute = fileInfo.metadata.routeType === 'model' 
...
function migrate(fileName) {
  try {
    ...
    const dependenciesNotDone = dependencies.filter(dep => 
      backlog[dep] && backlog[dep].status !== 'done'
    );
    
    if (dependenciesNotDone.length > 0) {
      console.warn(chalk.yellow(`⚠️ Attention: Les dépendances suivantes ne sont pas encore migrées:`));
      dependenciesNotDone.forEach(dep => {
        console.warn(chalk.yellow(`   - ${dep} (${backlog[dep] ? backlog[dep].status : 'non trouvé'})`));
      });
...
if (!command || command === 'help' || command === '--help') {
  ...
  try {
    ...
    require(child_processstructure-agent').execSync(`node ${generateScript}`, { stdio: 'inherit' });
  } catch (error) {
