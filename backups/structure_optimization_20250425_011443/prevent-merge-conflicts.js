...
// Fonction principale
function main() {
    console.log(`${colors.blue}1. Recherche de dossiers avec une profondeur excessive...${colors.reset}`);
    ...
    if (deepFolders.length > 0) {
        ...
        for (const folder of deepFolders) {
            console.log(`  ${colors.yellow}Profondeur excessive: ${folder.path} (${folder.depth} niveaux, max: ${MAX_DEPTH})${colors.reset}`);
        }
    } else {
    ...
    if (longPaths.length > 0) {
        ...
        for (const longPath of longPaths) {
            console.log(`  ${colors.red}⚠️ Chemin trop long: ${longPath} (${longPath.length} caractères, max: ${MAX_PATH_LENGTH})${colors.reset}`);
        }
    } else {
    ...
    if (errorCount > 0) {
        console.log(`${colors.red}⛔ Des erreurs critiques ont été détectées. Correction nécessaire avant de continuer.${colors.reset}`);
        process.exit(1);
    } else if (warningCount > 0) {
        console.log(`${colors.yellow}⚠️ Des avertissements ont été détectés. Veuillez les examiner avant de continuer.${colors.reset}`);
        process.exit(0);
    } else {
        console.log(`${colors.green}✅ Aucun problème détecté. La structure du projet est propre.${colors.reset}`);
        process.exit(0);
    }
}
