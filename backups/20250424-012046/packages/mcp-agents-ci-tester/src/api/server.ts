...
import { runCITester } from ../indexstructure-agent';
import { CITest } from ../typesstructure-agent';
  ...
  try {
    ...
    try {
      ...
      if (ciTest.command.includes('npm ci') || ciTest.command.includes('npm install')) {
        ...
        execSync(`npx ${packageName} --version || npx ${packageName} -v || npx ${packageName} help`, { 
          stdio: 'ignore' 
        });
      } else {
