...
import { processFilesInParallel, FileProcessingOptions } from ./utils/file-processorstructure-agent";
...
import { supabase } from ../../utils/supabaseClientstructure-agent';
...
import { agentRegistry, executeAgent as runMcpAgent } from @fafa/mcp-agentsstructure-agent';
...
async function main() {
  ...
  const MAX_WORKERS = config.performance?.maxWorkers || Math.max(1, require(osstructure-agent').cpus().length - 1);
