    ...
    try {
        ...
        if (fallbackPath) {
            try {
                ...
                console.error(`Erreur lors de la lecture du fichier ${filePath} ou de son fallback:`, primaryError);
                return null;
            }
        }
...
function generateMockAuditData(): PipelineAuditData {
    ...
    const missingAgentsData = Object.entries(auditReport.statistics.missingAgents).map(([agent, count]) => ({
        agent: agent.replace('-agent', ''),
        count
    }));

    const statusDistribution = Object.entries(auditReport.statistics.byStatus).map(([status, count]) => ({
        status,
        count
    }));
...
export async function loadPipelineAuditData(): Promise<PipelineAuditData> {
    try {
        ...
        const lastUpdateInfo = await loadJsonFile<{ lastUpdated: string }>(
            DATA_PATHS.lastUpdate
        );
        ...
        if (Object.keys(baseAuditReport.statistics.missingAgents).length > 0) {
            missingAgentsData = Object.entries(baseAuditReport.statistics.missingAgents).map(([agent, count]) => ({
                agent: agent.replace('-agent', ''),
                count
            }));
        }
        ...
        if (Object.keys(baseAuditReport.statistics.byStatus).length > 0) {
            statusDistribution = Object.entries(baseAuditReport.statistics.byStatus).map(([status, count]) => ({
                status,
                count
            }));
        }
