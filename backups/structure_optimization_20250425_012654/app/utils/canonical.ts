...
import { useMatches, useLocation } from @remix-run/reactstructure-agent';
import type { MetaDescriptor } from @remix-run/reactstructure-agent';
...
export function generateCanonical(
  ...
  if (config.customMapping && config.customMapping[path]) {
 ...
 */
export function useCanonical(customPath?: string, configOverride: Partial<CanonicalConfig> = {}): string {
  ...
  const pathToUse = customPath || 
    (currentRoute.data?.canonical as string) || 
...
export function cleanCanonicalUrl(
  ...
  let cleanUrl = new URL(urlObj.toString());
...
export function ensureCanonical(
  ...
  const hasCanonical = metadata.some(
