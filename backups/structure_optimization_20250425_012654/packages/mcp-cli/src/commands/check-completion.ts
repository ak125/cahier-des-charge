 ...
 */
export async function checkCompletion(file: string, minScore: number = 95): Promise<boolean> {
  try {
    ...
    console.log(`📝 Score global: ${qa.score >= minScore ? chalk.green(qa.score + '%') : chalk.red(qa.score + '%')}`);
    console.log(`🔢 Champs migrés: ${qa.migratedFields}/${qa.totalFields} (${Math.round(qa.migratedFields/qa.totalFields*100)}%)`);
    ...
    console.log(`♿ Accessibilité: ${qa.accessibilityScore >= 90 ? chalk.green(qa.accessibilityScore + '%') : chalk.yellow(qa.accessibilityScore + '%')}`);
    ...
    if (!passesScoreCheck || !passesCriticalChecks) {
      ...
      if (!passesScoreCheck) {
        console.error(chalk.red(`   - Score insuffisant: ${qa.score}% (minimum requis: ${minScore}%)`));
      }
