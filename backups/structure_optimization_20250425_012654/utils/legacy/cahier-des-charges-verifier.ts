...
class CahierDesChargesVerifier {
  ...
  private async verifyAudits(): Promise<void> {
    ...
    for (const auditPath of this.result.scannedFiles.auditFiles) {
      try {
        ...
        const inDiscoveryMap = this.discoveryMap.some(item => 
          item.id === baseFileName || item.path.includes(baseFileName)
        );
        ...
        console.error(`❌ Erreur lors de la vérification de l'audit ${auditPath}: ${error.message}`);
      }
    }
  }
    ...
    for (const backlogPath of this.result.scannedFiles.backlogFiles) {
      try {
        ...
        try {
          ...
          const obsoleteTasks = backlog.tasks.filter(task => task.status === 'completed' || task.status === 'cancelled');
        ...
        console.error(`❌ Erreur lors de la vérification du backlog ${backlogPath}: ${error.message}`);
      }
    }
  }
  ...
  private async verifyImpactGraphs(): Promise<void> {
    ...
    for (const graphPath of this.result.scannedFiles.impactGraphFiles) {
      try {
        ...
        console.error(`❌ Erreur lors de la vérification du graphe d'impact ${graphPath}: ${error.message}`);
      }
    }
  }
  ...
  private async detectDuplicates(): Promise<void> {
    ...
    const backlogGroups = this.groupFilesByBaseName(this.result.scannedFiles.backlogFiles, '.backlog.json');
    const graphGroups = this.groupFilesByBaseName(this.result.scannedFiles.impactGraphFiles, '.impact_graph.json');
   ...
   */
  private groupFilesByBaseName(files: string[], suffix: string): Record<string, string[]> {
      ...
      for (const [file2, graph2] of Object.entries(impactGraphs)) {
        ...
        if (similarity > 0.7) {
          this.result.mergeSuggestions.push({
            files: [file1, file2],
            similarity,
            reason: `Les fichiers partagent ${(similarity * 100).toFixed(0)}% de dépendances communes`
          });
        }
      }
    }
  ...
  public generateQualityReport(): string {
    ...
    if (this.result.completeFiles.length > 0) {
      ...
      for (const file of this.result.completeFiles.slice(0, 10)) { // Limiter à 10 pour la lisibilité
    ...
    if (this.result.issues.length === 0 && this.result.missingFiles.length === 0) {
      ...
      report += `⚠️ Le cahier des charges présente quelques problèmes qui devraient être résolus :\n\n`;
  ...
  public generateMergePlan(): string {
    ...
    if (auditMerges.length > 0) {
      for (const merge of auditMerges) {
        ...
        plan += `- **Stratégie** : Combiner les sections uniques, conserver les plus complètes pour les sections communes\n\n`;
        
        plan += `**Commande** :\n`;
        plan += `\`\`\`bash\n`;
        plan += `npx ts-node scripts/merge-files.ts ${merge.files.join(' ')} --output=${path.dirname(merge.files[0])}/${path.basename(merge.files[0]).replace('.v1', '')}\n`;
        plan += `\`\`\`\n\n`;
      }
    } else {
    ...
    if (backlogMerges.length > 0) {
      for (const merge of backlogMerges) {
        ...
        plan += `**Commande** :\n`;
        plan += `\`\`\`bash\n`;
        plan += `npx ts-node scripts/merge-backlogs.ts ${merge.files.join(' ')} --output=${path.dirname(merge.files[0])}/${path.basename(merge.files[0]).replace('.v1', '')}\n`;
        plan += `\`\`\`\n\n`;
      }
    } else {
  ...
  public generateDeletionPlan(): string {
    ...
    plan += `Ce document présente un plan pour nettoyer les fichiers obsolètes du cahier des charges.\n\n`;
