...
import QualityTrendsChart from ../components/QualityTrendsChartstructure-agent";
import QualityImprovementTips from ../components/QualityImprovementTipsstructure-agent";
import QualityAlertNotification, { QualityAlert } from ../components/QualityAlertNotificationstructure-agent";
  ...
  return (
    <div className="flex border-b mb-4 overflow-x-auto">
      {tabs.map(tab => (
        <button
          key={tab.id}
          className={`px-4 py-2 font-medium whitespace-nowrap ${activeTab === tab.id ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500'}`}
...
export default function DashboardQualityScores() {
  ...
  const handleMarkAsRead = (alert: QualityAlert) => {
    setCurrentAlerts(currentAlerts.map(a => 
      a === alert ? { ...a, isRead: true } : a
    ));
  };
  ...
  const filteredResults = useMemo(() => {
    return Object.entries(dashboardData.results).filter(([filePath, result]) => {
      ...
      const matchesSearch = searchTerm === "" || 
                           ...
                           result.missingFields.some(field => field.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const historicalChartData = useMemo(() => {
    ...
    return {
      ...
      performanceScores: dashboardData.historicalData.map(entry => entry.avgPerformanceScore || 0),
      accessibilityScores: dashboardData.historicalData.map(entry => entry.avgAccessibilityScore || 0),
      bestPracticesScores: dashboardData.historicalData.map(entry => entry.avgBestPracticesScore || 0)
    };
  ...
  const qualityIssues = useMemo(() => {
      ...
      if (result.missingFields && result.missingFields.length > 0) {
        result.missingFields.forEach(field => {
          const existingIssue = issues.find(issue => 
            issue.category === 'seo' && issue.message === `Champ manquant: ${field}`
          );
      ...
      if (result.accessibilityIssues && result.accessibilityIssues.length > 0) {
        result.accessibilityIssues.forEach(issue => {
          const existingIssue = issues.find(i => 
            i.category === 'accessibility' && i.message === issue.message
          );
          ...
          if (!existingIssue) {
            issues.push({
              ...
              solution: `Corriger l'élément ${issue.selector || 'concerné'} pour assurer une meilleure accessibilité.`,
              documentation: `/docs/accessibility/${issue.type?.toLowerCase().replace(/\s+/g, '-')}`
            });
          }
        });
      }
    ...
    Object.values(dashboardData.results).forEach(result => {
      if (result.performanceMetrics) {
        if (result.performanceMetrics.largestContentfulPaint && result.performanceMetrics.largestContentfulPaint > 2.5) {
          const existingIssue = issues.find(i => 
            i.category === 'performance' && i.message.includes('Largest Contentful Paint')
          );
          ...
          if (!existingIssue) {
            issues.push({
              ...
              solution: 'Optimiser les images, utiliser le lazy loading, et améliorer le temps de chargement des ressources principales.',
              documentation: '/docs/performance/optimize-lcp'
            });
          }
        }
        
        if (result.performanceMetrics.cumulativeLayoutShift && result.performanceMetrics.cumulativeLayoutShift > 0.1) {
          const existingIssue = issues.find(i => 
            i.category === 'performance' && i.message.includes('Cumulative Layout Shift')
          );
          ...
          if (!existingIssue) {
            issues.push({
              ...
              solution: 'Définir les dimensions des images et vidéos, éviter d\'insérer du contenu dynamique au-dessus du contenu existant.',
    ...
    Object.values(dashboardData.results).forEach(result => {
      if (result.bestPracticesIssues && result.bestPracticesIssues.length > 0) {
        result.bestPracticesIssues.forEach(issue => {
          const existingIssue = issues.find(i => 
            i.category === 'bestPractices' && i.message === issue.message
          );
          ...
          if (!existingIssue) {
            issues.push({
              ...
              documentation: `/docs/best-practices/${issue.category?.toLowerCase().replace(/\s+/g, '-')}`
            });
          }
        });
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="flex items-end">
                <span className={`text-4xl font-bold ${
                  dashboardData.averageScore >= 90 ? 'text-green-600' : 
                  dashboardData.averageScore >= 70 ? 'text-yellow-600' : 
                  'text-red-600'
                }`}>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="flex space-x-4">
                <div>
                  <div className="text-2xl font-bold text-green-600">{dashboardData.stats.passedFiles}</div>
                  <div className="text-sm text-gray-500">Succès</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-yellow-600">{dashboardData.stats.partialFiles}</div>
                  <div className="text-sm text-gray-500">Partiel</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-red-600">{dashboardData.stats.failedFiles}</div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-50 p-4 rounded">
                    ...
                    <div className={`text-2xl font-bold ${
                      averageScores.seo >= 90 ? 'text-green-600' : 
                      averageScores.seo >= 70 ? 'text-yellow-600' : 
                      'text-red-600'
                    }`}>
                  ...
                  <div className="bg-gray-50 p-4 rounded">
                    ...
                    <div className={`text-2xl font-bold ${
                      averageScores.performance >= 90 ? 'text-green-600' : 
                      averageScores.performance >= 70 ? 'text-yellow-600' : 
                      'text-red-600'
                    }`}>
                  ...
                  <div className="bg-gray-50 p-4 rounded">
                    ...
                    <div className={`text-2xl font-bold ${
                      averageScores.accessibility >= 90 ? 'text-green-600' : 
                      averageScores.accessibility >= 70 ? 'text-yellow-600' : 
                      'text-red-600'
                    }`}>
                  ...
                  <div className="bg-gray-50 p-4 rounded">
                    ...
                    <div className={`text-2xl font-bold ${
                      averageScores.bestPractices >= 90 ? 'text-green-600' : 
                      averageScores.bestPractices >= 70 ? 'text-yellow-600' : 
                      'text-red-600'
                    }`}>
                ...
                {dashboardData.topIssues.map((issue, index) => (
                  <li key={index} className="py-3">
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      ...
                      {issue.affectedFiles.length > 3 && ` et ${issue.affectedFiles.length - 3} autres`}
                    </p>
                  </li>
                ))}
              </ul>
              ...
              {filteredResults.length > 0 ? (
                <ul className="divide-y divide-gray-200">
                  {filteredResults.map(([filePath, result]) => (
                    <li 
                      key={filePath}
                      className={`p-4 cursor-pointer hover:bg-gray-50 ${selectedFile === filePath ? 'bg-blue-50' : ''}`}
            ...
            <div className="p-6 overflow-y-auto h-96 md:h-[32rem]">
              {selectedFileDetails ? (
                <>
                  ...
                  <div className="mb-6">
                    <div className="flex items-center mb-2">
                      ...
                      <span className={`font-semibold ${
                        selectedFileDetails.score >= 90 ? 'text-green-600' :
                        selectedFileDetails.score >= 70 ? 'text-yellow-600' :
                        'text-red-600'
                      }`}>
                    ...
                    <div className="flex items-center mb-2">
                      ...
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        selectedFileDetails.status === 'success' ? 'bg-green-100 text-green-800' :
                        selectedFileDetails.status === 'partial' ? 'bg-yellow-100 text-yellow-800' :
                        'bg-red-100 text-red-800'
                      }`}>
                        {selectedFileDetails.status === 'success' ? 'Succès' :
                         selectedFileDetails.status === 'partial' ? 'Partiel' :
                         'Échec'}
                      </span>
                    </div>
                    <div className="flex items-center">
                  ...
                  <div className="mb-6">
                    <div className="mb-2">
                      <h5 className="text-sm font-medium text-gray-700">Champs requis ({selectedFileDetails.requiredFields.length})</h5>
                    ...
                    {selectedFileDetails.optionalFields.length > 0 && (
                      <div>
                        <h5 className="text-sm font-medium text-gray-700">Champs optionnels ({selectedFileDetails.optionalFields.length})</h5>
                        <div className="mt-1 flex flex-wrap gap-1">
                        ...
                        {selectedFileDetails.history.map((entry, index) => (
                          <li key={index} className="py-2 flex justify-between">
                            ...
                            <span className={`text-sm font-medium ${
                              ...
                              entry.score >= 70 ? 'text-yellow-600' :
                              'text-red-600'
                            }`}>
        ...
        <div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="flex items-end">
                <span className={`text-4xl font-bold ${
                  (dashboardData.performanceStats?.averageScore || 0) >= 90 ? 'text-green-600' : 
                  (dashboardData.performanceStats?.averageScore || 0) >= 70 ? 'text-yellow-600' : 
                  'text-red-600'
                }`}>
              ...
              <div className="mt-2 text-sm text-gray-500">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-green-600">{dashboardData.performanceStats?.goodPerformanceCount || 0}</span>
                </div>
                <div className="flex justify-between">
                  <span>Sites lents:</span> 
                  <span className="font-medium text-red-600">{dashboardData.performanceStats?.poorPerformanceCount || 0}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="flex items-end">
                <span className={`text-4xl font-bold ${
                  (dashboardData.accessibilityStats?.averageScore || 0) >= 90 ? 'text-green-600' : 
                  (dashboardData.accessibilityStats?.averageScore || 0) >= 70 ? 'text-yellow-600' : 
                  'text-red-600'
                }`}>
              ...
              <div className="mt-2 text-sm text-gray-500">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-red-600">{dashboardData.accessibilityStats?.criticalIssuesCount || 0}</span>
                </div>
                <div className="flex justify-between">
                  <span>Problèmes sérieux:</span> 
                  <span className="font-medium text-orange-500">{dashboardData.accessibilityStats?.seriousIssuesCount || 0}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="flex items-end">
                <span className={`text-4xl font-bold ${
                  (dashboardData.bestPracticesStats?.averageScore || 0) >= 90 ? 'text-green-600' : 
                  (dashboardData.bestPracticesStats?.averageScore || 0) >= 70 ? 'text-yellow-600' : 
                  'text-red-600'
                }`}>
              ...
              <div className="mt-2 text-sm text-gray-500">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-red-600">{dashboardData.bestPracticesStats?.highSeverityCount || 0}</span>
                </div>
                <div className="flex justify-between">
                  <span>Problèmes moyens:</span> 
                  <span className="font-medium text-yellow-600">{dashboardData.bestPracticesStats?.mediumSeverityCount || 0}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-4">
                {Object.entries(filteredResults).slice(0, 5).map(([filePath, result]) => (
                    ...
                    <div key={filePath} className="border-b pb-3">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div>
                          ...
                          <span>{result.performanceMetrics.firstContentfulPaint?.toFixed(1) || 'N/A'} s</span>
                        </div>
                        <div>
                          <span className="text-gray-600">TTI: </span>
                          <span>{result.performanceMetrics.timeToInteractive?.toFixed(1) || 'N/A'} s</span>
                        </div>
                        <div>
                          <span className="text-gray-600">LCP: </span>
                          <span>{result.performanceMetrics.largestContentfulPaint?.toFixed(1) || 'N/A'} s</span>
                        </div>
                        <div>
                          <span className="text-gray-600">CLS: </span>
                          <span>{result.performanceMetrics.cumulativeLayoutShift?.toFixed(3) || 'N/A'}</span>
                        </div>
                      </div>
                    </div>
                  )
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-2">
                {Object.entries(filteredResults).slice(0, 3).map(([filePath, result]) => (
                    ...
                    <div key={filePath} className="border-b pb-3">
                      ...
                      <ul className="text-sm space-y-1">
                        ...
                        {result.accessibilityIssues.length > 3 && (
                          <li className="text-sm text-gray-500">+ {result.accessibilityIssues.length - 3} autres problèmes</li>
                        )}
                      </ul>
                    </div>
                  )
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sévérité</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {Object.values(dashboardData.results)
                    .filter(result => result.bestPracticesIssues && result.bestPracticesIssues.length > 0)
                    .flatMap(result => result.bestPracticesIssues)
                    .slice(0, 10)
                    .map((issue, idx) => (
                      <tr key={idx}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{issue.category}</td>
          ...
          {qualityIssues.length > 0 ? (
            <>
              <div className="mb-6">
                <p className="text-gray-600">
                  Voici les recommandations pour améliorer la qualité de votre site sur les critères SEO, performance, accessibilité et bonnes pratiques.
                  Ces recommandations sont classées par ordre de priorité et par catégorie.
                </p>
              </div>
              ...
              <div className="mt-6 pt-6 border-t border-gray-200">
                ...
                <div className="bg-gray-50 p-4 rounded">
                  ...
                  <p className="text-sm text-gray-600 mb-3">
                    Certaines corrections peuvent être appliquées de manière automatique par le système. Cliquez sur le bouton ci-dessous pour lancer le processus de correction automatique.
                  </p>
                  <button
                    type="button"
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-gray-500">
                Aucune recommandation n'a été détectée. Votre site suit déjà les meilleures pratiques.
              </p>
            </div>
          )}
        </div>
                ...
                <table className="min-w-full divide-y divide-gray-300">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Score moyen</th>
                      <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Succès</th>
                      <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Partiel</th>
                      <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Échec</th>
                    </tr>
                  </thead>
            ...
            <div>
                ...
                <div key={index} className="mb-8">
                  ...
                  <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                    <table className="min-w-full divide-y divide-gray-300">
                      <thead className="bg-gray-50">
                        <tr>
                          <th scope="col" className="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-6">Fichier</th>
                      ...
                      <tbody className="divide-y divide-gray-200 bg-white">
                        {issue.affectedFiles.slice(0, 10).map((file, fileIndex) => (
                          <tr key={fileIndex}>
                            ...
                            <td className="whitespace-nowrap px-3 py-4 text-sm">
                              <span className={`font-medium ${
                                (dashboardData.results[file]?.score || 0) >= 90 ? 'text-green-600' :
                                (dashboardData.results[file]?.score || 0) >= 70 ? 'text-yellow-600' :
                                'text-red-600'
                              }`}>
              ...
              <div className="mt-8 border-t border-gray-200 pt-6">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Les problèmes listés ci-dessus peuvent être corrigés automatiquement par notre outil d'autofix. 
                  Cliquez sur le bouton ci-dessous pour lancer la correction automatique des champs manquants.
                </p>
