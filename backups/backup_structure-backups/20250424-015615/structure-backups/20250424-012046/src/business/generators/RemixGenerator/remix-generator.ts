...
import { AgentResult, MigrationConfig, PhpAnalysisResult } from ../typesstructure-agent';
...
export default function ${this.capitalize(analysisResult.fileName)}() {
  ...
  private async generateMetaFile(sourceContent: string, analysisResult: PhpAnalysisResult): Promise<string> {
    ...
    return `import type { MetaFunction } from @remix-run/nodestructure-agent";
import { loader } from ./${analysisResult.fileName}.loaderstructure-agent";
...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  ...
  private async generateLoaderFile(sourceContent: string, analysisResult: PhpAnalysisResult): Promise<string> {
    ...
    return `import { json, LoaderFunctionArgs } from @remix-run/nodestructure-agent";
import { db } from ~/lib/db.serverstructure-agent";
import { NotFoundError } from ~/lib/errorsstructure-agent";
...
export async function loader({ params, request }: LoaderFunctionArgs) {
  ...
  private async generateSchemaFile(analysisResult: PhpAnalysisResult): Promise<string> {
    ...
    return `import { z } from zodstructure-agent";
...
`;
  ...
  private generateImports(analysisResult: PhpAnalysisResult): string {
    const imports = [
      'import { useLoaderData } from @remix-run/react";structure-agent',
      'import { Link, Form } from @remix-run/react";structure-agent',
      `import { loader } from ./${analysisResult.fileName}.loaderstructure-agent";`,
    ];
    ...
    if (hasFormElements) {
      imports.push('import { useActionData } from @remix-run/react";structure-agent');
    }
  ...
  private generateLocalState(analysisResult: PhpAnalysisResult): string {
    ...
    if (localStateCode.length > 0) {
      ...
      localStateCode.unshift('import { useState } from react";structure-agent');
