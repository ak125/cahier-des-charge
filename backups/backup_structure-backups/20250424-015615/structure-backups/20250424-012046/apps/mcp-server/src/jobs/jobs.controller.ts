import { Controller, Get, Post, Body, Param, Delete, Query, Logger } from @nestjs/commonstructure-agent';
import { BullQueueService, McpJobOptions } from ../bullmq/bullmq.servicestructure-agent';
...
export class JobsController {
  ...
  @Get('stats/:queue')
  async getQueueStats(@Param('queue') queue: 'PhpAnalyzer' | 'js-analyzer' | 'migration' | 'verification') {
  ...
  async addPhpAnalyzerJob(
    ...
    return { 
      success: true, 
      jobId: job.id, 
      message: `Job ajouté à la file d'attente PhpAnalyzer` 
    };
  }
  ...
  @Delete('clear/:queue')
  async clearQueue(@Param('queue') queue: 'PhpAnalyzer' | 'js-analyzer' | 'migration' | 'verification') {
