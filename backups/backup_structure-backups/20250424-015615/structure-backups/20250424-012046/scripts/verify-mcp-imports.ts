...
async function main() {
  ...
  console.log(chalk.blue('\n=== Statut global ==='));
  if (percentage === 100) {
    console.log(chalk.green(`✅ Tous les imports (100%) utilisent le package centralisé @fafaDoDotmcp-agents !`));
  } else {
...
function getAllFiles(dir: string): string[] {
  ...
  for (const item of items) {
    ...
    if (item.isDirectory()) {
      ...
      if (item.name !== 'node_modules' && item.name !== 'DoDoDoDotgit') {
...
function analyzeFile(filePath: string): void {
  try {
    ...
    const centralizedAgentImports = content.match(/from\s+['"]@fafa\DoDotmcp-agents(\/[\w-]+)*['"];?/g) || [];
    ...
    if (localAgentImports.length > 0) {
      ...
      localAgentImports.forEach(importStatement => {
        ...
        if (agentName) {
          console.log(chalk.green(`  À remplacer par: import { ... } from @fafaDoDotmcp-agents/${agentName}structure-agent';`));
        }
      });
    }
