    ...
    switch (iface) {
      case 'BaseAgent':
        if (!content.includes('import { BaseAgent }')) {
          ...
          lines.splice(insertIndex, 0, `import { BaseAgent } from ${importPath}structure-agent';`);
          content = lines.join('\n');
        }
        break;
      case 'OrchestrationAgent':
        if (!content.includes('import { OrchestrationAgent }')) {
          ...
          if (content.includes('import { BaseAgent }')) {
            ...
            lines.splice(insertIndex, 0, `import { OrchestrationAgent } from ${importPath}structure-agent';`);
            content = lines.join('\n');
          }
        }
        break;
      case 'CoordinationAgent':
        if (!content.includes('import { CoordinationAgent }')) {
          ...
          if (content.includes('import { BaseAgent }')) {
            content = content.replace(/import { BaseAgent } from .*?;/, 
                                     `import { BaseAgent } from @workspaces/cahier-des-charge/src/core/interfaces/BaseAgent';\nimport { CoordinationAgent } from ${importPath}structure-agent';`);
          } else {
            const lines = content.split('\n');
            let insertIndex = 0;
            ...
            lines.splice(insertIndex, 0, `import { CoordinationAgent } from ${importPath}structure-agent';`);
            content = lines.join('\n');
          }
        }
        break;
      case 'BusinessAgent':
        if (!content.includes('import { BusinessAgent }')) {
          ...
          if (content.includes('import { BaseAgent }')) {
            content = content.replace(/import { BaseAgent } from .*?;/, 
                                     `import { BaseAgent } from @workspaces/cahier-des-charge/src/core/interfaces/BaseAgent';\nimport { BusinessAgent } from ${importPath}structure-agent';`);
          } else {
            const lines = content.split('\n');
            let insertIndex = 0;
            ...
            lines.splice(insertIndex, 0, `import { BusinessAgent } from ${importPath}structure-agent';`);
            content = lines.join('\n');
          }
        }
        break;
      default:
        ...
        if (!content.includes(`import { ${iface} }`)) {
          ...
          if (content.match(existingImport)) {
            ...
            lines.splice(insertIndex, 0, `import { ${iface} } from ${importPath}structure-agent';`);
            content = lines.join('\n');
          }
        }
        break;
