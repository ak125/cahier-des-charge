...
try {
  ...
  pipelineData.workflows.forEach((workflow, index) => {
    ...
    try {
      ...
      console.error(`❌ Erreur lors de l'extraction du workflow "${workflowName}": ${error.message}`);
    }
  });
  ...
  console.log(`   4. Sélectionnez "Import from file" et choisissez un fichier depuis: ${OUTPUT_DIR}`);
