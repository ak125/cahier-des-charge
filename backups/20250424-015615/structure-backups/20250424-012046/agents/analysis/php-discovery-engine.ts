 ...
 * Fournit des fonctions d'analyse détaillée et de scoring
 */
...
function determineFileRole(content: string, filePath: string): string {
  ...
  if (content.includes('function') && content.includes('SELECT') && 
      content.includes('FROM') && content.includes('WHERE')) {
    return 'data_access';
  }
...
function estimateBusinessImpact(content: string, sqlTables: string[], role: string): number {
  ...
  const criticalTables = [
    ...
    'ORDER', 'ORDERS', 'COMMANDE', 'COMMANDES',
    'CART', 'PANIER', 'CUSTOMER', 'CLIENT'
  ];
  
  const criticalTablesFound = sqlTables.filter(table => 
  ...
  const businessKeywords = [
    'payment', 'paiement', 'checkout', 'cart', 'panier',
    'order', 'commande', 'client', 'compte', 'account',
    'password', 'mot de passe', 'login', 'connexion'
  ];
  
  const lowerContent = content.toLowerCase();
  const keywordsFound = businessKeywords.filter(keyword => 
...
function estimateSeoImpact(content: string): number {
  ...
  if (content.includes('301') && content.includes('header(') || 
  ...
  const seoKeywords = [
    'seo', 'sitemap', 'google', 'robots', 'noindex', 'nofollow',
    'canonical', 'redirect', 'meta', 'description', 'keywords'
  ];
  
  const lowerContent = content.toLowerCase();
  const keywordsFound = seoKeywords.filter(keyword => 
    lowerContent.includes(keyword)
  ).length;
...
export function calculatePriorityScore(
  ...
  const trafficScore = Math.min(
    (trafficData.hits || 0) / 10000 + 
    (trafficData.searchBots || 0) / 100, 
    1
  );
  ...
  const priorityScore = 
    (complexityScore * weights.complexity +
     ...
     trafficScore * weights.traffic) * 10;
...
import { BusinessAgent } from @workspaces/cahier-des-charge/src/core/interfaces/businessstructure-agent';
