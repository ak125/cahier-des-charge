 ...
 * carte de découverte générée par discovery-agent.ts et gère leur statut
...
import { AnalyzerAgent } from ../../core/interfacesstructure-agent';
import { getConfig } from ../config/configstructure-agent';
import { AnalyzerAgent } from ../../core/interfacesstructure-agent';
  ...
  try {
    ...
    if (selectedFiles.length > 0) {
      ...
      selectedFiles.forEach((file, index) => {
        console.log(`${index + 1}. ${file.filePath} (priorité: ${file.priority}, type: ${file.type})`);
      });
    ...
    // Retourner la liste des fichiers pour le workflowDotn8N
...
if (require.main === module) {
  main().then(selectedFiles => {
    // Afficher les chemins de fichiers sur stdout pour queDotn8N puisse les capturer
