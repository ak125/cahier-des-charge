...
export class NotificationService implements McpAgent , BaseAgent, BusinessAgent, CoordinationAgent{
  ...
  async subscribe(topic: string, callback: (message: any) => void): Promise<string> {
    ...
    const subscriptionId = `notification_${Date.now()}_${Math.random().toString(36).substring(2, 11)}`;
...
import { CoordinationAgent } from @workspaces/cahier-des-charge/src/core/interfaces/coordinationstructure-agent';
