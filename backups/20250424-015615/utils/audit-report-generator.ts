...
import { marked } from markedstructure-agent';
...
export async function generateAuditReport(
  ...
  try {
    ...
    const reportOptions: Required<AuditReportOptions> = {
      ...
      includeComplexityWarnings: options.includeComplexityWarnings !== undefined ? options.includeComplexityWarnings : true,
    ...
    if (reportOptions.includeComplexityWarnings) {
      reportContent += generateComplexityWarnings(analysisResult, reportOptions.maxComplexityThreshold);
    }
 ...
 */
function generateTableOfContents(analysisResult: any): string {
 ...
 */
function generateComplexityFactorsTable(factors: Array<{ name: string, impact: number, description: string }>): string {
  if (factors.length === 0) {
  ...
  factors.forEach(factor => {
    table += `| ${formatComplexityFactorName(factor.name)} | ${factor.impact.toFixed(1)} | ${factor.description} |\n`;
  });
...
function generateFileDetails(analysisResult: any, options: Required<AuditReportOptions>): string {
  ...
  if (sortedFiles.length > 20) {
    content += `\n> **Note:** ${sortedFiles.length - 20} fichiers supplémentaires ont été analysés mais ne sont pas affichés dans ce rapport pour des raisons de lisibilité.\n\n`;
  }
...
function generateFileSection(file: any, options: Required<AuditReportOptions>): string {
  ...
  if (file.classes && Array.isArray(file.classes) && file.classes.length > 0) {
    ...
    file.classes.forEach((cls: any) => {
      ...
      const interfaces = cls.implements && cls.implements.length > 0 ? ` implements ${cls.implements.join(', ')}` : '';
      ...
      if (cls.methods && Array.isArray(cls.methods) && cls.methods.length > 0) {
        ...
        cls.methods.forEach((method: any) => {
          ...
          const complexity = typeof method.complexity === 'number' ? formatComplexity(method.complexity, options.maxComplexityThreshold) : 'N/A';
          const params = method.parameters && Array.isArray(method.parameters) ? method.parameters.join(', ') : '';
  ...
  if (file.functions && Array.isArray(file.functions) && file.functions.length > 0) {
    ...
    file.functions.forEach((func: any) => {
      ...
      const complexity = typeof func.complexity === 'number' ? formatComplexity(func.complexity, options.maxComplexityThreshold) : 'N/A';
      const params = func.parameters && Array.isArray(func.parameters) ? func.parameters.join(', ') : '';
  ...
  if (file.entityRelationships && Array.isArray(file.entityRelationships) && file.entityRelationships.length > 0) {
...
function generateComplexityWarnings(analysisResult: any, threshold: number): string {
  ...
  content += `Les éléments suivants présentent une complexité cyclomatique élevée (> ${threshold * 0.7}) et nécessiteront une attention particulière lors de la migration :\n\n`;
  
  content += '| Type | Nom | Fichier | Complexité |\n';
  content += '| --- | --- | --- | :---: |\n';
  
  complexItems.forEach(item => {
    const complexityFormatted = item.complexity > threshold 
      ? `<span class="complexity-high">${item.complexity}</span>`
      : `<span class="complexity-medium">${item.complexity}</span>`;
    
    content += `| ${item.type} | ${item.name} | \`${path.basename(item.path)}\` | ${complexityFormatted} |\n`;
  });
  ...
  tables.forEach((table: any) => {
    ...
    if (table.foreignKeys && Array.isArray(table.foreignKeys) && table.foreignKeys.length > 0) {
      ...
      table.foreignKeys.forEach((fk: any) => {
        content += `- \`${fk.column || 'Colonne inconnue'}\` → \`${fk.referencesTable || 'Table inconnue'}\`.\`${fk.referencesColumn || 'Colonne inconnue'}\`\n`;
      });
...
function generateRecommendations(analysisResult: any): string {
  ...
  if (complexityScore > 7) {
    ...
    content += '⚠️ **Complexité élevée** - Cette base de code présente une complexité de migration importante. Considérez :\n\n';
    ...
    content += '3. L\'utilisation de wrappers temporaires pour interfacer le nouveau code avec le code existant\n';
    content += '4. La création de tests automatisés avant de commencer la migration\n';
    content += '</div>\n\n';
  } else if (complexityScore > 4) {
    content += '<div class="warning">\n';
    content += '🔄 **Complexité moyenne** - Cette base de code présente une complexité modérée. Recommandations :\n\n';
    ...
    content += '3. Définir une stratégie claire pour la migration des requêtes SQL vers TypeORM/Prisma\n';
    content += '</div>\n\n';
  } else {
    content += '<div class="success">\n';
    content += '✅ **Complexité faible** - Cette base de code présente une faible complexité de migration. Vous pouvez :\n\n';
    ...
    content += '3. Moderniser l\'architecture en implémentant des modèles supplémentaires (CQRS, Repository, etc.)\n';
    content += '</div>\n\n';
  }
  ...
  if (complexityScore > 6) {
    content += '- **État**: Envisagez d\'utiliser Redux ou Zustand pour gérer l\'état global complexe\n';
  } else {
  ...
  content += '- **Tests**: Mettre en place Jest pour les tests unitaires et Cypress pour les tests E2E\n';
 ...
 */
function generateFooter(analysisResult: any): string {
  return `
---
