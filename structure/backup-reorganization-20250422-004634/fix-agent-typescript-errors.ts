...
async function main() {
  ...
  if (architectureInfo) {
    ...
    console.log(`   - ${architectureInfo.structure.layers.length} couches d'architecture identifiées`);
    console.log(`   - ${architectureInfo.structure.agentTypes.length} types d'agents reconnus`);
    console.log(`   - ${Object.keys(architectureInfo.conventions.naming).length} conventions de nommage extraites`);
  } else {
  ...
  console.log(`🔍 Recherche des fichiers d'agents à corriger${options.dryRun ? ' (mode simulation)' : ''}...`);
  ...
  for (const filePath of agentFiles) {
    try {
      ...
      if (wasFixed) {
        ...
        if (options.verbose) {
          console.log(`✅ ${options.dryRun ? 'Serait corrigé' : 'Corrigé'}: ${path.relative(process.cwd(), filePath)}`);
        }
      } else {
      ...
      stats.failed++;
      const errorMessage = `❌ Erreur sur ${path.relative(process.cwd(), filePath)}: ${error.message}`;
      if (options.verbose) {
      ...
      if (options.revertOnFailure && !options.dryRun && backups.has(filePath)) {
        try {
          ...
          console.log(`↩️ Restauration de la sauvegarde pour ${path.relative(process.cwd(), filePath)}`);
        } catch (restoreErr) {
          console.error(`⚠️ Impossible de restaurer la sauvegarde pour ${path.relative(process.cwd(), filePath)}: ${restoreErr.message}`);
        }
      }
    }
  }
...
async function parseArchitectureDocument(): Promise<ArchitectureInfo | null> {
  try {
    ...
    const layersRegex = /##\s+📊\s*Architecture\s+à\s+Trois\s+Couches[^\n]*\s*\n([\s\S]*?)(?=\n##|$)/i;
    ...
    if (layersMatch) {
      ...
      const coordinationLayerRegex = /Couche\s+[dD]e\s+Coordination[^-]*-\s*([\s\S]*?)(?=\d+\.\s+\*\*Couche|$)/i;
    ...
    while ((agentTypeMatch = agentTypesRegex.exec(content)) !== null) {
      ...
      if (!architectureInfo.structure.agentTypes.includes(agentType) && 
          agentType.length > 2 && 
    ...
    if (architectureInfo.structure.agentTypes.length === 0) {
      architectureInfo.structure.agentTypes = ['Analyzer', 'Generator', 'Orchestrator', 'Validator'];
...
async function fixAgentFile(
  ...
  let newContent = generateCorrectImplementation(content, agentName, interfaceName, architectureInfo);
 ...
 */
function determineAgentType(filePath: string, architectureInfo: ArchitectureInfo | null = null): string {
...
function generateCorrectImplementation(
  ...
  const hasInterfaceImport = imports.some(imp => 
    imp.includes(interfaceName) || imp.includes('Agent')
  );
  ...
  if (!hasInterfaceImport) {
    const importPath = architectureInfo?.conventions.naming?.imports || 
                       '../../interfaces';
    
    newImports += `\nimport { ${interfaceName} } from '${importPath}/${interfaceName.toLowerCase()}';\n`;
  }
 ...
 */
function getLayerForAgentType(agentType: string, architectureInfo: ArchitectureInfo | null): string {
  ...
  if (agentType.includes('Orchestrator') && architectureInfo.structure.layers.includes('Coordination')) {
    return 'Coordination';
  }
  
  if ((agentType.includes('Adapter') || agentType.includes('Connector')) && 
...
async function updateArchitectureDocument(
  ...
  try {
    if (!fs.existsSync(ARCHITECTURE_MD_PATH)) {
      console.warn(`⚠️ Le fichier ARCHITECTURE.md n'a pas été trouvé à l'emplacement: ${ARCHITECTURE_MD_PATH}`);
      return;
    }
...
|-------------|-----|-----------|--------|--------|
${Object.entries(agentsByType).flatMap(([type, agents]) => 
  agents.map(agent => {
    const relativePath = agent;
...
function generateReport(
  ...
  const architectureSummary = architectureInfo 
    ? `\n\n### Couches d'architecture identifiées\n\n${architectureInfo.structure.layers.map(layer => `- ${layer}`).join('\n')}`
    : '';
...
- **Corrigés**: ${stats.fixed} fichiers
- **Statut**: ${tscSuccess && stats.failed === 0 ? '✅ Tous les problèmes ont été résolus' : '⚠️ Des problèmes persistent'}

${stats.fixed > 0 ? '### Agents corrigés\n\n' + stats.fixedAgents.slice(0, 5).map(a => `- \`${a}\``).join('\n') + (stats.fixedAgents.length > 5 ? `\n- et ${stats.fixedAgents.length - 5} autres...` : '') : ''}
${stats.errors.length > 0 ? '### Erreurs\n\n' + stats.errors.slice(0, 3).map(e => `- ${e}`).join('\n') + (stats.errors.length > 3 ? `\n- et ${stats.errors.length - 3} autres...` : '') : ''}
