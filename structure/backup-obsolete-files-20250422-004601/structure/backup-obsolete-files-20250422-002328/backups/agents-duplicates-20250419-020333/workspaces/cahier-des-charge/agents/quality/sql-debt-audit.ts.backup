...
const PATTERNS = {
  REDUNDANT_TIMESTAMPS: [
    'created_on', 'created_at', 'date_created', 
    'updated_on', 'updated_at', 'last_update'
  ],
  GENERIC_COLUMNS: [
    'data', 'data1', 'data2', 'json_blob', 'json_data',
    'extra', 'additional_data', 'metadata', 'config',
    'settings', 'properties', 'attributes', 'params'
  ],
  STATUS_DUPLICATES: [
    'status', 'status_code', 'status_txt', 'status_description'
  ],
  OBSOLETE_PREFIX: [
    'old_', 'legacy_', 'temp_', 'tmp_', 'deprecated_', 'test_'
  ],
  CATCH_ALL_TABLES: [
    'config', 'settings', 'params', 'properties', 'metadata',
    'core_param', 'config_all_in_one', 'setup_globals', 'misc'
  ],
  SOFT_DELETE_FLAGS: [
    'deleted', 'is_deleted', 'deleted_at', 'deleted_flag', 'active'
  ],
  INCONSISTENT_NAMING: {
...
async function main() {
  program
    ...
    .option('--format <format>', 'Format de sortie (json, md, ou les deux séparés par une virgule)', 'json,md')
  ...
  if (auditReport.summary.overall_debt_score > threshold) {
    console.log(chalk.red(`⚠️ ALERTE: Score de dette technique (${auditReport.summary.overall_debt_score.toFixed(1)}) supérieur au seuil (${threshold})`));
    ...
    console.log(chalk.green(`✅ Score de dette technique (${auditReport.summary.overall_debt_score.toFixed(1)}) en dessous du seuil (${threshold})`));
  }
}
...
function analyzeSchema(schema: any): SQLDebtReport {
  ...
  const obsoleteTables = tables.filter(table => {
    return PATTERNS.OBSOLETE_PREFIX.some(prefix => table.startsWith(prefix)) || 
  ...
  const catchAllTablesFound = tables.filter(table => {
    return PATTERNS.CATCH_ALL_TABLES.some(pattern => 
      table.includes(pattern) || table === pattern
    );
  });
  ...
  tables.forEach(table => {
    ...
    const jsonTextCols = columns.filter(col => 
    ...
    const inappropriateTypes = columns.filter(col => {
      ...
      if ((col.name.startsWith('is_') || col.name.endsWith('_flag')) && 
          (col.type === 'INT' || col.type === 'TINYINT') && col.length === 1) return true;
      return false;
    });
    ...
    if (inappropriateTypes.length > 0) {
      ...
      inappropriateTypesColumns.push(
        ...inappropriateTypes.map(c => `${table}.${c.name} (${c.type} au lieu de ${getAppropriateType(c)})`)
      );
    ...
    columns.forEach(col => {
      ...
      // Identifiants
      if (col.name === 'id' || col.name.endsWith('_id') || col.name.endsWith('_uuid') || 
          col.name.endsWith('_code') || col.name.endsWith('_number')) {
    ...
    if (idColumns.length > 2) { // Permettre une clé primaire + une clé externe
  ...
  const multiPurposeScore = Math.min(10, (multiPurposeColumns / Math.max(1, totalColumns * 0.05)) * 4);
  const poorDesignScore = Math.min(10, (poorSchemaDesign / Math.max(1, tables.length * 0.2)) * 5);
  
  const overallScore = (
    redundantScore * 0.2 + 
    unusedScore * 0.1 + 
    multiPurposeScore * 0.4 + 
    poorDesignScore * 0.3
  );
...
function generateRecommendations(report: SQLDebtReport): void {
  ...
  if (report.poor_schema_design.some(issue => 
  ...
  if (report.redundant_fields.some(issue => issue.pattern === "Champs d'état textuels avec codes numériques")) {
  ...
  if (report.multi_purpose_columns.some(issue => issue.pattern === "Colonnes JSON non structurées")) {
    report.detailed_recommendations.medium_term.push(
      "Migrer les colonnes JSON stockées comme TEXT vers le type JSON/JSONB natif"
    );
  }
  
  if (report.multi_purpose_columns.some(issue => issue.pattern === "Colonnes génériques de données")) {
    report.detailed_recommendations.medium_term.push(
...
function getAppropriateType(column: any): string {
  if (column.name.includes('price') || column.name.includes('amount') || column.name.includes('cost')) {
    return 'DECIMAL';
  }
...
function generateMarkdownReport(report: SQLDebtReport): string {
  ...
  markdown += `Notre analyse a identifié plusieurs patterns récurrents de dette technique dans le schéma de base de données. L'indice global de dette technique est de **${report.summary.overall_debt_score}/10** (niveau ${report.summary.risk_level.toLowerCase()}), avec ${report.summary.tables_analyzed} tables et ${report.summary.columns_analyzed} colonnes analysées.\n\n`;
  ...
  if (report.multi_purpose_columns.length > 0) {
    ...
    if (criticalIssue) {
      ...
      markdown += `**Cas critique** : la table \`${report.multi_purpose_columns[0].affected_tables?.[0]}\` utilise ${report.multi_purpose_columns[0].pattern.toLowerCase()}.\n\n`;
    }
  }
  ...
  if (report.poor_schema_design.length > 0) {
    ...
    const foreignKeyIssue = report.poor_schema_design.find(i => i.pattern === "Clés étrangères manquantes");
    if (foreignKeyIssue) {
    ...
    // Types inappropriés
    const typeIssue = report.poor_schema_design.find(i => i.pattern === "Champs de type inapproprié");
    if (typeIssue) {
