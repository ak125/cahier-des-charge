...
interface ExtractedSQLQuery {
  ...
  columns: Array<{        // Colonnes référencées dans la requête
    table: string;
    column: string;
  }>;
...
export function extractSQLQueries(auditContent: string): ExtractedSQLQuery[] {
  ...
  while ((sqlSectionMatch = sqlSectionRegex.exec(auditContent)) !== null) {
  ...
  while ((codeBlockMatch = codeBlockRegex.exec(auditContent)) !== null) {
 ...
 */
function extractQueriesFromSection(
  codeSection: string, 
  queries: ExtractedSQLQuery[]
): void {
...
function extractConcatenatedQuery(lines: string[], startIndex: number): string | null {
  ...
  let currentLine = lines[startIndex].trim();
...
function analyzeQuery(
  ...
  if (query.includes('SELECT')) {
    ...
    if (fromMatch) {
      ...
      tablesPart.forEach(tablePart => {
        const tableMatch = tablePart.trim().match(/([a-zA-Z0-9_]+)(?:\s+(?:as\s+)?([a-zA-Z0-9_]+))?/i);
        if (tableMatch) {
    ...
    if (selectMatch) {
      ...
      if (columnsPart.trim() === '*') {
        ...
        columnItems.forEach(columnItem => {
          ...
          const tableColumnMatch = trimmedItem.match(/([a-zA-Z0-9_]+)\.([a-zA-Z0-9_*]+)(?:\s+(?:as\s+)?([a-zA-Z0-9_]+))?/i);
          if (tableColumnMatch) {
            columns.push({ table: tableColumnMatch[1], column: tableColumnMatch[2] });
          } else {
            // Colonne sans alias de table, utiliser la première table
            const columnMatch = trimmedItem.match(/([a-zA-Z0-9_*]+)(?:\s+(?:as\s+)?([a-zA-Z0-9_]+))?/i);
