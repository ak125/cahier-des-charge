...
export const devIntegratorAgent = {
  ...
  async checkConflits(elementPath: string, targetBranch: string): Promise<string[]> {
    try {
      ...
      try {
        ...
        execSync(`git checkout -b ${tempBranch}`, { encoding: 'utf8' });
        
        // Tenter une fusion de la branche cible dans notre branche temporaire
        execSync(`git merge origin/${targetBranch} --no-commit --no-ff`, { encoding: 'utf8' });
        ...
        execSync(`git checkout ${currentBranch}`, { encoding: 'utf8' });
        execSync(`git branch -D ${tempBranch}`, { encoding: 'utf8' });
  ...
  generateRemediationSuggestions({ lintIssues, typeIssues, buildIssues, missingDeps, conflits }: {
    ...
    if (conflits.length > 0) {
      suggestions.push(`Résoudre les conflits git dans les fichiers: ${conflits.join(', ')}`);
    }
