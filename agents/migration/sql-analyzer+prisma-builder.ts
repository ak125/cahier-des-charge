...
import { TypeConverter } from '../analysis/core/type-converter';
import { SchemaAnalyzer } from '../analysis/core/schema-analyzer';
import { PrismaGenerator } from '../analysis/core/prisma-generator';
import { RelationAnalyzer } from '../analysis/core/relation-analyzer';
import { DebtAnalyzer } from '../analysis/core/debt-analyzer';
...
program
  ...
  .option('--output-dir <dir>', 'Dossier de sortie', `./reports/sql-audit-${new Date().toISOString().replace(/:/g, '-').split('.')[0]}`)
...
const analyzePerformance = options.analyzePerformance === 'true' || options.analyzePerformance === true;
const includeTables = options.includeTables ? options.includeTables.split(',') : [];
const excludeTables = options.excludeTables ? options.excludeTables.split(',') : ['migrations', 'schema_migrations', 'ar_internal_metadata'];
...
async function main() {
  try {
    ...
    if (!connectionString) {
      ...
      console.error(chalk.yellow('Utilisez --connection=<url> ou définissez la variable d\'environnement DATABASE_URL'));
      process.exit(1);
    }
    ...
    try {
      ...
      const schemaWithRelations = await relationAnalyzer.analyze(analyzedSchema, connection, databaseName);
      ...
      const { schema: analyzedSchemaWithDebt, issues: debtIssues } = await debtAnalyzer.analyze(convertedSchema);
      ...
      if (analyzePerformance) {
        ...
        performanceResults = await performanceAnalyzer.analyze(connection, databaseName, analyzedSchemaWithDebt);
      }
      ...
      saveToJson(
        path.join(outputDir, 'mysql_schema_map.json'), 
        {
          version: '1.0.0',
          ...
          analyses: {
            dataTypes: {
              ...
              optimizationPotential: debtIssues.filter(issue => issue.type === 'DATA_TYPE_OPTIMIZATION')
            },
            indexes: {
              missing: performanceResults.indexSuggestions.filter(suggestion => suggestion.type === 'MISSING'),
              redundant: performanceResults.indexSuggestions.filter(suggestion => suggestion.type === 'REDUNDANT')
            },
            constraints: {
      ...
      // 5. migration_plan.md
      if (generatePrisma) {
        const migrationPlanMd = generateMigrationPlanMarkdown(migrationDiff, analyzedSchemaWithDebt);
      ...
      if (analyzePerformance) {
        const indexSuggestionsSql = generateIndexSuggestionsSQL(performanceResults.indexSuggestions);
      ...
      if (analyzedSchemaWithDebt.phpMappings && Object.keys(analyzedSchemaWithDebt.phpMappings).length > 0) {
      ...
      printSummary(analyzedSchemaWithDebt, stats, debtIssues.length, performanceResults.indexSuggestions.length);
 ...
 */
async function getDatabaseCollation(connection: mysql.Connection, database: string): Promise<string> {
  const [rows] = await connection.query(`
...
function generateSqlAnalysisMarkdown(
  ...
  markdown += `*Généré le ${new Date().toLocaleDateString('fr-FR')} à ${new Date().toLocaleTimeString('fr-FR')}*\n\n`;
  ...
  for (const issueType of sortedIssueTypes) {
    const severityIcon = issueType.severity === 'high' ? '🔴' : issueType.severity === 'medium' ? '🟠' : '🟡';
    markdown += `### ${severityIcon} ${issueType.type} (${issueType.issues.length})\n\n`;
    
    // Regrouper par table pour une meilleure lisibilité
    const issuesByTable = issueType.issues.reduce((acc, issue) => {
    ...
    for (const [tableName, tableIssues] of Object.entries(issuesByTable)) {
      ...
      for (const issue of tableIssues) {
        markdown += `- **${issue.columnName ? `Colonne \`${issue.columnName}\`` : 'Table'}**: ${issue.message}\n`;
  ...
  const typeIssues = issues.filter(issue => issue.type === 'DATA_TYPE_INCONSISTENCY' || issue.type === 'DATA_TYPE_OPTIMIZATION');
  ...
  if (typeIssues.length > 0) {
    ...
    for (const issue of typeIssues) {
      const currentType = schema.tables[issue.tableName]?.columns[issue.columnName as string]?.type || 'N/A';
      markdown += `| ${issue.tableName} | ${issue.columnName} | \`${currentType}\` | ${issue.message} | ${issue.recommendation || 'N/A'} |\n`;
    }
  } else {
  ...
  if (namingIssues.length > 0) {
    ...
    for (const issue of namingIssues) {
      markdown += `| ${issue.tableName} | ${issue.columnName} | ${issue.message} | ${issue.recommendation || 'N/A'} |\n`;
    }
  } else {
  ...
  if (schema.relationships && schema.relationships.length > 0) {
    ...
    for (const relation of schema.relationships) {
      markdown += `| ${relation.name || 'N/A'} | ${relation.type} | ${relation.sourceTable} | ${relation.sourceColumn} | ${relation.targetTable} | ${relation.targetColumn} | ${relation.isMandatory ? 'Oui' : 'Non'} |\n`;
    }
  } else {
 ...
 */
function generateMigrationPlanMarkdown(
  migrationDiff: any,
  schema: MySQLSchema
): string {
  ...
  markdown += `- [Outil pgloader pour la migration de données](https://github.com/dimitri/pgloader)\n`;
...
function generateIndexSuggestionsSQL(indexSuggestions: IndexSuggestion[]): string {
  ...
  const suggestionsByTable = indexSuggestions.reduce((acc, suggestion) => {
...
function printSummary(
  ...
  if (stats.largestTable) {
    console.log(chalk.cyan(`- Table la plus volumineuse: ${stats.largestTable.name} (${stats.largestTable.sizeInMB} MB)`));
  }
  
  console.log(chalk.cyan('\n🚀 Prochaines étapes:'));
  console.log(chalk.cyan('1. Examiner le rapport d\'audit SQL (sql_analysis.md)'));
  console.log(chalk.cyan('2. Vérifier les modèles Prisma générés (prisma_models.suggestion.prisma)'));
  console.log(chalk.cyan('3. Suivre le plan de migration (migration_plan.md)'));
  console.log(chalk.cyan('4. Appliquer les suggestions d\'optimisation (sql_index_suggestions.sql)'));
}
