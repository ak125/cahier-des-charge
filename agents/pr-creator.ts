...
export class PRCreator {
  ...
  constructor(githubToken: string, config: PullRequestConfig) {
  ...
  private createLocalBranch(branch: string): void {
    try {
      ...
      execSync(`git checkout ${this.config.base}`, { stdio: 'inherit' });
      execSync(`git pull origin ${this.config.base}`, { stdio: 'inherit' });
  ...
  private async addFilesToBranch(branch: string, files: FileData[]): Promise<void> {
    try {
      ...
      // Créer un commit
      execSync(`git commit -m "Migration: Fichiers générés par MCP"`, { stdio: 'inherit' });
    } catch (error) {
...
export async function runPRCreator(contextOrPath: string | JobContext): Promise<string> {
  ...
  try {
    ...
    const labels = process.env.GITHUB_LABELS ? process.env.GITHUB_LABELS.split(',') : ['migration', 'mcp-generated'];
    ...
    const prCreator = new PRCreator(githubToken, {
      owner,
      repo,
      base,
      reviewers,
