...
export class StructureAgent extends AbstractAnalyzerAgent {
  // Propriétés d'identité de l'agent requises par AbstractAnalyzerAgent
  public id: string = 'structureagent';
  public name: string = 'StructureAgent';
  public version: string = '1.0.0'; // À adapter
  public description: string = 'Agent StructureAgent'; // À compléter
  ...
  private analyzeFileStructure(): string {
    ...
    if (forLoops + foreachLoops + whileLoops > 0) {
      structure += `${forLoops + foreachLoops + whileLoops} boucle(s) (${forLoops} for, ${foreachLoops} foreach, ${whileLoops} while). `;
    }
  ...
  private identifyEntryPoints(): string {
    ...
    if (getAccess && getAccess.length > 0) {
      entryPoints += "Paramètres GET: " + getAccess
        .map(match => {
    ...
    if (postAccess && postAccess.length > 0) {
      entryPoints += "Paramètres POST: " + postAccess
        .map(match => {
    ...
    if (requestAccess && requestAccess.length > 0) {
      entryPoints += "Paramètres REQUEST: " + requestAccess
        .map(match => {
    ...
    if (fileContent.includes('header(\'Content-Type: application/json\')') || 
    ...
    if (fileContent.includes('$_SESSION[\'user\'') || 
  ...
  private analyzeTemplateUsage(): string {
    ...
    if (templateFiles.length > 0) {
      templates += `Utilisation de ${templateFiles.length} fichier(s) template : ${templateFiles.join(', ')}. `;
    }
    ...
    if (echoStatements + printStatements > 0) {
      templates += `${echoStatements + printStatements} instruction(s) d'affichage direct (${echoStatements} echo, ${printStatements} print). `;
    }
    
    // Détecter les blocs HTML inline
    const htmlBlocks = fileContent.match(/<[a-z][^>]*>/ig);
