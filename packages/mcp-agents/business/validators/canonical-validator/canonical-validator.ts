...
import { AgentConfig, AgentContext, MCPAgent } from '../packages/mcp-core';
...
export class CanonicalValidator
  implements BaseAgent, BusinessAgent, BaseAgent, BusinessAgent, MCPAgent<CanonicalValidatorConfig>
{
  ...
  private async validateMetaFile(filePath: string): Promise<void> {
    try {
      ...
      const canonicalPath = new URL(
        canonical.startsWith('http') ? canonical : `${this.config.baseUrl}${canonical}`
      ).pathname;
  ...
  private async addCanonicalToFile(filePath: string, canonical: string): Promise<void> {
    try {
      ...
      if (filePath.endsWith('.meta.ts')) {
        ...
        const metaContent = `import type { MetaFunction } from '@remix-run/node';
  ...
  private async updateCanonicalInFile(
    ...
    try {
      ...
      const updatedContent = content
        .replace(
          new RegExp(
            `(rel:\\s*["']canonical["'],\\s*href:\\s*["'])${oldCanonical.replace(
              /[.*+?^${}()|[\]\\]/g,
              '\\$&'
            )}(["'])`
          ),
        ...
        .replace(
          new RegExp(
            `(canonical['"]\s*[,:]\s*["'])${oldCanonical.replace(
              /[.*+?^${}()|[\]\\]/g,
              '\\$&'
            )}(["'])`
          ),
