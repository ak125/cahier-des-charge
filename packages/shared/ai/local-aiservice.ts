...
export class LocalAiService {
   ...
   */
  async generate(prompt: string, options: AiServiceOptions = {}): Promise<string> {
    try {
      ...
      if (this.config.useCache) {
        ...
        this.logger.debug(
          `💾 Résultat mis en cache avec la clé ${cacheKey} (expiration: ${this.config.cacheExpiry}s)`
        );
      }
...
`;
    ...
    const codeOptions = {
      ...
      cacheKey:
        options.cacheKey ||
        this.generateCacheKey(`code:${languageName}:${code.slice(0, 100)}`, options),
    };
...
`;
    ...
    const transformOptions = {
      ...
      cacheKey:
        options.cacheKey ||
        this.generateCacheKey(`transform:php:${phpCode.slice(0, 100)}`, options),
    };
