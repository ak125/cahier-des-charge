 ...
 * /workspaces/cahier-des-charge/packages/migrated-agents/business/generate-agent-manifest.ts
 * 
 * Migré le 2025-05-08
 */
...
async function extractAgentMetadata(filePath: string): Promise<ExtractedAgentMetadata | null> {
  try {
    ...
    function extractJSDocComments(node: ts.Node): Record<string, any> {
      ...
      for (const tag of jsDocTags) {
        ...
        if (ts.isJSDocVersionTag(tag) && tag.version) {
          ...
          result.dependencies = tag.comment
            .toString()
            .split(',')
            .map((d) => d.trim());
        } else if (tagName === 'tags' && tag.comment) {
          result.tags = tag.comment
            .toString()
            .split(',')
            .map((t) => t.trim());
        ...
        node.members.forEach((member) => {
          if (ts.isMethodDeclaration(member) && member.name.getText() === 'getVersion') {
            ...
            if (member.body) {
              ...
              if (returnStatement?.expression && ts.isStringLiteral(returnStatement.expression)) {
            ...
            member.name.getText() === 'getDependencies'
          ) {
    ...
    const _modulePath = `@fafaDoDotmcp-agents/${path.basename(filePath, path.extname(filePath))}`;
...
async function generateManifest(config: GeneratorConfig): Promise<void> {
  try {
    ...
    logger.log(
      `📊 ${agents.length} agents au total, ${activeAgents} actifs, ${inactiveAgents} inactifs`
    );
  } catch (err: any) {
