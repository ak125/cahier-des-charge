...
const migrationsConfigSchema = z.object({
  ...
  validation: z
    .object({
      requiredSteps: z
        .array(z.string())
        .default(['QaAnalyzer', 'seo-analyzer', 'typescript-check']),
...
const domainsConfigSchema = z.object({
  ...
  prioritization: z
    .object({
      factors: z
        .array(z.enum(['complexity', 'dependencies', 'businessValue', 'effort']))
...
const rulesConfigSchema = z.object({
  ...
  rulesets: z.record(
    ...
    z.object({
      ...
      failureAction: z.enum(['abort', 'warn', 'log', 'retry', 'handleGracefully']).default('warn'),
    })
  ),
});
...
const circuitBreakerSchema = z.object({
  ...
  strategy: z
    .enum(['isolateDomain', 'skipValidation', 'revertToSafeState'])
    .default('isolateDomain'),
});
...
export const defaultBusinessConfig: Partial<BusinessConfig> = {
  migrations: {
    ...
    validation: {
      requiredSteps: ['QaAnalyzer', 'seo-analyzer', 'typescript-check'],
      minimumScores: {
        QaAnalyzer: 80,
        'seo-analyzer': 85,
      },
