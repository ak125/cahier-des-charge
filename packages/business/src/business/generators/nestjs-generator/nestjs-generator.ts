...
import { analyzePhpFile } from '../analysis/php-analyzer-agent';
...
export class NestJSGenerator {
   ...
   */
  private generateController(moduleName: string, _dataStructure: DataStructure): string {
...
export class Update${entityName}Dto {
  @IsOptional()
${properties.replace(/^ {2}/gm, '  @IsOptional()\n  ')}
}
...
export class ${entityName}Module {}`;
  ...
  private async generatePrismaSchema(
    dataStructure: DataStructure,
    _analysisResult: PhpAnalysisResult
  ): Promise<string> {
