...
import { TaskType, standardizedOrchestrator } from '../../orchestration/standardized-orchestrator';
...
export class SeoService {
  ...
  async runSeoAudit() {
    try {
      ...
      // Utiliser l'orchestrateur standardisé au lieu du mécanisme personnalisé
      const auditJobId = await standardizedOrchestrator.scheduleTask(
        'seo-audit',
        {
      ...
      this.logger.log(`Audit SEO planifié avec l'ID: ${auditReport.id}, tâche: ${auditJobId}`);
  ...
  async auditSinglePage(url: string) {
    try {
      ...
      // Utiliser l'orchestrateur standardisé pour planifier l'audit de la page
      const auditJobId = await standardizedOrchestrator.scheduleTask(
        'seo-page-audit',
        {
          pageId: page.id,
      ...
      this.logger.log(`Audit SEO de page planifié pour: ${url}, tâche: ${auditJobId}`);
