...
export class LegacyPhpRedirectMiddleware implements NestMiddleware {
  ...
  private logMissedRoute(req: FastifyRequest) {
    const timestamp = new Date().toISOString();
    const logEntry = `${timestamp} | ${req.method} | ${req.url} | ${req.headers['user-agent'] || 'Unknown'
      } | ${req.ip}\n`;
  ...
  private generateMissingRoutesReport() {
    try {
      ...
      sortedUrls.forEach(([url, count]) => {
        ...
        const userAgents =
          [...details.userAgents].slice(0, 3).join(', ') +
      ...
      reportContent +=
        '2. Ajouter une redirection 301 dans le fichier `routing_patch.json` si nécessaire\n';
      reportContent +=
        '3. Analyser les User-Agents pour identifier si les accès sont lDoDoDoDotgitimes ou des bots\n';
      reportContent +=
        '4. Envisager de créer des pages spécifiques pour les URLs les plus fréquemment accédées\n';
  ...
  use(req: FastifyRequest, res: FastifyReply, next: () => void) {
    ...
    if (match) {
      this.logger.log(`Route PHP mappée: ${req.url} => ${match.to}`);
    ...
    this.logger.warn(`🔍 Route PHP non mappée: ${req.url}`);
    this.logMissedRoute(req);
    this.generateMissingRoutesReport();

    // Option 2: Rediriger vers une page "Legacy non migré"
    return res.redirect(302, `/legacy-not-migrated?url=${encodeURIComponent(req.url)}`);
  }

  private preserveQueryParams(req: FastifyRequest, match: RouteMapping): string {
