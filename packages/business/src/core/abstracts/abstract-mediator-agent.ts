...
import { AbstractCoordinationAgent } from './abstract-coordination-agent';
import {
    MediatorAgent,
    MediatorOptions,
    MediatorResult
} from '../interfaces/coordination/mediator/mediator-agent';
import { CoordinationResult } from '../interfaces/coordination/coordination-agent';
 ...
 */
export abstract class AbstractMediatorAgent extends AbstractCoordinationAgent implements MediatorAgent {
    /**
     * Registre des agents
     */
    ...
    protected async checkConnections(): Promise<ConnectionStatus> {
        const services: Record<string, { connected: boolean, error?: string }> = {};
        ...
        for (const [agentId, agent] of this.agentRegistry.entries()) {
            try {
                ...
                services[agentId] = { connected: isConnected };
