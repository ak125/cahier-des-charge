...
export class CascadeValidationService {
  ...
  async validateCascade(request: ValidationRequest): Promise<ValidationResult> {
    ...
    while (currentLevel <= maxLevel) {
      ...
      const levelValid =
        levelResults.every((r) => r.valid) && !levelViolations.some((v) => v.severity === 'error');
  ...
  async validateSingleLevel(
    ...
    const valid = results.every((r) => r.valid) && !violations.some((v) => v.severity === 'error');
