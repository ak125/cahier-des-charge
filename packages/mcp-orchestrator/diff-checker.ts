...
export class DiffChecker {
  ...
  constructor(private readonly config: DiffCheckerConfig = {}) {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      if (
        this.config.templateMappingsPath &&
        (await fs.pathExists(this.config.templateMappingsPath))
      ) {
        this.templateMappings = await fs.readJson(this.config.templateMappingsPath);
        this.logger.log(
          `Mappings de templates chargés: ${Object.keys(this.templateMappings).length} motifs`
        );
      }
  ...
  async compareFiles(phpFilePath: string, tsxFilePath: string): Promise<DiffResult> {
    try {
      this.logger.log(
        `Comparaison de ${path.basename(phpFilePath)} avec ${path.basename(tsxFilePath)}`
      );
      ...
      if (phpHtml && tsxHtml) {
        ...
        result.summary.criticalDifferences = structuralDiffs.filter(
  ...
  private extractJsxOutput(tsxContent: string): string | null {
    try {
      ...
      const jsxRegex =
        /<([A-Z][a-zA-Z]*|[a-z][\w-]*)[\s\S]*?>([\s\S]*?)<\/\1>|<([A-Z][a-zA-Z]*|[a-z][\w-]*)[^>]*\/>/g;
  ...
  private compareElements(
    $php: cheerio.CheerioAPI,
    $tsx: cheerio.CheerioAPI,
    selector: string,
  ...
  private findMissingForms(phpContent: string, tsxContent: string): string[] {
    ...
    while ((match = phpFormRegex.exec(phpContent)) !== null) {
      ...
      const tsxFormRegex = new RegExp(
        `<Form[^>]*name=["']${formName}["'][^>]*>|<Form[^>]*id=["']${formName}["'][^>]*>|useForm\\(.*["']${formName}["'].*\\)`,
        'i'
      );
  ...
  private findMissingFields(phpContent: string, tsxContent: string): string[] {
    ...
    const phpFieldRegex =
      /<input[^>]*name=["']([^"']+)["'][^>]*>|<select[^>]*name=["']([^"']+)["'][^>]*>|<textarea[^>]*name=["']([^"']+)["'][^>]*>/gi;
    ...
    while ((match = phpFieldRegex.exec(phpContent)) !== null) {
      ...
      const tsxFieldRegex = new RegExp(
        `<[^>]*name=["']${fieldName}["'][^>]*>|register\\(.*["']${fieldName}["'].*\\)|useField\\(.*["']${fieldName}["'].*\\)`,
        'i'
      );
  ...
  private findMissingRouteParams(phpContent: string, tsxContent: string): string[] {
    ...
    for (const param of phpParams) {
      ...
      const tsxParamRegex = new RegExp(
        `params\\.${param}|searchParams\\.get\\(["']${param}["']\\)|useSearchParams.*get\\(["']${param}["']\\)`,
        'i'
      );
  ...
  private generateReport(result: DiffResult): string {
    ...
    if (result.details.structuralDifferences.length > 0) {
      ...
      const criticalDiffs = result.details.structuralDifferences.filter(
        (d) => d.importance === 'critical'
      );
      const importantDiffs = result.details.structuralDifferences.filter(
        (d) => d.importance === 'important'
      );
      const minorDiffs = result.details.structuralDifferences.filter(
        (d) => d.importance === 'minor'
      );
    ...
    if (result.summary.criticalDifferences > 0) {
      report +=
        '- ⚠️ **Action requise**: Corriger les éléments critiques manquants avant toute mise en production\n';
    }

    if (result.details.missingForms.length > 0) {
      report +=
        '- Implémenter les formulaires manquants en utilisant les composants Form de Remix\n';
    }
    ...
    if (result.details.missingRouteParams.length > 0) {
      report +=
        "- Vérifier que les paramètres d'URL sont correctement gérés dans les loaders/actions Remix\n";
    }
