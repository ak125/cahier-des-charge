...
import { audit, checkCompletion, dryRun, migrate, qa, verifyRoutes } from './commands';
...
yargs(hideBin(process.argv))
  ...
  .command(
    'migrate <file>',
    'Migre un fichier PHP vers Remix',
    (yargs) => {
  ...
  .command(
    'audit [status]',
    'Audite les fichiers selon leur statut',
    (yargs) => {
    ...
    async (argv) => {
      console.log(
        chalk.blue(`📊 Audit des fichiers ${argv.status ? `avec statut ${argv.status}` : ''}...`)
      );
  ...
  .command(
    'dry-run <file>',
    "Simule la migration d'un fichier sans l'exécuter",
    (yargs) => {
      return yargs.positional('file', {
        type: 'string',
  ...
  .command(
    'generate',
    'Génère ou met à jour le backlogDoDotmcp.json à partir du discovery_map.json',
    () => {},
    async () => {
  ...
  .command(
    'qa <file>',
    "Analyse la qualité de la migration d'un fichier PHP",
    (yargs) => {
      return yargs.positional('file', {
        type: 'string',
  ...
  .command(
    ...
    "Vérifie si la migration d'un fichier est complète",
    (yargs) => {
      return yargs
        .positional('file', {
          type: 'string',
