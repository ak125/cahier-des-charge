...
function findPackageJsonFiles(dir) {
    ...
    try {
        ...
        log(`${colors.yellow}Erreur lors de la lecture du répertoire ${dir}: ${err.message}${colors.reset}`);
        return [];
    }
}
...
if (!fs.existsSync(PACKAGES_DIR)) {
    ...
    try {
        ...
        log(`${colors.yellow}Erreur lors de la lecture du répertoire de base: ${err.message}${colors.reset}`);
    }
...
if (packageFiles.length === 0) {
    ...
    for (const filePath of packageFiles) {
        try {
            ...
            log(`${colors.yellow}⚠️ Erreur lors de la lecture de ${filePath}: ${err.message}${colors.reset}`);
        }
    }
    ...
    if (Object.keys(collisions).length > 0) {
        log(`\n${colors.red}⛔ ${Object.keys(collisions).length} collisions de noms de packages détectées:${colors.reset}`);
...
try {
    ...
    if (maxDepthFound > MAX_DEPTH) {
        log(`${colors.yellow}⚠️ Profondeur maximale de dossiers: ${maxDepthFound} (supérieure au maximum recommandé de ${MAX_DEPTH})${colors.reset}`);
        log(`  Il est recommandé d'aplatir la structure du projet pour améliorer la maintenabilité.`);
    } else {
        log(`${colors.green}✓ Profondeur maximale de dossiers: ${maxDepthFound} (dans les limites recommandées)${colors.reset}`);
    }
} catch (err) {
    log(`${colors.yellow}Erreur lors de la vérification de la profondeur des dossiers: ${err.message}${colors.reset}`);
}
...
if (hasErrors) {
    ...
    reportLines.push("1. Exécuter `node fix-package-name-collisions.js` pour résoudre les collisions de noms");
    reportLines.push("2. Exécuter `bash clean-recursive-structure.sh` pour nettoyer les structures récursives");
} else {
