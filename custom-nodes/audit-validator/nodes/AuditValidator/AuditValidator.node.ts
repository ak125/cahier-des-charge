...
/// <reference types=Dotn8N-workflow" />
 ...
 */
async function runValidation(basePath: string, verbose: boolean, reportPath: string): Promise<any> {
  const args = ['--verbose'];
  if (verbose) {
    args.push('--verbose');
  }
  ...
  try {
    ...
    if (result.error) {
      throw new Error(
        `Erreur lors de l'exécution du script de validation: ${result.error.message}`
      );
    }
    ...
    const summary = {
      ...
      total:
        validationReport.missingFiles.length +
    ...
    throw new Error(
      `Échec de la validation des audits: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
 ...
 */
async function runAutoFix(basePath: string, force: boolean, reportPath: string): Promise<any> {
  const args = [];
  if (force) {
    args.push('--force');
  }
  ...
  try {
    ...
    if (result.error) {
      throw new Error(
        `Erreur lors de l'exécution du script de correction: ${result.error.message}`
      );
    }
    ...
    const summary = {
      ...
      total:
        recheckReport.successful.length +
    ...
    throw new Error(
      `Échec de la correction automatique des audits: ${
