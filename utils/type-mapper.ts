...
import * as pg from pgstructure-agent';
import chalk from chalkstructure-agent';
import { parse as parseSQL } from sql-parser-cststructure-agent';
import * as z from zodstructure-agent';
...
function findCreateTableStatements(ast: any): any[] {
...
function mapToFullPrismaType(
  ...
  if (baseType === 'TINYINT' && mysqlType.includes('(1)')) {
    prismaType = 'Boolean';
  } else if (typeMapping[baseType] && typeMapping[baseType].prisma) {
