...
async function updateCahier(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      console.error(
        chalk.yellow(`⚠️ Erreur lors de la mise à jour de discovery_map.json: ${error.message}`)
      );
    }
...
async function updateDiscoveryMap(discoveryMapPath: string): Promise<void> {
  ...
  for (const item of mapData) {
    ...
    const hasAudit = await fs
      .access(auditPath)
      .then(() => true)
      .catch(() => false);
    const hasBacklog = await fs
      .access(backlogPath)
      .then(() => true)
      .catch(() => false);
    const hasImpactGraph = await fs
      .access(impactGraphPath)
      .then(() => true)
      .catch(() => false);
...
Documentation / Exemples`;
    ...
    for (const auditFile of auditFiles) {
      ...
      if (missingSections.length > 0) {
        console.log(
          chalk.yellow(`⚠️ Sections manquantes dans ${auditFile}, tentative de complétion...`)
        );
  ...
  for (const auditFile of auditFiles) {
    ...
    if (!backlogExists) {
      ...
      try {
        ...
        execSync(
          `ts-node scripts/full-audit.ts --file="${path.join(
            cahierPath,
            auditFile
          )}" --backlog-only`
        );
        ...
        console.error(
          chalk.yellow(`⚠️ Impossible de créer le backlog pour ${auditFile}: ${error.message}`)
        );
      }
    } else {
      // Mise à jour du backlog existant si nécessaire
      const _auditContent = await fs.readFile(path.join(cahierPath, auditFile), 'utf-8');
      ...
      try {
        ...
        if (backlog.tasks.length < 3) {
          console.log(
            chalk.yellow(`⚠️ Peu de tâches dans le backlog pour ${auditFile}, mise à jour...`)
          );
          ...
          try {
            ...
            console.error(
              chalk.yellow(
                `⚠️ Impossible de mettre à jour le backlog pour ${auditFile}: ${error.message}`
              )
            );
          }
        }
        ...
        console.error(
          chalk.yellow(`⚠️ Erreur de parsing JSON pour ${backlogPath}: ${error.message}`)
        );
      }
    }
  }
  ...
  for (const auditFile of auditFiles) {
    ...
    if (!graphExists) {
      ...
      try {
        ...
        execSync(
          `ts-node scripts/full-audit.ts --file="${path.join(cahierPath, auditFile)}" --impact-only`
        );
        console.log(chalk.green(`✅ Graphe d'impact créé pour ${auditFile}`));
      } catch (error) {
        console.error(
          chalk.yellow(
            `⚠️ Impossible de créer le graphe d'impact pour ${auditFile}: ${error.message}`
          )
        );
      }
    }
...
async function updateSummary(cahierPath: string): Promise<void> {
  ...
  const summaryFile = await fs
    .readdir(cahierPath)
    .then((files) =>
      files.find(
        (file) => file === '00-sommaire.md' || file === 'README.md' || file === 'sommaire.md'
      )
    );
  ...
  if (!summaryFile) {
    ...
    const mdFiles = await fs
      .readdir(cahierPath)
      .then((files) =>
        files.filter(
          (file) => file.endsWith('.md') && !file.includes('.audit.md') && file !== summaryFile
        )
      );
    ...
    if (missingFiles.length > 0) {
      console.log(
        chalk.yellow(
          `⚠️ ${missingFiles.length} fichiers non référencés dans le sommaire, mise à jour...`
        )
      );
...
async function generateSummary(cahierPath: string): Promise<string> {
  ...
  const mdFiles = await fs
    .readdir(cahierPath)
    .then((files) =>
      files.filter((file) => file.endsWith('.md') && !file.includes('.audit.md')).sort()
    );
