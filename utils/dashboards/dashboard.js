...
// Déterminer le port à utiliser en fonction du type de vue
let PORT;
if (argv.view === 'agents') {
  PORT = DASHBOARD.AGENTS_PORT;
} else if (argv.view === 'audit') {
  PORT = DASHBOARD.AUDIT_PORT;
} else if (argv.view === 'unified') {
  PORT = DASHBOARD.UNIFIED_PORT;
} else {
  // Vue par défaut (migration)
  PORT = DASHBOARD.PORT;
}
...
app.get('/', (_req, res) => {
  ...
  if (fs.existsSync(layeredDashboardPath)) {
    ...
    const html = `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Tableau de bord ${argv.view || 'migration'}</title>
        <style>
          body {
      ...
      <body>
        <h1>Tableau de bord ${argv.view || 'migration'}</h1>
        
        <div class="info">
          <p><strong>Note :</strong> Ce tableau de bord utilise les données en direct de l'API REST. Les composants Remix complets sont disponibles dans le dossier 'layered-dashboards'.</p>
        </div>
        ...
        <div class="container">
          ...
          <div class="metrics" id="metrics-container">
            <div class="metric-card">
              ...
              <p id="global-progress">Chargement...</p>
            </div>
            <div class="metric-card">
              <h3>Modules</h3>
              <p id="modules-count">Chargement...</p>
            </div>
            <div class="metric-card">
              <h3>Dernière mise à jour</h3>
              <p id="last-updated">Chargement...</p>
            </div>
          </div>
        </div>
          ...
          fetch('/api/status')
            ...
            .then(data => {
              document.getElementById('global-progress').textContent = 
                data.progress && data.progress.global !== undefined 
                  ? \`\${data.progress.global}%\` 
                  : 'Non disponible';
  ...
  // Fallback au cas où le fichier index.html existerait dans le dossier dashboard
  const dashboardPath = path.join(PATHS.DASHBOARD_DIR, 'index.html');
  if (fs.existsSync(dashboardPath)) {
    return res.sendFile(dashboardPath);
  }

  // Générer une page HTML basique si aucun fichier n'est trouvé
  const html = `
    <!DOCTYPE html>
    <html lang="fr">
    <head>
    ...
    <body>
      <h1>Bienvenue sur le tableau de bord de migration</h1>
      <p>Aucune vue spécifique n'est disponible pour le moment.</p>
    </body>
    </html>
  `;
  res.send(html);
