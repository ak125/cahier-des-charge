...
const isIncrementalAnalysis = args.includes('--incremental') || !isFullAnalysis;
...
async function runSeoAnalysis() {
  ...
  try {
    ...
    const outputPath = path.join(CONFIG.qaReportDir, 'seo-analysis-results.json');
...
async function runUsabilityUXAnalysis() {
  ...
  try {
    ...
    console.error('❌ Erreur lors de l\'analyse d\'utilisabilité et d\'expérience utilisateur:', error.message);
    throw error;
  }
}
...
function aggregateResults() {
  ...
  try {
    ...
    const seoResults = JSON.parse(fs.readFileSync(path.join(CONFIG.qaReportDir, 'seo-analysis-results.json'), 'utf8'));
    const performanceResults = JSON.parse(fs.readFileSync(path.join(CONFIG.qaReportDir, 'performance-analysis-results.json'), 'utf8'));
    const accessibilityResults = JSON.parse(fs.readFileSync(path.join(CONFIG.qaReportDir, 'accessibility-analysis-results.json'), 'utf8'));
    const bestPracticesResults = JSON.parse(fs.readFileSync(path.join(CONFIG.qaReportDir, 'best-practices-analysis-results.json'), 'utf8'));
    ...
    dashboardData.averageScore = results.reduce((sum, r) => sum + (r.score || 0), 0) / (results.length || 1);
    ...
    dashboardData.performanceStats.goodPerformanceCount = 
      results.filter(r => (r.performanceScore || 0) >= CONFIG.thresholds.performance).length;
    
    dashboardData.performanceStats.poorPerformanceCount = 
      results.filter(r => (r.performanceScore || 0) < CONFIG.thresholds.performance).length;
    
    dashboardData.accessibilityStats.criticalIssuesCount = 
      results.reduce((sum, r) => sum + (r.accessibilityIssues?.filter(i => i.impact === 'critical').length || 0), 0);
    
    dashboardData.accessibilityStats.seriousIssuesCount = 
      results.reduce((sum, r) => sum + (r.accessibilityIssues?.filter(i => i.impact === 'serious').length || 0), 0);
    
    dashboardData.bestPracticesStats.highSeverityCount = 
      results.reduce((sum, r) => sum + (r.bestPracticesIssues?.filter(i => i.severity === 'high').length || 0), 0);
    
    dashboardData.bestPracticesStats.mediumSeverityCount = 
      results.reduce((sum, r) => sum + (r.bestPracticesIssues?.filter(i => i.severity === 'medium').length || 0), 0);
  ...
  try {
    ...
    if (Math.abs(seoDiff) >= significantChange) {
      alerts.push({
        ...
        category: 'seo',
        message: `Score SEO ${seoDiff < 0 ? 'dégradé' : 'amélioré'} de ${Math.abs(seoDiff).toFixed(1)}%`,
    ...
    if (Math.abs(perfDiff) >= significantChange) {
      alerts.push({
        ...
        message: `Score de performance ${perfDiff < 0 ? 'dégradé' : 'amélioré'} de ${Math.abs(perfDiff).toFixed(1)}%`,
        timestamp: new Date().toISOString(),
    ...
    if (Math.abs(a11yDiff) >= significantChange) {
      alerts.push({
        ...
        message: `Score d'accessibilité ${a11yDiff < 0 ? 'dégradé' : 'amélioré'} de ${Math.abs(a11yDiff).toFixed(1)}%`,
        timestamp: new Date().toISOString(),
    ...
    if (Math.abs(bpDiff) >= significantChange) {
      alerts.push({
        ...
        message: `Score des meilleures pratiques ${bpDiff < 0 ? 'dégradé' : 'amélioré'} de ${Math.abs(bpDiff).toFixed(1)}%`,
        timestamp: new Date().toISOString(),
    ...
    if (currentEntry.avgUsabilityScore && previousEntry.avgUsabilityScore) {
      ...
      if (Math.abs(usabilityDiff) >= significantChange) {
        alerts.push({
          ...
          category: 'usability',
          message: `Score d'utilisabilité ${usabilityDiff < 0 ? 'dégradé' : 'amélioré'} de ${Math.abs(usabilityDiff).toFixed(1)}%`,
          timestamp: new Date().toISOString(),
    ...
    if (currentEntry.avgUXScore && previousEntry.avgUXScore) {
      ...
      if (Math.abs(uxDiff) >= significantChange) {
        alerts.push({
          ...
          category: 'ux',
          message: `Score d'expérience utilisateur ${uxDiff < 0 ? 'dégradé' : 'amélioré'} de ${Math.abs(uxDiff).toFixed(1)}%`,
          timestamp: new Date().toISOString(),
    ...
    Object.entries(currentData.results).forEach(([filePath, result]) => {
      ...
      if (result.history && result.history.length >= 2) {
        ...
        if (scoreDiff <= -10) { // Dégradation de 10% ou plus
          alerts.push({
            type: 'degradation',
            category: 'file',
            file: filePath,
    ...
    if (alerts.length > 0) {
      ...
      if (CONFIG.notifications && (CONFIG.notifications.slack || CONFIG.notifications.teams || CONFIG.notifications.email)) {
        try {
          // Dynamically import the NotificationService
          const { default: NotificationService } = require(../app/services/NotificationServicestructure-agent');
