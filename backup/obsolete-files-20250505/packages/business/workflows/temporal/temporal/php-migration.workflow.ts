...
export const PhpToNestJsMigrationWorkflow = defineWorkflow('PhpToNestJsMigration', async (input: MigrationInput) => {
    ...
    try {
        ...
        // Cette phase extrait la structure du code, les modèles, les contrôleurs, etc.
        logger.info('Phase 1: Analyse du code PHP source');
        const analysisResult = await executeActivity('analyzeLegacyPhp', {
            taskQueue: 'php-analysis',
        ...
        logger.info(`Analyse PHP terminée: ${analysisResult.entityCount} entités détectées`);
