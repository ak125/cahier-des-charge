import phpAnalyzerV2 from '../../packagesDoDotmcp-agents/analyzers/php-analyzer/php-analyzer-v2';
...
async function main() {
  ...
  const interfaceMethods = ['initialize', 'validate', 'execute', 'run', 'validateData', 'generate'];
  ...
  for (const method of interfaceMethods) {
    ...
    if (typeof (phpAnalyzerV2 as any)[method] === 'function') {
      ...
      console.log(
        `✕ Method ${method} is not implemented, should be added for interface compliance`
      );
    }
  }
