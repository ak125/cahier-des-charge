...
const traverse = require('@typescript-eslint/typescript-estree').traverseFast;
...
async function extractMetadata(filePath, config) {
  try {
    ...
    try {
      ...
      console.warn(chalk.yellow(`⚠️ Impossible d'analyser le fichier comme TypeScript, extraction limitée.`));
    }
 ...
 */
function extractSqlModel(content, ast, config) {
 ...
 */
function extractRoutes(content, ast, config) {
  const routes = [];
  ...
  const expressRouteRegex = /\.(get|post|put|delete|patch)\(['"]([^'"]+)['"],\s*(?:async\s*)?\(?([^)]*)\)?\s*=>/g;
 ...
 */
function generateAuditContent(metadata, config) {
...
async function createPullRequest(auditFilePath, metadata, config, branchName) {
  try {
    ...
    if (!githubToken) {
      console.error(chalk.red('❌ Erreur: Token GitHub non trouvé dans les variables d\'environnement'));
...
`
      ...
      if (config.github.default_reviewers.length > 0) {
        ...
        console.log(chalk.green(`✅ Reviewers assignés: ${config.github.default_reviewers.join(', ')}`));
      }
