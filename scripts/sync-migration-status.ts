...
program
  ...
  .option('-c, --commit', 'Create a git commit with changes', false)
  .option('-s, --slack', 'Send Slack notification for critical changes', false)
  .option('-g, --github', 'Create GitHub issues for warnings', false);
...
function createGitCommit(
  ...
  try {
    ...
    execSync('git add **/*.audit.md **/*.backlog.json migration_warnings.json');
