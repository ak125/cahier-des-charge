 ...
 * Identifie les fichiers qui n'utilisent pas encore le package centralisé @fafa/mcp-agents
 */
...
const DIRECTORIES_TO_CHECK = [
  '/workspaces/cahier-des-charge/apps/mcp-server',
...
async function main() {
  ...
  console.log(chalk.blue('\n=== Statut global ==='));
  if (percentage === 100) {
    console.log(chalk.green(`✅ Tous les imports (100%) utilisent le package centralisé @fafa/mcp-agents !`));
  } else {
...
function analyzeFile(filePath: string): void {
  try {
    ...
    const localAgentImports = content.match(/from\s+['"]\.\.?(\/[\w-]+)*\/agents\/([\w-]+)['"];?/g) || [];
    
    // Rechercher les imports centralisés (@fafa/mcp-agents)
    const centralizedAgentImports = content.match(/from\s+['"]@fafa\/mcp-agents(\/[\w-]+)*['"];?/g) || [];

    if (localAgentImports.length > 0) {
      filesWithLocalImports++;
      localImportsCount += localAgentImports.length;
      
      console.log(chalk.yellow(`\nFichier avec imports locaux: ${path.relative('/workspaces/cahier-des-charge', filePath)}`));
      localAgentImports.forEach(importStatement => {
        ...
        if (agentName) {
          console.log(chalk.green(`  À remplacer par: import { ... } from '@fafa/mcp-agents/${agentName}';`));
        }
      });
    }
