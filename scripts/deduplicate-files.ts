...
async function deduplicateFilesByType(
  ...
  try {
    ...
    for (const group of result.groups) {
      ...
      if (uniqueHashes.size < group.files.length) {
        ...
        console.log(
          chalk.yellow(
            `🔄 Traitement des doublons exacts pour ${group.baseFileName}${extension}...`
          )
        );
        ...
        for (const filePath of filesToDelete) {
          try {
            ...
            console.error(
              chalk.red(`  ❌ Erreur lors de la suppression de ${filePath}: ${error.message}`)
            );
          }
        }
        ...
        try {
          ...
          console.error(
            chalk.red(
              `  ❌ Erreur lors de la fusion des fichiers pour ${group.baseFileName}: ${error.message}`
            )
          );
        }
...
async function mergeJsonFiles(targetPath: string, otherPaths: string[]): Promise<void> {
  ...
  for (const filePath of otherPaths) {
    try {
      ...
      console.warn(
        chalk.yellow(
          `  ⚠️ Erreur lors de la lecture/parsing de ${filePath}, ignoré: ${error.message}`
        )
      );
    }
  }
      ...
      if (source[key] && typeof source[key] === 'object' && !Array.isArray(source[key])) {
        ...
        for (const item of source[key]) {
          ...
          const exists = target[key].some((targetItem) => {
            const targetItemStr =
              typeof targetItem === 'object' ? JSON.stringify(targetItem) : targetItem;
...
async function createDeduplicationReport(
  cahierPath: string,
