...
const commands = {
    list: () => {
        ...
        const mainCommands = [
            'dev', 'build', 'test', 'lint', 'format', 'setup',
            'migrate', 'audit', 'docker:up', 'docker:down'
        ];
        ...
        const workflowCommands = [
            'n8n:start', 'n8n:stop', 'n8n:import', 'n8n:logs',
            'start:worker', 'start:client', 'bullmq:start', 'bullmq:stop'
        ];
        ...
        console.log(`  ${colors.fg.white}pnpm run ${colors.fg.cyan}<nom-de-la-commande>${colors.reset}\n`);

        console.log(`${colors.bright}${colors.fg.yellow}Pour les commandes NX spécifiques:${colors.reset}`);
        console.log(`  ${colors.fg.white}nx run ${colors.fg.cyan}<target>${colors.reset}`);
        console.log(`  ${colors.fg.white}nx run ${colors.fg.cyan}<target> --configuration=<config>${colors.reset}\n`);
