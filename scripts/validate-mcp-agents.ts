...
function getAgentsFromDirectory(): string[] {
  const agents: string[] = [];
  const agentsBasePath = path.resolve(__dirname, '../packages/mcp-agents');
...
async function main() {
  ...
  if (isModuleAvailable('@fafa/mcp-agents')) {
    try {
      const mcpAgents = require('@fafa/mcp-agents');
      agentRegistry = mcpAgents.agentRegistry || {};
      agentNames = Object.keys(agentRegistry);
      console.log(chalk.green('✅ Package @fafa/mcp-agents trouvé et chargé avec succès.'));
    } catch (error: any) {
      console.log(chalk.yellow('⚠️ Package @fafa/mcp-agents trouvé mais erreur lors du chargement:'), error.message || String(error));
      agentNames = getAgentsFromDirectory();
    }
  } else {
    console.log(chalk.yellow('⚠️ Package @fafa/mcp-agents non trouvé. Recherche des agents dans le répertoire...'));
    agentNames = getAgentsFromDirectory();
  }
  ...
  if (agentNames.length === 0) {
    console.log(chalk.yellow('  Aucun agent trouvé. Vérifiez que le package @fafa/mcp-agents est correctement configuré.'));
  } else {
  ...
  const sourceDirectories = [
    path.resolve(__dirname, '../apps/mcp-server'),
          ...
          if (relativeAgentImports && relativeAgentImports.length > 0) {
            for (const importStr of relativeAgentImports) {
              ...
              if (agentName && agentNames.includes(agentName)) {
                ...
                console.log(chalk.green(`    À remplacer par: import { ${agentName}Agent } from '@fafa/mcp-agents';`));
              }
            }
          }
          
          // Rechercher les imports corrects depuis le package centralisé
          const correctAgentImports = content.match(/from\s+['"]@fafa\/mcp-agents(\/[\w-]+)*['"];?/g);
  ...
  console.log(`  - ${chalk.green(`${correctImports} imports corrects`)} depuis @fafa/mcp-agents`);
  console.log(`  - ${chalk.yellow(`${incorrectImports} imports incorrects`)} (imports relatifs à remplacer)`);
  
  if (incorrectImports === 0) {
    console.log(chalk.green('\n✅ Tous les imports d\'agents utilisent le package centralisé @fafa/mcp-agents!'));
  } else {
    console.log(chalk.yellow(`\n⚠️  Attention: ${incorrectImports} imports ne sont pas encore migrés vers le package centralisé.`));
    console.log('   Remplacez-les par des imports depuis @fafa/mcp-agents pour éviter les doublons.');
  }

  // Suggestions pour la suite
  console.log(chalk.cyan('\nÉtapes suivantes recommandées:'));
  if (!isModuleAvailable('@fafa/mcp-agents')) {
    console.log(chalk.yellow(`1. Initialisez correctement le package @fafa/mcp-agents s'il n'est pas encore créé.`));
    ...
    console.log(chalk.yellow(`1. Remplacez progressivement les imports relatifs par des imports depuis @fafa/mcp-agents.`));
    ...
    console.log(chalk.green(`2. Pensez à ajouter de nouveaux agents directement dans le package @fafa/mcp-agents.`));
  }
}
