...
async function main() {
  try {
    console.log(chalk.blue('🔍 Vérification des technologies obsolètes dans le cahier des charges...'));
    ...
    if (obsoleteTechs.length > 0) {
      ...
      if (criticalTechs.length > 0) {
        console.log(chalk.red(`❌ ${criticalTechs.length} technologies critiquement obsolètes détectées!`));
        process.exit(1);
      }
    } else {
...
async function loadTechDatabase() {
  try {
    ...
    console.log(chalk.yellow('⚠️ Base de données des technologies non trouvée, création d\'une nouvelle'));
    techDatabase = { 
      technologies: {},
...
function calculateObsolescenceScore(techName, techInfo) {
  ...
  if (techData && techData.obsolescenceScore) {
...
async function generateReport(obsoleteTechs) {
  ...
  const sortedTechs = [...obsoleteTechs].sort((a, b) => 
    b.obsolescenceScore - a.obsolescenceScore
  );
  ...
  reportContent += `- **Niveau critique** (score ≥ 75): ${sortedTechs.filter(t => t.obsolescenceScore >= 75).length}\n`;
  reportContent += `- **Niveau préoccupant** (score 50-74): ${sortedTechs.filter(t => t.obsolescenceScore >= 50 && t.obsolescenceScore < 75).length}\n`;
  reportContent += `- **Niveau à surveiller** (score < 50): ${sortedTechs.filter(t => t.obsolescenceScore < 50).length}\n\n`;
  ...
  for (const tech of sortedTechs) {
    const severity = tech.obsolescenceScore >= 75 ? '🔴' : 
                    tech.obsolescenceScore >= 50 ? '🟠' : '🟡';
    
    reportContent += `| ${severity} ${tech.name} | ${tech.version || 'N/A'} | ${tech.obsolescenceScore} | ${tech.category} | ${tech.alternatives.join(', ')} | ${tech.file} |\n`;
  }
  ...
  if (criticalTechs.length > 0) {
    ...
    for (const tech of criticalTechs) {
      reportContent += `- Remplacer **${tech.name}** ${tech.version ? `v${tech.version}` : ''} par ${tech.alternatives[0]}\n`;
    }
  ...
  const concerningTechs = sortedTechs.filter(t => t.obsolescenceScore >= 50 && t.obsolescenceScore < 75);
  if (concerningTechs.length > 0) {
    reportContent += `### Planification à moyen terme\n\n`;
    
    for (const tech of concerningTechs) {
      reportContent += `- Envisager la migration de **${tech.name}** ${tech.version ? `v${tech.version}` : ''} vers ${tech.alternatives[0]}\n`;
    }
