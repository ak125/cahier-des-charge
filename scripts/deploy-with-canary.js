...
async function performRollback() {
  ...
  try {
    ...
    await execAsync('node scripts/notify-rollback.js "Rollback automatique suite à un dépassement des métriques"');
    ...
    await execAsync('node scripts/notify-emergency.js "Échec du rollback automatique - Intervention manuelle requise"');
...
async function canaryDeploy() {
  ...
  for (const targetPercentage of config.percentageIncrements) {
    ...
    console.log(`📊 Métriques actuelles: Error Rate=${metrics.errorRate}%, Response Time=${metrics.responseTime}ms, CPU=${metrics.cpuUsage}%`);
