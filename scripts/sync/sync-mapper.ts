...
class SyncMapper {
    ...
    for (const table of sqlTables) {
      ...
      for (const column of table.columns) {
        ...
        if (field) {
          ...
          console.warn(`⚠️ Pas de champ correspondant à ${column.name} dans le modèle ${model.name}.`);
        }
      }
    ...
    for (const table of sqlTables) {
      ...
      for (const column of table.columns) {
        if (column.name.endsWith('_id')) {
          ...
          const referencedTable = sqlTables.find(t => 
            t.name === referencedTableName || t.name === pluralTableName
          );
  ...
  private writeMappingFiles(): void {
    ...
    fs.writeFileSync(
      migrationPatchPath,
      JSON.stringify(this.migrationPatch, null, 2),
      'utf8'
    );
    ...
    fs.writeFileSync(
      phpSqlLinksPath,
      JSON.stringify(this.phpSqlLinks, null, 2),
      'utf8'
    );
