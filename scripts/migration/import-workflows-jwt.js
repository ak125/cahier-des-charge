...
const JWT_TOKEN =
  process.env.N8N_JWT_TOKEN ||
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJmYTUyMDlhNy00MjZkLTQyODctYjZiNi0yMzAwNWVhZDM0YjkiLCJpc3MiOiJuOG4iLCJhdWQiOiJwdWJsaWMtYXBpIiwiaWF0IjoxNzQ0MzI2MzA4fQ.gpWfP0WtHe1ke7OppwbWxvUzAqgCai8ftijuGHu5EJM';
...
async function testConnection() {
  try {
    ...
    console.log(
      `✅ Connexion réussie! ${result.data ? result.data.length : 0} workflows existants trouvés.`
    );
    return true;
  } catch (error) {
...
async function createWorkflow(workflow) {
  try {
    ...
    console.error(
      `❌ Erreur lors de l'importation du workflow ${workflow.name || workflow.id || 'sans nom'}: ${
        error.message
      }`
    );
    return null;
...
async function main() {
  try {
    ...
    if (!connected) {
      console.error(
        '❌ Impossible de se connecter à n8n. Vérifiez la connexion et les identifiants.'
      );
      process.exit(1);
    }
    ...
    if (fs.existsSync(workflowsDir)) {
      ...
      console.log(
        `📊 ${files.length} fichiers de workflow trouvés dans le dossier ${workflowsDir}`
      );
    ...
    if (fs.existsSync(configDir)) {
      const files = fs
        ...
        .filter((f) => f.endsWith('.json') && f.includes('n8n'));
      console.log(
        `📊 ${files.length} fichiers de configuration n8n trouvés dans le dossier ${configDir}`
      );
