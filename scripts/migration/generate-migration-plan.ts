 ...
 *   --inject-supabase: Injecte directement le backlog dans Supabase viaDotn8N
...
import { analyzeWithAdvancedTools } from './enhanced-PhpAnalyzer';
...
// Configuration
const _N8N_WEBHOOK_URL =
  process.env.N8N_WEBHOOK_URL || 'http://localhost:5678/webhook/migration-plan';
const SUPABASE_WEBHOOK_URL =
  process.env.SUPABASE_WEBHOOK_URL || 'http://localhost:5678/webhook/backlog-inject';

/**
 * Analyse un fichier PHP pour générer un plan de migration
 */
async function analyzePHPFile(
  filePath: string,
  ...
  const _formHandling = fileContent.includes('$_POST') || fileContent.includes('$_GET');
  ...
  const globalScore = Math.ceil(
    (score.couplageInterne +
      score.complexiteMetier +
      score.qualiteTechnique +
      score.dependancesExternes +
      score.volumeLignes) /
      5
  );
  ...
  if (advancedAnalysis) {
    ...
    if (advancedAnalysis.metrics.cyclomaticComplexity > 20) {
      difficulties.push(
        `Complexité cyclomatique élevée (${advancedAnalysis.metrics.cyclomaticComplexity})`
      );
    }

    if (advancedAnalysis.metrics.maintainabilityIndex < 50) {
      difficulties.push(
        `Faible indice de maintenabilité (${advancedAnalysis.metrics.maintainabilityIndex.toFixed(
          2
        )}/100)`
      );
    }
    ...
    if (advancedAnalysis.legacyCode.unsafeConstructs.length > 0) {
      difficulties.push(
        `Constructions non sécurisées (${advancedAnalysis.legacyCode.unsafeConstructs.join(', ')})`
      );
    }
    ...
    if (
      ...
      (fileContent.includes("$_GET['page']") || fileContent.includes('$_GET["page"]'))
    ) {
  ...
  const migrationPlanNestJS: MigrationTargetNestJS[] = [
    {
      elementPhp: 'Requêtes SQL',
      cibleNestJS: `Prisma dans ${fileName.replace('.php', '')}.service.ts`,
    },
    {
      elementPhp: `Blocs métier (${isCart ? 'panier' : isAuth ? 'auth' : 'page'})`,
      cibleNestJS: `${fileName.replace('.php', '')}Service + ${fileName.replace(
        '.php',
        ''
      )}Controller`,
    },
  ...
  if (sessionUsage) {
    migrationPlanNestJS.push({
      elementPhp: 'Auth via session',
      cibleNestJS: 'Passport + RedisSessionStore',
    });
  }
  ...
  if (sessionUsage) {
    migrationPlanRemix.push({
      elementPhp: 'États de session',
      cibleRemix: 'useFetcher() ou sessionStorage',
    });
  }

  // Spécialisation backend / frontend
  const specializationBackendFrontend = {
    nestjs: ['Accès DB, logique métier', 'Auth / Session', 'Validation Zod côté API'],
    remix: ['UI, interactivité, routing', 'Formulaires, rendering', 'conform + zod côté form'],
  };
  ...
  const recommendations = [
    ...
    `✅ Créer module ${fileName.replace('.php', '')} complet dans @fafa/backend/${fileName.replace(
      '.php',
      ''
    )}`,
    `🧩 Générer automatiquement :\n\n${fileName.replace(
      '.php',
      ''
    )}.controller.ts, ${fileName.replace('.php', '')}.service.ts, ${fileName.replace(
      '.php',
      ''
  ...
  if (advancedAnalysis) {
    ...
    if (advancedAnalysis.metrics.maintainabilityIndex < 60) {
      recommendations.push(
        '⚠️ Effectuer un refactoring préalable pour améliorer la maintenabilité avant migration'
      );
    }

    if (advancedAnalysis.typeCoverage < 50) {
      recommendations.push(
        '⚠️ Créer des interfaces TypeScript bien définies pour compenser la faible couverture de type'
      );
    }
  ...
  if (advancedAnalysis) {
    if (
      advancedAnalysis.metrics.maintainabilityIndex < 40 ||
      advancedAnalysis.legacyCode.unsafeConstructs.length > 0
    ) {
      migrationWave = 3; // Les fichiers très problématiques en dernière vague
    } else if (
      advancedAnalysis.metrics.cyclomaticComplexity > 20 ||
      advancedAnalysis.metrics.codeSmells > 15
    ) {
  ...
  return {
    ...
    impact: {
      ...
      requiresDedicatedWave: advancedAnalysis
        ? advancedAnalysis.metrics.cyclomaticComplexity > 15 ||
          advancedAnalysis.metrics.maintainabilityIndex < 50
    ...
    difficulty:
      globalScore >= 4
        ? '📌 Difficulté élevée, nécessite phase intermédiaire de découplage.'
        : globalScore >= 3
          ? '📌 Difficulté moyenne, refactoring préalable recommandé.'
...
|----------|--------|----------------|
| Complexité cyclomatique | ${plan.advancedMetrics.cyclomaticComplexity} | ${
      plan.advancedMetrics.cyclomaticComplexity > 20
        ? '⚠️ Élevée'
        : plan.advancedMetrics.cyclomaticComplexity > 10
          ? '⚠️ Moyenne'
          : '✅ Bonne'
    } |
| Indice de maintenabilité | ${plan.advancedMetrics.maintainabilityIndex.toFixed(2)}/100 | ${
      plan.advancedMetrics.maintainabilityIndex < 40
        ? '⚠️ Faible'
        : plan.advancedMetrics.maintainabilityIndex < 60
          ? '⚠️ Moyenne'
          : '✅ Bonne'
    } |
| Couverture de type | ${plan.advancedMetrics.typeCoverage.toFixed(2)}% | ${
      plan.advancedMetrics.typeCoverage < 50
        ? '⚠️ Faible'
        : plan.advancedMetrics.typeCoverage < 70
          ? '⚠️ Moyenne'
          : '✅ Bonne'
    } |
| Code smells | ${plan.advancedMetrics.codeSmells} | ${
      plan.advancedMetrics.codeSmells > 15
        ? '⚠️ Nombreux'
        : plan.advancedMetrics.codeSmells > 7
          ? '⚠️ Quelques-uns'
          : '✅ Peu'
    } |
| Problèmes de sécurité | ${plan.advancedMetrics.securityIssues} | ${
      plan.advancedMetrics.securityIssues > 0 ? '⚠️ À corriger' : '✅ Aucun'
    } |`;
  };
...
${
  plan.impact.requiresDedicatedWave
    ? "✅ Doit être migré dans une vague dédiée, avec contrôle d'intégrité."
    : '✅ Peut être migré dans une vague standard.'
}
...
${plan.outputs.generatedModule} → généré via ComponentGenerator MCP
${
...
/**
 * Injecte le backlog dans Supabase via un webhookDotn8N
 */
...
async function main() {
  ...
  // Si --inject-supabase, envoyer le backlog à Supabase viaDotn8N
  if (injectSupabase) {
