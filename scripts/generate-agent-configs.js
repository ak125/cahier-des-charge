...
agents.forEach(agent => {
  ...
  const config = {
    ...
    maxTokens: 4096,
    temperature: 0.2,
    systemPrompt: `You are an expert ${agent.name.replace('-', ' ')} that helps with code migration.`,
    autoRetry: true,
    retryCount: 3
  };
  ...
  try {
    ...
    console.error(chalk.red(`❌ Erreur lors de la génération de la configuration pour ${agent.name}: ${error.message}`));
  }
});
